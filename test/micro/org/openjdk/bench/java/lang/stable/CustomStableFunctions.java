...
final class CustomStableFunctions {
    ...
    static <T> Predicate<T> cachingPredicate(Set<? extends T> inputs,
        ...
        final Function<T, Boolean> delegate = StableValue.ofFunction(inputs, original::test);
    ...
    static <T, U, R> BiFunction<T, U, R> cachingBiFunction(Set<Pair<T, U>> inputs,
        ...
        final Map<T, Function<U, R>> map = tToUs.entrySet().stream()
                .map(e -> Map.entry(e.getKey(), StableValue.<U, R>ofFunction(e.getValue(), (U u) -> original.apply(e.getKey(), u))))
    ...
    static <T> UnaryOperator<T> cachingUnaryOperator(Set<? extends T> inputs,
        ...
        final Function<T, T> function = StableValue.ofFunction(inputs, original);
        return function::apply;

    }

    static IntSupplier cachingIntSupplier(IntSupplier original) {
        final Supplier<Integer> delegate = StableValue.ofSupplier(original::getAsInt);
        return delegate::get;
    }
