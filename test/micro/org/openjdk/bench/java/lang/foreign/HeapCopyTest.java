...
public class HeapCopyTest {
    ...
    // If confined arena or auto, then we can access without risking closing from underneath

    // Intel to drop 512b SIMD? AMD has 256b
    // Maybe 256b is faster?

    // Use 256b


    private static final boolean IS_X64 = Architecture.isX64();

    private static final VectorSpecies<Byte> SPECIES = VectorSpecies.ofLargestShape(byte.class);

    private static final VectorSpecies<Byte> SPECIES512 = vectorSpeciesOrNull(VectorShape.S_512_BIT);
    private static final VectorSpecies<Byte> SPECIES256 = vectorSpeciesOrNull(VectorShape.S_256_BIT);
    private static final VectorSpecies<Byte> SPECIES128 = vectorSpeciesOrNull(VectorShape.S_128_BIT);

    private static final int STEP = SPECIES.vectorByteSize();
    ...
    public void setup() {
        ...
        int upperBound = (int) SPECIES.loopBound(srcSegment.byteSize()) / SPECIES.length();
        System.out.println("upperBound = " + upperBound);
        System.out.println("SPECIES.length() = " + SPECIES.length());
        System.out.println("SPECIES.vectorByteSize() = " + SPECIES.vectorByteSize());
        System.out.println("SPECIES256 = " + SPECIES256);
        System.out.println("SPECIES128 = " + SPECIES128);
    }
