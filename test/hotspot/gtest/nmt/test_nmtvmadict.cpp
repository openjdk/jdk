...
struct Implementation {
  ...
  static void register_reservation(address addr, size_t size, MEMFLAGS f) {
    if (new_impl) {
      if (dolock) {
        ThreadCritical tc;
        VirtualMemoryView::Interface::reserve_memory(addr, size, f, CURRENT_PC);
      } else {
        VirtualMemoryView::Interface::reserve_memory(addr, size, f, CURRENT_PC);
      }
    } else {
  ...
  static void register_commit(address addr, size_t size, MEMFLAGS f) {
    if (new_impl) {
      if (dolock) {
        ThreadCritical tc;
        VirtualMemoryView::Interface::commit_memory(addr, size, CURRENT_PC);
      } else {
        VirtualMemoryView::Interface::commit_memory(addr, size, CURRENT_PC);
      }
    } else {
  ...
  static void register_uncommit(address addr, size_t size, MEMFLAGS f) {
    if (new_impl) {
      if (dolock) {
        ThreadCritical tc;
        VirtualMemoryView::Interface::uncommit_memory(addr, size);
      } else {
      }
    } else {
      if (dolock) {
