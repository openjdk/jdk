...
// A RAII object reserving a range; needed to isolate the tests from each other and from
// the rest of the VM
class ReservedAddressRange {
  char* const _base;
  const size_t _size;
  static char* reserve(size_t s) {
    return os::reserve_memory(s, false, mtTest);
  }
public:
  ReservedAddressRange(size_t size) : _base(reserve(size)), _size(size) {}
