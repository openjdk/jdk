...
public:
  ...
  static void test_merging() {
    auto& reserved_ranges = vmv._virt_mem.reserved_regions.at(space.id);
...
VirtualMemoryView VirtualMemoryViewTest::vmv{true /*is_detailed_mode*/};
VirtualMemoryView::PhysicalMemorySpace VirtualMemoryViewTest::space{0}; // Doesn't matter, will make new one during construction

TEST_VM_F(VirtualMemoryViewTest, TestOne) {
  VirtualMemoryViewTest::test_merging();
}
