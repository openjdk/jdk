...
void JNICALL ClassPrepare(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    err = (*jvmti_env)->GetClassSignature(jvmti_env, cls, &inf.sig, &generic);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetClassSignature#%" PRIuPTR ") unexpected error: %s (%d)\n",
               eventsCount, TranslateError(err), err);
        result = STATUS_FAILED;
    }
    err = (*jvmti_env)->GetClassStatus(jvmti_env, cls, &inf.status);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetClassStatus#%" PRIuPTR ") unexpected error: %s (%d)\n",
               eventsCount, TranslateError(err), err);
        result = STATUS_FAILED;
    }
    err = (*jvmti_env)->GetClassMethods(jvmti_env, cls, &inf.mcount, &methods);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetClassMethods#%" PRIuPTR ") unexpected error: %s (%d)\n",
               eventsCount, TranslateError(err), err);
        result = STATUS_FAILED;
    }
    err = (*jvmti_env)->GetClassFields(jvmti_env, cls, &inf.fcount, &fields);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetClassMethods#%" PRIuPTR ") unexpected error: %s (%d)\n",
               eventsCount, TranslateError(err), err);
        result = STATUS_FAILED;
    }
    err = (*jvmti_env)->GetImplementedInterfaces(jvmti_env, cls,
    ...
    if (printdump == JNI_TRUE) {
        ...
        for (i = 0; i < inf.fcount; i++) {
            ...
            if (fields[i] == NULL) {
                ...
                err = (*jvmti_env)->GetFieldName(jvmti_env, cls, fields[i],
                    &name, &sig, &generic);
                if (err == JVMTI_ERROR_NONE) {
                    printf(" \"%s, %s\"", name, sig);
                } else {
        ...
        for (i = 0; i < inf.icount; i++) {
            ...
            if (interfaces[i] == NULL) {
                ...
                err = (*jvmti_env)->GetClassSignature(jvmti_env,
...
Java_nsk_jvmti_ClassPrepare_classprep001_getReady(JNIEnv *env, jclass cls) {
    ...
    if ((err = ((*jvmti)->GetCurrentThread(jvmti, &prep_thread))) != JVMTI_ERROR_NONE) {
