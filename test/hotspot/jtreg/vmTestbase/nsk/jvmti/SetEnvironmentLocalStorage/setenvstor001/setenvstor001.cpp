...
callbackVMInit(jvmtiEnv* jvmti, JNIEnv* jni, jthread thread) {
    ...
    if (!checkEnvStorage(jvmti, initialStorage, "VM_INIT callback")) {
        nsk_jvmti_setFailStatus();
    }
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_setenvstor001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_setenvstor001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_setenvstor001(JavaVM *jvm, char *options, void *reserved) {
...
jint Agent_Initialize(JavaVM *jvm, char *options, void *reserved) {
    ...
    NSK_DISPLAY0(">>> Testcase #0: Set env storage in JVM_OnLoad()\n");
    if (!setEnvStorage(jvmti, initialStorage, "JVM_OnLoad()")) {
        nsk_jvmti_setFailStatus();
    }

    NSK_DISPLAY0(">>> Testcase #1: Check env storage in JVM_OnLoad()\n");
    if (!checkEnvStorage(jvmti, initialStorage, "JVM_OnLoad()")) {
        nsk_jvmti_setFailStatus();
    }
