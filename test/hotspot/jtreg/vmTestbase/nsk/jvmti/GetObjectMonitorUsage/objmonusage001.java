...
if (CHECK_FOR_BAD_RESULTS) {
                check(NUMBER_OF_THREADS + i, syncObject[i],
                      mainThread, 1, 1, runn[i], 1, runn[i]);
} else {
                check(NUMBER_OF_THREADS + i, syncObject[i],
                      mainThread, 1, 0, null, 1, runn[i]);
}
            }
...
//
if (objmonusage001.CHECK_FOR_BAD_RESULTS) {
            objmonusage001.check(ind, syncObject, this, 1, 1, mainThread,
                                 1, mainThread);
} else {
            objmonusage001.check(ind, syncObject, this, 1, 0, null,
                                 1, mainThread);
}
            syncObject.notify();
            try {
