...
Java_nsk_jvmti_GetFrameLocation_frameloc003_check(JNIEnv *env, jclass cls, jthread thr) {
    ...
    err = (*jvmti)->GetFrameLocation(jvmti, cls, 0, &mid, &loc);
    ...
    err = (*jvmti)->GetFrameLocation(jvmti, thr, -1, &mid, &loc);
    ...
    err = (*jvmti)->GetFrameLocation(jvmti, thr, 0, NULL, &loc);
    ...
    err = (*jvmti)->GetFrameLocation(jvmti, thr, 0, &mid, NULL);
    if (err != JVMTI_ERROR_NULL_POINTER) {
        printf("Error expected: JVMTI_ERROR_NULL_POINTER,\n");
        printf("        actual: %s (%d)\n", TranslateError(err), err);
        result = STATUS_FAILED;
