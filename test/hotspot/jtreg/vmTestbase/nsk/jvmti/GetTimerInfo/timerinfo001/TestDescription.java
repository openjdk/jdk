 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jvmti/GetTimerInfo/timerinfo001.
 * VM Testbase keywords: [quick, jpda, jvmti, noras]
 * VM Testbase readme:
 * DESCRIPTION
 *     This JVMTI test exercises JVMTI thread function GetTimerInfo().
 *     This test checks that GetTimerInfo() returns info structure
 *     with correct values and produce no errors. And these values
 *     are not changed between calls.
 *     The following checks are performed for GetTimerInfo():
 ...
 *     Each check passes if GetTimerInfo() returns not negative or zero
 *     value in max_value field, and values of all fields are equal to
 *     initial values.
