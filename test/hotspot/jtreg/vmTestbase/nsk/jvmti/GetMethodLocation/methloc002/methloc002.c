...
Java_nsk_jvmti_GetMethodLocation_methloc002_check(JNIEnv *env, jclass cls) {
    ...
    err = (*jvmti)->GetMethodLocation(jvmti, NULL, &startLocation, &endLocation);
    ...
    err = (*jvmti)->GetMethodLocation(jvmti, mid, NULL, &endLocation);
    ...
    err = (*jvmti)->GetMethodLocation(jvmti, mid, &startLocation, NULL);
    if (err != JVMTI_ERROR_NULL_POINTER) {
    ...
    err = (*jvmti)->GetMethodLocation(jvmti, mid, &startLocation, &endLocation);
