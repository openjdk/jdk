...
Java_nsk_jvmti_unit_clsldrclss00x_check(JNIEnv *env, jclass appCls, jclass objCls) {
    ...
    err = (*jvmti)->GetClassLoader(jvmti, appCls, &appClassloader);
    ...
    err = (*jvmti)->GetClassLoader(jvmti, objCls, &objClassloader);
    if (err != JVMTI_ERROR_NONE) {
    ...
    err = (*jvmti)->GetClassLoaderClasses(jvmti, appClassloader, &classCount, &classes);
    if (err != JVMTI_ERROR_NONE) {
      ...
      if (JNI_FALSE) {
        for (i = 0; i < classCount; ++i) {
          ...
          if (err != JVMTI_ERROR_NONE) {
            ...
            printf("    %s\n", classSig);
            err = (*jvmti)->Deallocate(jvmti, (unsigned char*)classSig);
            if (err != JVMTI_ERROR_NONE) {
    ...
    for (i = 0; i < classCount; ++i) {
      jclass k = classes[i];
      if ( (*env)->IsSameObject(env, k, appCls) ) {
        if (printdump) {
    ...
    err = (*jvmti)->GetClassLoaderClasses(jvmti, objClassloader, &classCount, &classes);
    if (err != JVMTI_ERROR_NONE) {
    ...
    for (i = 0; i < classCount; ++i) {
      jclass k = classes[i];
      if ( (*env)->IsSameObject(env, k, objCls) ) {
        if (printdump) {
