...
static const char *cls_exp = "Lnsk/jvmti/unit/ForceEarlyReturn/earlyretstr$earlyretThread;";

static jstring str_exp = NULL;
static const char  *sig_exp = "(I)Ljava/lang/String;";
static const char *name_exp = "countDownString";
...
void JNICALL SingleStep(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    if (method == midRun) {
        ...
        framesCount++;
        err = (*jvmti_env)->ForceEarlyReturnObject(jvmti_env, thread, str_exp);
        if (err != JVMTI_ERROR_NONE) {
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_earlyretstr(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_earlyretstr(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_earlyretstr(JavaVM *jvm, char *options, void *reserved) {
...
Java_nsk_jvmti_unit_ForceEarlyReturn_earlyretstr_getReady(
    JNIEnv *env, jclass c, jclass cls, jint depth, jstring expected_str) {
    jvmtiError err;
    ...
    if (err != JVMTI_ERROR_NONE) {
        ...
        str_exp = JNI_ENV_PTR(env)->NewGlobalRef(JNI_ENV_ARG(env, expected_str));
        framesExpected = depth;
    }
}

JNIEXPORT jint JNICALL
Java_nsk_jvmti_unit_ForceEarlyReturn_earlyretstr_check(JNIEnv *env, jclass cls) {
