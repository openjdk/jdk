...
Java_nsk_jvmti_unit_functions_nosuspendMonitorInfo_JvmtiTest_CheckMonitorInfo(JNIEnv * env, jclass cls, jthread thr, jobject oobj, jint expected_count) {
  ...
  debug_printf(" jvmti GetMonitorInfo \n");
    ...
    ret = JVMTI_ENV_PTR(jvmti)->GetOwnedMonitorInfo(JVMTI_ENV_ARG1(jvmti), thr, &count , &owned_monitor);
    ...
    if (expected_count !=0 ) {
        ...
        ret = JVMTI_ENV_PTR(jvmti)->GetCurrentContendedMonitor(JVMTI_ENV_ARG1(jvmti), thr,  owned_monitor);
        if (ret != JVMTI_ERROR_NONE) {
