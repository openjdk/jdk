 ...
 * @test
 *
 * @summary converted from VM Testbase nsk/jvmti/GetCurrentThreadCpuTimerInfo/curthrtimerinfo001.
 * VM Testbase keywords: [quick, jpda, jvmti, onload_only_logic, noras]
 * VM Testbase readme:
 * DESCRIPTION
 *     This JVMTI test exercises JVMTI thread function GetCurrentThreadCpuTimerInfo().
 *     This test checks that GetCurrentThreadCpuTimerInfo() returns info structure
 *     with correct values and produce no errors. And these values are not
 *     changed between calls.
 *     The following checks are performed for GetCurrentThreadCpuTimerInfo():
 *         - in VM_INIT event callback on live phase
 *         - in agent thread on live phase
 *         - in THREAD_START event callback on live phase
 *         - in THREAD_END event callback on live phase
 *         - in VM_DEATH callback on live phase
 *     Each check passes if GetCurrentThreadCpuTimerInfo() returns not negative
 *     or zero value in max_value field, and values of all fields are equal to
 *     initial values.
 *     Debuggee class runs special tested thread to generate THREAD_START
 *     and THREAD_END events.
