...
static char *exp_class_sig = "Ljava/lang/Object;";
static char *arr_sigs[] = {"[B", "[C", "[D", "[F", "[I", "[J", "[S", "[Z",
    "[Lnsk/jvmti/GetMethodDeclaringClass/declcls003;",
    "[[Lnsk/jvmti/GetMethodDeclaringClass/declcls003;"
};

void do_check(JNIEnv *env, char *name, char *meth, char *sig) {
    jvmtiError err;
    jclass cl;
    jclass ret_decl_cl;
    jmethodID mid;
    ...
    err = (*jvmti)->GetMethodDeclaringClass(jvmti, mid, &ret_decl_cl);
    ...
    err = (*jvmti)->GetClassSignature(jvmti, ret_decl_cl, &cl_sig, &generic);
    if (err != JVMTI_ERROR_NONE) {
        printf("%s.%s%s: ", name, meth, sig);
