...
callbackClassLoad(jvmtiEnv *jvmti,
    ...
    if (strcmp(className,CLASS_NAME) == 0) {
        jmethodID method;
        method = (*jni)->GetMethodID(jni,klass,METHOD_NAME,METHOD_SIGN);
        if (method != NULL) {
            jlocation start;
            jlocation end;
            jvmtiError err ;
            ...
            if (err == JVMTI_ERROR_NONE) {
                ...
                err= (*jvmti)->SetBreakpoint(jvmti,method, start);
                if ( err == JVMTI_ERROR_NONE) {
...
void JNICALL callbackSingleStep(jvmtiEnv *jvmti,
    ...
    threadClass = (*jni)->FindClass(jni,SEARCH_NAME);
...
Java_nsk_jvmti_scenarios_hotswap_HS203_hs203t002_hs203t002_resumeThread(JNIEnv * jni,
    ...
    jboolean retvalue;
    retvalue = JNI_FALSE;
    err =(*jvmti)->SetEventNotificationMode(jvmti,JVMTI_DISABLE,JVMTI_EVENT_SINGLE_STEP,thread);
    if (err == JVMTI_ERROR_NONE) {
        nsk_printf(" Agent:: cleared Single Step event");
    } else {
        nsk_printf(" Agent :: Failed to clear Single Step Event");
    }
    err =(*jvmti)->SetEventNotificationMode(jvmti,JVMTI_DISABLE,JVMTI_EVENT_BREAKPOINT,thread);
    if (err == JVMTI_ERROR_NONE) {
