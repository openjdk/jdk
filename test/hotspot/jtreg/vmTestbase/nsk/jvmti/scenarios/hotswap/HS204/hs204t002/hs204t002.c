...
callbackClassPrepare(jvmtiEnv *jvmti,
    ...
    if( (strcmp(className, CLASS_NAME) == 0 ) ) {
        jclass cls;
        cls = (*jni)->FindClass(jni,SEARCH_NAME);
        if (cls == NULL) {
            printf("Agent::CLS is null");
        } else {
            jmethodID method;
            method = (*jni)->GetMethodID(jni,cls, METHOD_NAME,METHOD_SIGNATURE);
            if (method == NULL) {
                ...
                jlocation end;
                jvmtiError err ;
                err=(*jvmti)->GetMethodLocation(jvmti, method, &start, &end);
                if ( err != JVMTI_ERROR_NONE) {
                    ...
                    err= (*jvmti)->SetBreakpoint(jvmti,method, start+1);
