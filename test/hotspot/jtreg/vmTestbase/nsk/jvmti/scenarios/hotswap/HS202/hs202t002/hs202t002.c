...
void JNICALL callbackMethodExit(jvmtiEnv *jvmti_env,
    ...
    if ( was_popped_by_exception ) {
        ...
        (*jvmti_env)->GetMethodName(jvmti_env, method, &name, &signature, &generic);
        if (strcmp(name,METHOD_NAME) == 0) {
            jclass cls;
            char fileName[512];
            nsk_jvmti_getFileName(redefineNumber, FILE_NAME, fileName,
                                  sizeof(fileName)/sizeof(char));
            (*jvmti_env)->GetMethodDeclaringClass(jvmti_env,method, &cls);
...
Java_nsk_jvmti_scenarios_hotswap_HS202_hs202t002_hs202t002_popThreadFrame(JNIEnv * jni,
    ...
    if ( state & JVMTI_THREAD_STATE_SUSPENDED) {
        err = (*jvmti)->PopFrame(jvmti,thread);
        if (err == JVMTI_ERROR_NONE) {
...
Java_nsk_jvmti_scenarios_hotswap_HS202_hs202t002_hs202t002_isThreadSuspended(JNIEnv* jni,
    ...
    if (suspend_error == thread_suspend_status) {
        jclass ex_class = (*jni)->FindClass(jni, "java/lang/IllegalThreadStateException");
        (*jni)->ThrowNew(jni, ex_class, "Thread has failed to self suspend");
        return JNI_FALSE;
    }
