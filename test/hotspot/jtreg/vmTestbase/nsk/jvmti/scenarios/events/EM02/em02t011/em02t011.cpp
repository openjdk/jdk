...
#define CLASS_NAME "nsk/jvmti/scenarios/events/EM02/em02t011"
#define METHOD_NAME "javaMethod"
...
int checkEvents(int step) {
    ...
    for (i = 0; i < JVMTI_EVENT_COUNT; i++) {
        ...
        switch (step) {
            case 1:
                isExpected = ((curr == JVMTI_EVENT_VM_INIT)
                                || (curr == JVMTI_EVENT_BREAKPOINT));
                break;

            case 2:
                isExpected = (curr == JVMTI_EVENT_BREAKPOINT);
                break;
        ...
        if (isExpected) {
            if (curr != JVMTI_EVENT_BREAKPOINT) {
...
void JNICALL
cbNewBreakpoint(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method, jlocation location) {

    changeCount(JVMTI_EVENT_BREAKPOINT, &newEventCount[0]);
}
...
setCallBacks(int step) {
    ...
    switch (step) {
        ...
        case 2:
            ...
            eventCallbacks.Breakpoint                = cbNewBreakpoint;
            break;
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_em02t011(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_em02t011(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_em02t011(JavaVM *jvm, char *options, void *reserved) {
