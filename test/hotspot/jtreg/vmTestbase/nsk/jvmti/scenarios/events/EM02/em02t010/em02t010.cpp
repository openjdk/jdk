...
#define CLASS_NAME "nsk/jvmti/scenarios/events/EM02/em02t010"
#define FIELD_ACC_NAME "testedField_acc"
#define FIELD_MOD_NAME "testedField_mod"
...
int checkEvents(int step) {
    ...
    for (i = 0; i < JVMTI_EVENT_COUNT; i++) {
        ...
        switch (step) {
            case 1:
                isExpected = ((curr == JVMTI_EVENT_VM_INIT)
                                || (curr == JVMTI_EVENT_FIELD_MODIFICATION)
                                || (curr == JVMTI_EVENT_FIELD_ACCESS));
                break;

            case 2:
                isExpected = ((curr == JVMTI_EVENT_FIELD_MODIFICATION)
                                || (curr == JVMTI_EVENT_FIELD_ACCESS));
                break;
        ...
        if (isExpected) {
            if (curr == JVMTI_EVENT_FIELD_MODIFICATION ||
                                curr == JVMTI_EVENT_FIELD_ACCESS) {
...
void JNICALL
cbNewFieldAccess(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method, jlocation location, jclass field_klass,
                jobject object, jfieldID field) {

    changeCount(JVMTI_EVENT_FIELD_ACCESS, &newEventCount[0]);
}

void JNICALL
cbNewFieldModification(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method, jlocation location, jclass field_klass,
                jobject object, jfieldID field, char signature_type,
                jvalue new_value) {

    changeCount(JVMTI_EVENT_FIELD_MODIFICATION, &newEventCount[0]);
}
...
static bool setCallBacks(int step) {
    ...
    switch (step) {
        ...
        case 2:
            ...
            eventCallbacks.FieldAccess               = cbNewFieldAccess;
            eventCallbacks.FieldModification         = cbNewFieldModification;
            break;
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_em02t010(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_em02t010(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_em02t010(JavaVM *jvm, char *options, void *reserved) {
