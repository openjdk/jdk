...
static int loadEvents = 0;
static int unloadEvents = 0;
...
int checkEvents(int step) {
    ...
    for (i = 0; i < JVMTI_EVENT_COUNT; i++) {
        ...
        switch (step) {
            case 1:
                isExpected = ((curr == JVMTI_EVENT_COMPILED_METHOD_LOAD)
        ...
        if (isExpected) {
            ...
            if (curr != JVMTI_EVENT_COMPILED_METHOD_UNLOAD) {
                if (currentCounts[i] > 0) {
...
void
handlerMC1(jvmtiEvent event, jvmtiEnv* jvmti, jmethodID method) {
...
cbCompiledMethodLoad(jvmtiEnv *jvmti_env, jmethodID method, jint code_size,
    ...
    loadEvents++;
    handlerMC1(JVMTI_EVENT_COMPILED_METHOD_LOAD, jvmti_env, method);
}
...
void JNICALL
cbNewCompiledMethodLoad(jvmtiEnv *jvmti_env, jmethodID method, jint code_size,
                const void* code_addr, jint map_length,
                const jvmtiAddrLocationMap* map, const void* compile_info) {
...
void JNICALL
cbNewCompiledMethodUnload(jvmtiEnv *jvmti_env, jmethodID method,
                const void* code_addr) {
...
agentProc(jvmtiEnv* jvmti, JNIEnv* agentJNI, void* arg) {
    ...
    /* checking amounts of COMPILED_METHOD_LOAD and COMPILED_METHOD_UNLOAD events */
    if (loadEvents < unloadEvents) {
        NSK_COMPLAIN2("Number of JVMTI_EVENT_COMPILED_METHOD_LOAD events %d is less than number of JVMTI_EVENT_COMPILED_METHOD_UNLOAD %d",
                            loadEvents,
                            unloadEvents);
    }
}
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_em02t003(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_em02t003(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_em02t003(JavaVM *jvm, char *options, void *reserved) {
