...
static int singleStepIdx = JVMTI_EVENT_SINGLE_STEP - JVMTI_MIN_EVENT_TYPE_VAL;
        ...
        switch (step) {
            case 1:
                isExpected = ((curr == JVMTI_EVENT_VM_INIT)
                                || (curr == JVMTI_EVENT_SINGLE_STEP));
                break;

            case 2:
                isExpected = (curr == JVMTI_EVENT_SINGLE_STEP);
                break;
...
cbSingleStep(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
    ...
    if (!strncmp(name,"javaMethod", 8)) {
        NSK_DISPLAY2("\tMethod: %s, location: %lld\n",
                            name, location);
...
void JNICALL
cbNewSingleStep(jvmtiEnv *jvmti_env, JNIEnv* jni_env, jthread thread,
                jmethodID method, jlocation location) {
    char *name;
    char *sign;
    char *genc;
...
static bool setCallBacks(int step) {
    ...
    switch (step) {
        ...
        case 2:
            ...
            eventCallbacks.SingleStep                = cbNewSingleStep;
            break;

        case 3:
            /* comparing number of JVMTI_EVENT_SINGLE_STEP events for steps*/
            if (eventCount[singleStepIdx] != newEventCount[singleStepIdx]) {
                NSK_COMPLAIN2("Number of JVMTI_EVENT_SINGLE_STEP events must be the same\n\t1-st callback=%d; 2-nd callback=%d\n",
                                    eventCount[singleStepIdx],
                                    newEventCount[singleStepIdx]);
            }
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_em02t007(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_em02t007(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_em02t007(JavaVM *jvm, char *options, void *reserved) {
