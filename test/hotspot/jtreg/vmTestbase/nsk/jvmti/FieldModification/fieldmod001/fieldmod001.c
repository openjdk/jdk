...
void JNICALL FieldModification(jvmtiEnv *jvmti_env, JNIEnv *env,
    ...
    err = (*jvmti_env)->GetClassSignature(jvmti_env, cls,
        &watch.m_cls, &generic);
    if (err != JVMTI_ERROR_NONE) {
    ...
    err = (*jvmti_env)->GetClassSignature(jvmti_env, field_klass,
        &watch.f_cls, &generic);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetClassSignature) unexpected error: %s (%d)\n",
               TranslateError(err), err);
    ...
    err = (*jvmti_env)->GetFieldName(jvmti_env, field_klass, field,
...
Java_nsk_jvmti_FieldModification_fieldmod001_getReady(JNIEnv *env, jclass klass,
    ...
    for (i = 0; i < sizeof(watches)/sizeof(watch_info); i++) {
        ...
        err = (*jvmti)->SetFieldModificationWatch(jvmti, cls, watches[i].fid);
        if (err == JVMTI_ERROR_NONE) {
            eventsExpected++;
        } else {
    ...
    watches[8].val.l = (*env)->NewGlobalRef(env, obj1);
    watches[9].val.l = (*env)->NewGlobalRef(env, arr1);
    ...
    watches[18].val.l = (*env)->NewGlobalRef(env, obj2);
    watches[19].val.l = (*env)->NewGlobalRef(env, arr2);
