...
FramePop(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thread,
    ...
    err = (*jvmti_env)->GetMethodDeclaringClass(jvmti_env, method, &popClass);
    if (err != JVMTI_ERROR_NONE) {
        printf("(GetMethodDeclaringClass) unexpected error: %s (%d)\n",
               TranslateError(err), err);
        result = STATUS_FAILED;
    }
    popClass = JNI_ENV_PTR(env)->NewGlobalRef(JNI_ENV_ARG((JNIEnv *)env,
        popClass));

    popMethod = method;
    popFlag = wasPopedByException;

    err = (*jvmti_env)->GetLocalInt(jvmti_env, thread, 0, 1, &popLoc);
...
ExceptionCatch(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thread,
    ...
    if (method == mid1 || method == mid2) {
        ...
        err = (*jvmti_env)->GetMethodDeclaringClass(jvmti_env,
            method, &currClass);
        if (err != JVMTI_ERROR_NONE) {
            printf("(GetMethodDeclaringClass) unexpected error: %s (%d)\n",
                   TranslateError(err), err);
        ...
        currClass = JNI_ENV_PTR(env)->NewGlobalRef(JNI_ENV_ARG((JNIEnv *)env,
            currClass));

        currMethod = method;

        err = (*jvmti_env)->GetLocalInt(jvmti_env, thread, 0, 1, &currLoc);
        if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY &&
                !caps.can_access_local_variables) {
            /* It is OK */
        } else if (err != JVMTI_ERROR_NONE) {
...
Java_nsk_jvmti_NotifyFramePop_nframepop001_setFramePopNotif(JNIEnv *env,
    ...
    currClass = JNI_ENV_PTR(env)->NewGlobalRef(JNI_ENV_ARG(env, currClass));
    ...
    err = (*jvmti)->GetLocalInt(jvmti, thr, 0, 1, &currLoc);
    if (err == JVMTI_ERROR_MUST_POSSESS_CAPABILITY &&
            !caps.can_access_local_variables) {
        /* It is OK */
    } else if (err != JVMTI_ERROR_NONE) {
    ...
    err = (*jvmti)->NotifyFramePop(jvmti, thr, 0);
    if (err != JVMTI_ERROR_NONE) {
