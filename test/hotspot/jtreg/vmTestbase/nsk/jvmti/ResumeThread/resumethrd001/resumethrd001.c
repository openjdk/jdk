    ...
    /* perform testing */
    {
        ...
        NSK_DISPLAY1("Get state vector for thread: %p\n", (void*)testedThread);
        {
            ...
            if ((state & JVMTI_THREAD_STATE_SUSPENDED) != 0) {
                NSK_COMPLAIN2("SuspendThread() does not turn off flag SUSPENDED:\n"
                              "#   state:  %s (%d)\n",
                                TranslateState(state), (int)state);
                nsk_jvmti_setFailStatus();
            }
...
#ifdef STATIC_BUILD
JNIEXPORT jint JNICALL Agent_OnLoad_resumethrd001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNICALL Agent_OnAttach_resumethrd001(JavaVM *jvm, char *options, void *reserved) {
    return Agent_Initialize(jvm, options, reserved);
}
JNIEXPORT jint JNI_OnLoad_resumethrd001(JavaVM *jvm, char *options, void *reserved) {
