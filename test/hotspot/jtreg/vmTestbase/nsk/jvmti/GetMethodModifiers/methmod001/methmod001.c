...
void checkMeth(jvmtiEnv *jvmti_env, JNIEnv *env, jclass cl,
        char *name, char *sig, int stat, int flags) {
    jvmtiError err;
    jmethodID mid;
    jint modifiers;
    ...
    err = (*jvmti_env)->GetMethodModifiers(jvmti_env, mid, &modifiers);
...
ClassLoad(jvmtiEnv *jvmti_env, JNIEnv *env, jthread thr, jclass cls) {
    ...
    err = (*jvmti_env)->GetClassSignature(jvmti_env, cls, &sig, &generic);
    if (err != JVMTI_ERROR_NONE) {
