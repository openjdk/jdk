 ...
 * <code>com.sun.jdi.ReferenceType.methods()</code>
 ...
 * reference types corresponding to debuggee field values, which are:<br>
 * primitive classes themselves, and arrays of primitive types and classes.
 */
public class methods005 {
    ...
    public static int run(String argv[], PrintStream out) {
        return new methods005().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "methods005t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                ...
                try {
                    log.display((i+1) + ") Trying to get a list of methods declared in the reference type \""
                        + testedType + "\"\tname " + testedType.name()
                        + " ...");
                    List meths = testedType.methods();
                    if (meths.size() == 0)
                        log.display("CHECK PASSED: returned list is empty as expected");
                    else {
                        log.complain("TEST FAILED: returned list of methods declared in the reference type \""
                            + testedType + "\"\tname: " + testedType.name()
                    ...
                    log.complain("TEST FAILED: caught unexpected "
                        + ue + "\n\tinstead of an empty list of methods"
                        + "\n\tdeclared in the reference type \""
                        + testedType + "\"\tname " + testedType.name());
                    tot_res = Consts.TEST_FAILED;
                }
