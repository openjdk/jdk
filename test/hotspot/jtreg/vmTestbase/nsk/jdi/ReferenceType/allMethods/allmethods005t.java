...
public class allmethods005t {
    ...
    public static int run(String args[]) {
        ...
        IOPipe pipe = argHandler.createDebugeeIOPipe(); boolean bool = true;

        pipe.println(allmethods005.COMMAND_READY);
        String cmd = pipe.readln();
        if (cmd.equals(allmethods005.COMMAND_QUIT)) {
            System.err.println("Debuggee: exiting due to the command: "
                + cmd);
            return Consts.TEST_PASSED;
        }

        int stopMeHere = 0; // // allmethods005.DEBUGGEE_STOPATLINE
