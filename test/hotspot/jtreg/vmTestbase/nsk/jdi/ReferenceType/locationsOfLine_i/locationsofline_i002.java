 ...
 * <code>com.sun.jdi.ReferenceType.locationsOfLine(int)</code>
 ...
 * field values/type declaration, which themselves are:<br>
 * primitive classes, arrays of primitive types and classes, and finally,
 * an interface type.
 */
public class locationsofline_i002 {
    ...
    static final int FLD_NUM = 25;
    // tested fields and used to verify the assertion
    ...
    public static int run(String argv[], PrintStream out) {
        return new locationsofline_i002().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "locationsofline_i002t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                ...
                if (fld.type() instanceof InterfaceType) {
                    testedType = (ReferenceType) fld.type();
                }
                else {
                ...
                try {
                    log.display("\n" + (i+1)
                        + ") Trying to get locations that map to the reference type \""
                        + testedType + "\"\tname: " + testedType.name() + " ...");
                    List locs = testedType.locationsOfLine(1);
                    if (locs.size() == 0)
                        log.display("CHECK PASSED: returned list is empty as expected");
                    else {
                        log.complain("TEST FAILED: returned list of locations of the reference type \""
                            + testedType + "\"\tname " + testedType.name()
                    ...
                    log.complain("TEST FAILED: caught unexpected "
                        + ue + "\n\tinstead of an empty list of locations of the reference type \""
                        + testedType + "\"\tname " + testedType.name());
                    tot_res = Consts.TEST_FAILED;
                }
