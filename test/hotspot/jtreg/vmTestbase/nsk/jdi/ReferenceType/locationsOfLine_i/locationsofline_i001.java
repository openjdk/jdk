 ...
 * <code>com.sun.jdi.ReferenceType.locationsOfLine(int)</code>  <BR>
 * complies with its specification.                             <BR>
 * <BR>
 * The test checks up that for each line in the List returned   <BR>
 * by the method ReferenceType.allLineLocations(),              <BR>
 * the method locationsOfLine(lineNumber) returns               <BR>
 * non-empty List object in which each object is a Location object.<BR>
 * Not throwing AbsentInformationException is checked up as well.<BR>
 */

public class locationsofline_i001 {
    ...
    public static int run (String argv[], PrintStream out) {
        ...
        return new locationsofline_i001().runThis(argv, out);
    }
    ...
    String mName = "nsk.jdi.ReferenceType.locationsOfLine_i";
    ...
    //------------------------------------------------------  testing section
        ...
        for (int i = 0; ; i++) {
            ...
            log2("......locations = refType.allLineLocations(); no AbsentInformationException is expected");
            try {
            ...
            log2("......the loop of checking locations in the returned list;");
            log2("      AbsentInformationException is not expected");
            ...
            label1:
                for (int ifor1 = 0; ifor1 < size; ifor1++) {
                    ...
                    try {
                        log2("......List lineLocations = refType.locationsOfLine(" + lineNumber + ");");
                        List lineLocations = refType.locationsOfLine(lineNumber);
                        ...
                        try {
                            for (; li2.hasNext(); ) {
                             ...
                             testExitCode = FAILED;
                             log3("ERROR: ClassCastException, that is, element is not Location");
                             break label1;
                        }
