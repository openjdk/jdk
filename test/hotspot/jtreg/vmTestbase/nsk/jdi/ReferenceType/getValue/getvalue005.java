 ...
 * properly throws <i>IllegalArgumentException</i> for non-static field.<p>
 *
 * The exception is provoked to be thrown by getting a Value of:<br>
 * <li>protected non-static field
 * <li>private non-static field
 * <li>package non-static field (default access)
 * <li>public non-static field.
 */
public class getvalue005 {
    static final String DEBUGGEE_CLASSES[] = {
        "nsk.jdi.ReferenceType.getValue.getvalue005t",
        "nsk.jdi.ReferenceType.getValue.getvalue005tDummyCls"
    };

    // debuggee source line where it should be stopped
    static final int DEBUGGEE_STOPATLINE = 56;

    static final int FLD_NUM = 32;
    // tested non-static fields used to provoke the exception
    ...
    public static int run(String argv[], PrintStream out) {
        return new getvalue005().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "getvalue005t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                Field fld = rType[1].fieldByName(DEBUGGEE_FLDS[i]);

                // Check the tested assersion
                try {
                    log.display("\n" + (i+1)
                        + ") Trying to get a Value of non-static field \""
                        + fld.name() + " " + fld.signature()
                        + "\"\n\tobtained from reference type \""
                        + rType[1] + "\" ...");

                    Value val = rType[1].getValue(fld);

                    log.complain("TEST FAILED: expected IllegalArgumentException was not thrown"
                        + "\n\twhen attempted to get a Value of non-static field \""
                        + fld.name() + " " + fld.signature()
                        + "\"\n\tobtained from reference type \""
                        + rType[1] + "\"\n\tReturned value: "
                        + val);
