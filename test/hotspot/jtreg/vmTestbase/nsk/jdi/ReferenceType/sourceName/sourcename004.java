 ...
 * <code>com.sun.jdi.ReferenceType.sourceName()</code>
 ...
 * by calling the JDI method. These fields have types of different
 * kinds: primitive classes themselves, and arrays of primitive types
 * and classes. The exception is expected to be thrown.
 */
public class sourcename004 {
    static final String DEBUGGEE_CLASS =
        "nsk.jdi.ReferenceType.sourceName.sourcename004t";

    // name of debuggee main thread
    static final String DEBUGGEE_THRNAME = "sourcename004tThr";
    ...
    public static int run(String argv[], PrintStream out) {
        return new sourcename004().runIt(argv, out);
    }
    ...
    private int runIt(String args[], PrintStream out) {
        ...
        vm = debuggee.VM();
        debuggee.redirectStderr(log, "sourcename004t.err> ");
        debuggee.resume();
        ...
        try {
            ...
            for (int i=0; i<FLD_NUM; i++) {
                ...
                try {
                    log.display("\n" + (i+1)
                        + ") Trying to get an identifying source name of reference type: \""
                        + testedType + "\"\tname: " + testedType.name() + " ...");

                    String srcName = testedType.sourceName();
