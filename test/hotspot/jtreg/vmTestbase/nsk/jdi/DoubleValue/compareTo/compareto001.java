...
public class compareto001 {
    ...
    private final static String prefix = "nsk.jdi.DoubleValue.compareTo";
    ...
    private static void execTest() {
        ...
        for (int i = 0; i < fieldList.size(); i++ ) {
            ...
            value = (DoubleValue )v1;

            // comparing with debuggee's fields
            display("Checking compateTo(Object object) method for DoubleValue: " + value);
    ...
    private static boolean PerformComparing(DoubleValue value, Object object ) {
        boolean result = true;
        ...
        if (object instanceof DoubleValue) {
            DoubleValue doubleObject = (DoubleValue)object;
            try {
                // assertion [ x.compareTo(y) == 0 <==> x.equals(y) ]
                if ( ((value.equals(object)) && (value.compareTo(doubleObject) != 0)) ||
                     (!(value.equals(object)) && (value.compareTo(doubleObject) == 0)) ) {
                ...
                if ( ((value.compareTo(doubleObject) == 0) && (doubleObject.compareTo(value) != 0)) ||
                     ((value.compareTo(doubleObject) != 0) && (doubleObject.compareTo(value) == 0)) ) {
                    complain("Failed assertion [ (x.compareTo(y) == 0) is identical to (y.compareTo(x) == 0) ] \n\t" +
                             "where 'x' is DoubleValue: " + value + " and 'y' is DoubleValue : " + doubleObject + " \n\t" +
                             "result of (x.compareTo(y)): " + value.compareTo(doubleObject) + "\n\t" +
                             "result of (y.compareTo(x)): " + doubleObject.compareTo(value) );
                    result = false;
                }
                if (value.compareTo(doubleObject) != 0) {
                    // assertion [ if (x.compareTo(y) == i) then (y.compareTo(x) == -i) ]
                    if (value.compareTo(doubleObject) != -(doubleObject.compareTo(value))) {
                ...
                if (value.compareTo(doubleObject) > 0) {
                     DoubleValue lessValue = FindLessDoubleValue(doubleObject);
                     if (lessValue != null) {
                ...
                complain("Caught unexpected " + e + " when comparing \n\t" +
                         "DoubleValue: " + value + " and DoubleValue argument: " + object);
                result = false;
            }
            ...
            try {
                ...
                complain("Does not throw expected NullPointerException when comparing \n\t" +
                         "DoubleValue: " + value + " and null argument");
                result = false;
            ...
            try {
                value.compareTo((DoubleValue)object);
                complain("Does not throw expected ClassCastException when comparing \n\t" +
                         "DoubleValue: " + value + " and argument: " + object);
                result = false;
     ...
     *  to find another DoubleValue field which is less then method's argument via
     *  <i>compareTo</i> method.
     */

    private static DoubleValue FindLessDoubleValue (DoubleValue currentValue) {
        DoubleValue result = null;

        for (int i = 0; i < fieldList.size(); i++ ) {

            Field field = (Field )fieldList.get(i);
            DoubleValue newValue = (DoubleValue)((ObjectReference )objectValue).getValue(field);
