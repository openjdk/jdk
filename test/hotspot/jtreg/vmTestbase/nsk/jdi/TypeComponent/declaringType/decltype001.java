...
public class decltype001 {
    ...
    private final static String className = "decltype001";
    ...
    public static int run(String argv[], PrintStream out) {
        ...
        // Get MainClass and MainInter from debugee
        ...
        // Check all fields from MainClass
        ...
        for (int i = 0; i < FIELD_NAME_CLASS.length; i++) {
            ...
            try {
                field = mainClass.fieldByName(FIELD_NAME_CLASS[i]);
            } catch (Exception e) {
                log.complain("debuger FAILURE 1> Can't get field by name "
                           + FIELD_NAME_CLASS[i] + " from " + mainClassName);
        ...
        // Check all fields from MainInter
        ...
        for (int i = 0; i < FIELD_NAME_INTER.length; i++) {
            ...
            try {
                field = mainInter.fieldByName(FIELD_NAME_INTER[i]);
            } catch (Exception e) {
                log.complain("debuger FAILURE 5> Can't get field by name "
                           + FIELD_NAME_INTER[i] + " from " + mainInterName);
            ...
            log.display("debuger> " + i + " field " + name + " from interface "
                      + mainInterName + " read.");
            try {
