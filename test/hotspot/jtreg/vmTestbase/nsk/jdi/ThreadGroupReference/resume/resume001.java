 ...
 * <code>com.sun.jdi.ThreadGroupReference.resume()</code>       <BR>
 ...
 * after suspending all the three threads:                              <BR>
 *  - group4.resume(); resumes only Thread4;                            <BR>
 *  - group3.resume(); resumes Thread3 and Thread4;                     <BR>
 *  - group2.resume(); resumes only Thread2;                            <BR>
 *  - group1.resume(); resumes Thread2, Thread3 and Thread4.            <BR>
 */
...
public class resume001 {
    ...
    private String testedClassName =
        "nsk.jdi.ThreadGroupReference.resume.Threadresume001a";

    //String mName = "nsk.jdi.ThreadGroupReference.resume";
        ...
        for (int i = 0; ; i++) {
            ...
            label0: {
                ...
                if (flag != 3) {
                    ...
                    if ((flag & 2) == 0)
                        log3("ERROR: 'Thread3' is not suspended");
                    if ((flag & 1) == 0)
                        log3("ERROR: 'Thread2' is not suspended");
                    if (flag >= 8)
                ...
                if (flag != 6) {
                    ...
                    if (flag == 0)
                        log3("ERROR: no threads suspunded");
                    if ((flag & 4) == 0)
                        log3("ERROR: 'Thread4' is not suspended");
                    if ((flag & 2) == 0)
                        log3("ERROR: 'Thread3' is not suspended");
                    if ((flag & 1) != 0)
                        log3("ERROR: 'Thread2' is among suspended threads");
                    if (flag >= 8)
                        log3("ERROR: some extra thread(s) is also suspended");
                }
                vm.resume();
