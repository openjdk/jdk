 ...
 *    Since debuggee's method runt1() calls to the method runt2(),      <BR>
 ...
 *    a method variable in the runt2(), which becomes invalid           <BR>
 *    when runt2() is resumed and runt1() is suspended.                 <BR>
 *    After getting the thread2 suspended second time,                  <BR>
 *    the debugger checks up that the method invocation                 <BR>
 *    StackFrame.getValue() on locvar1 does throw                       <BR>
...
public class getvalue002 {
    ...
    private String testedClassName =
        "nsk.jdi.StackFrame.getValue.Threadgetvalue002a";
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        log2("getvalue002a debuggee launched");
        debuggee.resume();
    ...
    //------------------------------------------------------  testing section
        ...
        pipe.println("newcheck");
            ...
            label1: {
                ...
                log2("      checking up that locvar1 is not valid for current StackFrame");

                try {
                    Value value = stackFrame.getValue(locvar1);

                    log3("ERROR:  no IllegalArgumentException after runt() returned");
                    expresult = returnCode1;
                } catch ( IllegalArgumentException e1 ) {
                    log2("     :  IllegalArgumentException after runt() returned");
                } catch ( Exception e ) {
                    log3("ERROR: Exception for: Value value = stackFrame.getValue(locvar1); :" + e);
                    expresult = returnCode1;
                    break label1;
                }
            }
