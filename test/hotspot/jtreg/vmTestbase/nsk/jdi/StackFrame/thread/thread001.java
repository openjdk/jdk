 ...
 * <code>com.sun.jdi.StackFrame.thread()</code>                 <BR>
 ...
 *    the name of the thread2 got with StackFrame.thread() is equal to  <BR>
 *    its name used for suspending the thread2.                         <BR>
 *    Since the thread2 is not resumed yet,                             <BR>
 *    InvalidStackFrameException must not be thrown.                    <BR>
...
public class thread001 {
    ...
    public static int run (String argv[], PrintStream out) {
        return new thread001().runThis(argv, out);
    }
    ...
    private String testedClassName =
        "nsk.jdi.StackFrame.thread.Threadthread001a";

    //String mName = "nsk.jdi.StackFrame.thread";
        ...
        pipe.println("newcheck");
           ...
           //BreakpointRequest breakpRequest3 = null;
            ...
            label1: {
                ...
                log2("      checking up that thread2's two names are equal");
                try {
                    if (!stackFrame.thread().name().equals(threadName) ) {
                        log3("ERROR:  !stackFrame.thread().name().equals(threadName)");
                        expresult = returnCode1;
                        break label1;
                    }
                } catch ( InvalidStackFrameException e ) {
                    log3("ERROR:  InvalidStackFrameException before the thread2 is resumed");
                    expresult = returnCode1;
                    break label1;
                }
                ...
                log2("       checking up throwing InvalidStackFrameException");
                try {
                    ThreadReference t = stackFrame.thread();
                    expresult = 1;
                    log3("ERROR:  no InvalidStackFrameExceprtion after the thread2 is resumed");
                } catch ( InvalidStackFrameException e ) {
                }
            }
            eventSet.resume(); // for case if thread2 was not resumed because of error in a check
