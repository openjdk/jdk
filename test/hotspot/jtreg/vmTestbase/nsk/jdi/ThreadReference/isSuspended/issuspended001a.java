...
/**
 * This class is used as debuggee application for the issuspended001 JDI test.
 */

public class issuspended001a {
    ...
    //------------------------------------------------------  section tested
                ...
                case 0:
                         ...
                         label:
                         synchronized (Threadissuspended001a.lockingObject) {

                             synchronized (Threadissuspended001a.waitnotifyObj) {
                                 log1("entered into  synchronized (waitnotifyObj)");
                                 log1("before: test_thread.start()");
                                 test_thread.start();

                                 try {
                                     log1("before:   waitnotifyObj.wait();");
                                     Threadissuspended001a.waitnotifyObj.wait();
                         ...
                         for (;;) {
                             log1(" out of synchronized block");
                             pipe.println("out_of_synchronized");
                             instruction = pipe.readln();
                             if (instruction.equals("continue")) {
                                 break;
                             }
                             if (!instruction.equals("enter_synchronized")) {
                                 logErr("ERROR: unexpected instruction: " + instruction);
                                 exitCode = FAILED;
                                 break;
                             }
                             synchronized (Threadissuspended001a.lockingObject) {
                             log1(" in synchronized block");
                                 pipe.println("in_synchronized");
                                 instruction = pipe.readln();
                                 if (!instruction.equals("leave_synchronized")) {
                                     logErr("ERROR: unexpected instruction: " + instruction);
                                     exitCode = FAILED;
                                     break;
                                 }
...
class Threadissuspended001a extends NamedTask {

    public Threadissuspended001a(String threadName) {
        super(threadName);
    }
    ...
    public static final int breakpointLineNumber3 = 11;

    public static final int breakpointLineNumber2 = 3;

    void log(String str) {
        issuspended001a.log2("thread2: " + str);
    }
