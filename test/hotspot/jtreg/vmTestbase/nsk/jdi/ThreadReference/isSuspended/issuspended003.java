 ...
 * - a debuggee with second thread, thread2 started up and      <BR>
 *   locked up at a synchronized block,                         <BR>
 * - suspending the thread2 with ThreadReference.suspend()      <BR>
 ...
 *   in which it creates the thread2;                                   <BR>
 * - informs a debugger of the thread2 creation,                        <BR>
 *   and is waiting for an instruction.                                 <BR>
 * Since thread2 uses the same lockingObject in its 'run' method        <BR>
 * it is locked up until the debuggee leaves the synchronized block.    <BR>
 * Upon the receiption a message from the debuggee, the debugger        <BR>
 * - removes all events from an EventQueue object;                      <BR>
 * - suspends thread2 with ThreadReference.suspend();                   <BR>
 * - checks up that the thread2 is suspended.                           <BR>
 * <BR>
 */

public class issuspended003 {
    ...
    public static int run (String argv[], PrintStream out) {
        return new issuspended003().runThis(argv, out);
    }
    ...
    private String testedClassName =
        "nsk.jdi.ThreadReference.isSuspended.Threadissuspended003a";
    ...
    private int runThis (String argv[], PrintStream out) {
        ...
        log2("issuspended003a debuggee launched");
        debuggee.resume();
