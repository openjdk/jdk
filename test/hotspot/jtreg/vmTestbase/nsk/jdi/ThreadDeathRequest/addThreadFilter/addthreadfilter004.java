 ...
 * <code>com.sun.jdi.ThreadDeathRequest.addThreadFilter()</code>
 ...
 * addThreadFilter() on the same ThreadDeathRequest object.
 * There are two ThreadDeathRequests to check as follows:
 ...
 *   - sets up ThreadDeathRequests 1&2 within the method
 *     in the class addthreadfilter004aTestClass which will be calling by both threads,
 *   - restricts the ThreadDeathRequest1 only to thread1,
 *   - restricts the ThreadDeathRequest2 only to thread2,
...
public class addthreadfilter004 extends TestDebuggerType1 {
    ...
    public static int run (String argv[], PrintStream out) {
        debuggeeName = "nsk.jdi.ThreadDeathRequest.addThreadFilter.addthreadfilter004a";
        return new addthreadfilter004().runThis(argv, out);
    }

    private String testedClassName =
      "nsk.jdi.ThreadDeathRequest.addThreadFilter.addthreadfilter004aTestClass";
    ...
    protected void testRun() {
        ...
        for (int i = 0; ; i++) {
            ...
            switch (i) {
                ...
                eventRequest1 = setting2ThreadDeathRequest (thread1,
                                       EventRequest.SUSPEND_ALL, property1);
                eventRequest2 = setting2ThreadDeathRequest (thread1,
                                       EventRequest.SUSPEND_ALL, property2);

                ((ThreadDeathRequest) eventRequest1).addThreadFilter(thread1);
                ((ThreadDeathRequest) eventRequest2).addThreadFilter(thread2);

                display("......waiting for ThreadDeathEvent in tested thread");
                ...
                if ( !(newEvent instanceof ThreadDeathEvent)) {
                    ...
                    display("       got new ThreadDeathEvent with property 'number' == " + property);
                    ...
                    if (!newEventRequest.equals(eventRequest1) ) {
                        setFailedStatus("The ThreadDeathEvent occured not for eventRequest1");
                    }

                    ThreadReference thr = ((ThreadDeathEvent)newEvent).thread();
                    if (!thr.equals(thread1)) {
                        setFailedStatus("The ThreadDeathEvent occured in unexpected thread: " + thr);
                    }
                }
                vm.resume();
                break;
