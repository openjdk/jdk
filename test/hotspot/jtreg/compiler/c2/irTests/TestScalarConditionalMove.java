...
public class TestScalarConditionalMove {
    ...
    private int cmoveIGTforI_Unsigned(int a, int b, int c, int d) {
    ...
    private float cmoveIGTforF_Unsigned(int a, int b, float c, float d) {
        return (Integer.compareUnsigned(a, b) > 0) ? c : d;
    }
    ...
    @IR(failOn = {IRNode.LOAD_VECTOR_F,
                  IRNode.VECTOR_MASK_CMP_F,
                  IRNode.VECTOR_BLEND_F,
                  IRNode.STORE_VECTOR})
    ...
    @IR(failOn = {IRNode.LOAD_VECTOR_D,
                  IRNode.VECTOR_MASK_CMP_D,
                  IRNode.VECTOR_BLEND_D,
                  IRNode.STORE_VECTOR})
    ...
    private static void testCMoveIGTforI_Unsigned(int[] a, int[] b, int[] c, int[] d, int[] r, int[] r2) {
        for (int i = 0; i < a.length; i++) {
            int cc = c[i];
            int dd = d[i];
            r2[i] = cc + dd;
    ...
    @Test
    @IR(failOn = {IRNode.LOAD_VECTOR_I,     IRNode.VECTOR_SIZE + "min(max_int, max_float)",
                  IRNode.LOAD_VECTOR_F,     IRNode.VECTOR_SIZE + "min(max_int, max_float)",
                  IRNode.VECTOR_MASK_CMP_I, IRNode.VECTOR_SIZE + "min(max_int, max_float)",
                  IRNode.VECTOR_BLEND_F,    IRNode.VECTOR_SIZE + "min(max_int, max_float)",
                  IRNode.STORE_VECTOR})
    ...
    private static void testCMoveIGTforF_Unsigned(int[] a, int[] b, float[] c, float[] d, float[] r, float[] r2) {
        for (int i = 0; i < a.length; i++) {
            float cc = c[i];
            float dd = d[i];
            r2[i] = cc + dd;
    ...
    @Test
    @IR(failOn = {IRNode.LOAD_VECTOR_L,     IRNode.VECTOR_SIZE + "min(max_long, max_double)",
                  IRNode.LOAD_VECTOR_D,     IRNode.VECTOR_SIZE + "min(max_long, max_double)",
                  IRNode.VECTOR_MASK_CMP_L, IRNode.VECTOR_SIZE + "min(max_long, max_double)",
                  IRNode.VECTOR_BLEND_D,    IRNode.VECTOR_SIZE + "min(max_long, max_double)",
                  IRNode.STORE_VECTOR})
    ...
    @Warmup(0)
    @Run(test = {"testCMoveIGTforI", "testCMoveIGTforI_Unsigned",
                 "testCMoveIGTforL",
                 "testCMoveIGTforF", "testCMoveIGTforF_Unsigned",
                 "testCMoveIGTforD",
                 "testCMoveLGTforI",
                 "testCMoveLGTforL",
                 "testCMoveLGTforF",
    ...
    private void testCMove_runner_two() {
        ...
        testCMoveIGTforI_Unsigned(aI, bI, cI, dI, rI, rI);
        for (int i = 0; i < SIZE; i++) {
            Asserts.assertEquals(rI[i], cmoveIGTforI_Unsigned(aI[i], bI[i], cI[i], dI[i]));
        }
        ...
        testCMoveIGTforF_Unsigned(aI, bI, cF, dF, rF, rF);
        for (int i = 0; i < SIZE; i++) {
            Asserts.assertEquals(rF[i], cmoveIGTforF_Unsigned(aI[i], bI[i], cF[i], dF[i]));
        }
