...
To perform a matching on a C2 IR node, the user can directly use the strings defined in class [IRNode](./IRNode.java) which most of the time represent real IR nodes as found in the C2 compiler as node classes (there are some exceptions). The IR node definitions in [IRNode](./IRNode.java) represent special placeholder strings which are replaced by the framework by regexes (defined in package [regex](./driver/irmatching/regexes)) depending on which compile phase (defined with attribute `phase` in [IR](./IR.java)) the IR rule should be applied on. If an [IRNode](./IRNode.java) cannot be applied for a compile phase (e.g. the IR node does not exist in this phase), a format violation will be reported.

The exact mapping from [IRNode](./IRNode.java) placeholder strings to regexes for compile phases and default phases (see next section) is defined in class [IRNodeMappings](./driver/irmatching/mapping/IRNodeMappings.java).

The user can also directly specify user-defined regexes in combination with a required compile phase (there is no default compile phase known by the framework for custom regexes). If such a user-defined regex represents a not yet supported C2 IR node, it is highly encouraged to directly add a new entry to [IRNode](./IRNode.java) for it together with a mapping in [IRNodeMappings](./driver/irmatching/mapping/IRNodeMappings.java) instead.

#### Default Compile Phase
When not specifying any compile phase with `phase` in [IR](./IR.java) (or explicitly setting `CompilePhase.DEFAULT`), the framework will perform IR matching on a default compile phase which for most IR nodes is `CompilePhase.PRINT_IDEAL` (output of flag `-XX:+PrintIdeal`, the state of the machine independent ideal graph after applying optimizations). The default phase for each [IR](./IR.java) node is defined in class [IRNodeMappings](./driver/irmatching/mapping/IRNodeMappings.java).
