...
public class VMOptionWarning {
    public static void main(String[] args) throws Exception {
        ProcessBuilder pb = ProcessTools.createLimitedJavaTestProcessBuilder("-XX:+AlwaysSafeConstructors", "-version");
        ...
        pb = ProcessTools.createLimitedJavaTestProcessBuilder("-XX:+PrintInlining", "-version");
        ...
        pb = ProcessTools.createLimitedJavaTestProcessBuilder("-XX:+VerifyStack", "-version");
        output = new OutputAnalyzer(pb.start());
        output.shouldNotHaveExitValue(0);
        output.shouldContain("Error: VM option 'VerifyStack' is develop and is available only in debug version of VM.");

        pb = ProcessTools.createLimitedJavaTestProcessBuilder("-XX:+CheckCompressedOops", "-version");
        output = new OutputAnalyzer(pb.start());
        output.shouldNotHaveExitValue(0);
