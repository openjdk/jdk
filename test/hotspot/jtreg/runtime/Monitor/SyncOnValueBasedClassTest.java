...
public class SyncOnValueBasedClassTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < fatalTests.length; i++) {
            ProcessBuilder pb = ProcessTools.createLimitedJavaTestProcessBuilder(fatalTests[i]);
        ...
        for (int i = 0; i < logTests.length; i++) {
            ProcessBuilder pb = ProcessTools.createLimitedJavaTestProcessBuilder(logTests[i]);
            OutputAnalyzer output = ProcessTools.executeProcess(pb);
    ...
    private static void virtualThreadTests() throws Exception {
        ...
        ProcessBuilder pb = ProcessTools.createLimitedJavaTestProcessBuilder(vtTest);
        OutputAnalyzer output = ProcessTools.executeProcess(pb);
        output.shouldContain("fatal error: Synchronizing on object");
        output.shouldNotContain("synchronization on value based class did not fail");
        output.shouldNotHaveExitValue(0);
        ...
        pb = ProcessTools.createLimitedJavaTestProcessBuilder(vtTest);
