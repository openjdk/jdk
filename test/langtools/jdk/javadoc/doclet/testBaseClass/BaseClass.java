...
/**
 * Regression test for: Javadoc does not process base class. If user specifies few classes on the
 * command line and few packages, with a situation where one of the specified classes(on the command
 * line) extends a class from one of the packages, then due to some anomaly in ordering in which all
 * the class and package objects get constructed, few classes were getting marked as "not included",
 * even thought they were included in this run and hence documentation for those packages was wrong.
 * The test case for which javadoc was failing is given in bug# 4197513.
 *
 * @bug 4197513
