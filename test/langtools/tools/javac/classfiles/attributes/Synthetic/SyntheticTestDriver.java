...
/**
 * The tests work as follows. Firstly, it looks through the test cases and extracts the appropriate
 * compiled classes. Each test case contains a set of expected classes, methods and fields. Those
 * class members must not have the Synthetic attribute, while other found classes, methods and
 * fields must have the Synthetic attribute if they are not in the set of expected class members.
 *
 * <p>Each test executes SyntheticTestDriver specifying the name of test cases and the number of
 * expected synthetic classes. Each test class is annotated by annotations which contains
 * non-synthetic class members.
 *
 * <p>See the appropriate class for more information about a test case.
 */
public class SyntheticTestDriver extends TestResult {
  ...
  public static void main(String[] args)
    ...
    if (args.length != 1 && args.length != 2) {
      throw new IllegalArgumentException(
          "Usage: SyntheticTestDriver <class-name> [<number-of-synthetic-classes>]");
    }
  ...
  public SyntheticTestDriver(String testCaseName) throws IOException, ClassNotFoundException {
    ...
    this.expectedClasses =
        Stream.of(clazz.getAnnotationsByType(ExpectedClass.class))
            .collect(Collectors.toMap(ExpectedClass::className, Function.identity()));
    this.classes = new HashMap<>();
    Path classDir = getClassDir().toPath();
    Pattern filePattern =
        Pattern.compile(
            Pattern.quote(testCaseName.replace('.', File.separatorChar)) + ".*\\.class");
    ...
    boolean success =
        classes.entrySet().stream().allMatch(e -> e.getKey().startsWith(testCaseName));
    if (!success) {
    ...
    try {
      ...
      for (Map.Entry<String, ClassModel> entry : classes.entrySet()) {
        ...
        Set<String> expectedMethods =
            expectedClass != null ? toSet(expectedClass.expectedMethods()) : new HashSet<>();
        ...
        for (MethodModel method : classFile.methods()) {
          ...
          if (testAttribute(
              ...
              expectedMethods,
              methodName,
              "Testing method " + methodName + " in class " + className)) {
        ...
        checkContains(
            foundMethods,
            expectedMethods,
            "Checking that all methods of class "
                + className
        ...
        Set<String> expectedFields =
            expectedClass != null ? toSet(expectedClass.expectedFields()) : new HashSet<>();
        ...
        for (FieldModel field : classFile.fields()) {
          ...
          if (testAttribute(
              ...
              expectedFields,
              fieldName,
              "Testing field " + fieldName + " in class " + className)) {
        ...
        checkContains(
            foundFields,
            expectedFields,
            "Checking that all fields of class "
                + className
                + " without Synthetic attribute have been found");
        checkEquals(
            numberOfSyntheticFields,
      ...
      checkContains(
          foundClasses, expectedClasses.keySet(), "Checking that all classes have been found");
      checkEquals(
          numberOfSyntheticClasses,
          expectedNumberOfSyntheticClasses,
  ...
  private boolean testAttribute(
    ...
    if (expectedMembers.contains(memberName)) {
      checkNull(attr, "Member must not have synthetic attribute : " + memberName);
      checkFalse(
          flags.contains(AccessFlag.SYNTHETIC),
          "Member must not have synthetic flag : " + memberName + " in class : " + className);
      return false;
    } else {
      return checkNull(attr, "Synthetic attribute should not be generated")
          && checkTrue(
              flags.contains(AccessFlag.SYNTHETIC),
              "Member must have synthetic flag : " + memberName + " in class : " + className);
    }
  }
