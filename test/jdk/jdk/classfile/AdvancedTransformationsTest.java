...
class AdvancedTransformationsTest {
  ...
  void testRemapClass() throws Exception {
    var map =
        Map.of(
            ...
            ClassDesc.of("remapped.List"),
            ClassDesc.ofDescriptor(
                AbstractPseudoInstruction.ExceptionCatchImpl.class.descriptorString()),
            ClassDesc.of("remapped.ExceptionCatchImpl"),
            ClassDesc.ofDescriptor(RawBytecodeHelper.class.descriptorString()),
            ClassDesc.of("remapped.RemappedBytecode"),
            ClassDesc.ofDescriptor(StackMapGenerator.class.descriptorString()),
            ClassDesc.of("remapped.StackMapGenerator"));
    try (var in = StackMapGenerator.class.getResourceAsStream("StackMapGenerator.class")) {
      var cc = ClassFile.of();
      var clm = cc.parse(in.readAllBytes());
          ...
          ClassFile.of(
                  ClassFile.ClassHierarchyResolverOption.of(
                      ClassHierarchyResolver.of(
                              Set.of(ClassDesc.of("remapped.List")),
                              Map.of(
                                  ClassDesc.of("remapped.RemappedBytecode"),
                                  ConstantDescs.CD_Object,
                                  ClassDesc.ofDescriptor(
                                      RawBytecodeHelper.class.descriptorString()),
                                  ClassDesc.of("remapped.RemappedBytecode")))
          ...
          .forEach(
              f ->
                  f.findAttribute(Attributes.signature())
                      .ifPresent(sa -> verifySignature(f.fieldTypeSymbol(), sa.asTypeSignature())));
      remapped
          .methods()
          .forEach(
              m ->
  ...
  void testRemapModule() throws Exception {
    ...
    var ma =
        cc.parse(
                ClassRemapper.of(Map.of(foo, bar))
                    .remapClass(
                        cc,
                        cc.parse(
                            cc.buildModule(
                                ModuleAttribute.of(
                                    ModuleDesc.of("MyModule"),
                                    mab -> mab.uses(foo).provides(foo, foo))))))
  ...
  void testRemapDetails() throws Exception {
    ...
    var remapped =
        cc.parse(
            ClassRemapper.of(Map.of(foo, bar, fooAnno, barAnno, objectMethods, bar))
                .remapClass(
                    cc,
                    cc.parse(
                        Rec.class
                            .getResourceAsStream(Rec.class.getName() + ".class")
                            .readAllBytes())));
    ...
    assertContains(
        ...
        "PUTSTATIC, owner: AdvancedTransformationsTest$Bar, field name: fooField, field type:"
            + " LAdvancedTransformationsTest$Bar;",
        "INVOKESTATIC, owner: AdvancedTransformationsTest$Bar, method name: fooMethod, method type:"
            + " (LAdvancedTransformationsTest$Bar;)LAdvancedTransformationsTest$Bar",
        "method type: ()LAdvancedTransformationsTest$Bar;",
        "GETFIELD, owner: AdvancedTransformationsTest$Rec, field name: foo, field type:"
            + " LAdvancedTransformationsTest$Bar;",
  ...
  void testInstrumentClass() throws Exception {
    ...
    var instrumentor =
        cc.parse(
            AdvancedTransformationsTest.class
                .getResourceAsStream("AdvancedTransformationsTest$InstrumentorClass.class")
                .readAllBytes());
    var target =
        cc.parse(
            AdvancedTransformationsTest.class
                .getResourceAsStream("AdvancedTransformationsTest$TargetClass.class")
                .readAllBytes());
    var instrumentedBytes =
        instrument(
            target, instrumentor, mm -> mm.methodName().stringValue().equals("instrumentedMethod"));
    assertEmpty(cc.verify(instrumentedBytes));
    var targetClass =
        new ByteArrayClassLoader(
                AdvancedTransformationsTest.class.getClassLoader(),
                "AdvancedTransformationsTest$TargetClass",
                instrumentedBytes)
            .loadClass("AdvancedTransformationsTest$TargetClass");
    assertEquals(
        targetClass
            .getDeclaredMethod("instrumentedMethod", Boolean.class)
            .invoke(targetClass.getDeclaredConstructor().newInstance(), false),
        34);
  }
  ...
  // synchronized copy of instrumentation code from jdk.jfr
  // jdk.jfr.internal.instrument.JIClassInstrumentation for testing purposes
                                        ...
                                        cle ->
                                            !(cle instanceof FieldModel fm
                                                ...
                                                && !(cle instanceof MethodModel mm
                                                    && !"<init>"
                                                        .equals(mm.methodName().stringValue())
