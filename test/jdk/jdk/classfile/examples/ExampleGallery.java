...
public class ExampleGallery {
    ...
    public byte[] overrideSuperclass(ClassModel cm, ClassDesc superclass) {
        return Classfile.transform(cm, ClassTransform.endHandler(cb -> cb.withSuperclass(superclass)));
    }
    ...
    public byte[] addInterface(ClassModel cm, ClassDesc newIntf) {
        return Classfile.transform(cm, ClassTransform.ofStateful(()  -> new ClassTransform() {
            boolean seen = false;
    ...
    public byte[] removeSignature(ClassModel cm) {
        return Classfile.transform(cm, ClassTransform.dropping(e -> e instanceof SignatureAttribute));
    }
    ...
    public byte[] stripFields(ClassModel cm, Predicate<String> filter) {
        return Classfile.transform(cm, ClassTransform.dropping(e -> e instanceof FieldModel fm
                                                         && filter.test(fm.fieldName().stringValue())));
    }

    public byte[] addField(ClassModel cm) {
        return Classfile.transform(cm, ClassTransform.endHandler(cb -> cb.withField("cool", ClassDesc.ofDescriptor("(I)D"), Classfile.ACC_PUBLIC)));
    }

    public byte[] changeFieldSig(ClassModel cm) {
        return Classfile.transform(cm, ClassTransform.transformingFields((fb, fe) -> {
    ...
    public byte[] addException(ClassModel cm, ClassDesc ex) {
        return Classfile.transform(cm, ClassTransform.transformingMethods(
    ...
    public byte[] addInstrumentation(ClassModel cm) {
        ...
        return Classfile.transform(cm, ClassTransform.transformingMethodBodies(transform));
    }

    public byte[] addInstrumentationBeforeInvoke(ClassModel cm) {
        return Classfile.transform(cm, ClassTransform.transformingMethodBodies((codeB, codeE) -> {
            switch (codeE) {
