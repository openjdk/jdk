...
public class TestStackFilter {
    ...
    // Use more @StartFrame filter than there is capacity for
    private static void testExceedMax() throws Exception {
        try (Recording r = new Recording()) {
            r.start();
            // Maximum number of simultaneous event classes than can
            // use a filter is 1024. Additional
            ...
            if (events.size() != 1200) {
                throw new Exception("Expected 1200 'Max' events");
            }
    ...
    // This test registers a start frame filter, emits an event with the filter
    // and unregisters the start frame. While this is happening another
