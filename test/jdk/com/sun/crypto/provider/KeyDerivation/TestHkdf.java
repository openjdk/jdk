...
public class TestHkdf {
    ...
    public static final List<TestData> testList = new LinkedList<TestData>() {{
        ...
        add(new TestData("RFC 5689 Test Case 4", "HkdfSHA1",
        ...
        add(new TestData("RFC 5689 Test Case 5", "HkdfSHA1",
            "000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f" +
            "202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f" +
            "404142434445464748494a4b4c4d4e4f",
            "606162636465666768696a6b6c6d6e6f707172737475767778797a7b7c7d7e7f" +
    ...
    private static boolean runVector(TestData testData)
        ...
        KeyDerivation hkdfGen;
        boolean result = true;
        byte[] actualPRK, expandOKM, macroOKM;

        // Get instances of the Extract and Expand HKDFGenerators
        try {
            hkdfGen = KeyDerivation.getInstance(testData.algName);
        ...
        // Perform a simple HKDF-Extract
        SecretKey ikmKey = new SecretKeySpec(testData.IKM, "HKDF-IKM");
        hkdfGen.init(ikmKey, new HkdfExtractParameterSpec(testData.salt));
        DerivedKeyParameterSpec deriveSpec =
                new DerivedKeyParameterSpec("PRK", 0);
        actualPRK = hkdfGen.deriveKey(deriveSpec).getEncoded();

        // Perform a simple HKDF-Expand
        SecretKey prkKey = new SecretKeySpec(testData.expectedPRK, "HKDF-PRK");
        hkdfGen.init(prkKey, new HkdfExpandParameterSpec(testData.info));
        deriveSpec = new DerivedKeyParameterSpec("OKM", testData.outLen);
        expandOKM = hkdfGen.deriveKey(deriveSpec).getEncoded();

        // Perform the Extract-then-Expand macro operation
        hkdfGen.init(ikmKey, new HkdfParameterSpec(testData.salt,
                testData.info));
        macroOKM = hkdfGen.deriveKey(deriveSpec).getEncoded();

        if (Arrays.equals(actualPRK, testData.expectedPRK)) {
            System.out.println("* HKDF-Extract: Pass");
        } else {
            result = false;
            System.out.println("* HKDF-Extract: FAIL");
            System.out.println("Expected:\n" +
                    dumpHexBytes(testData.expectedPRK, 16, "\n", " "));
            System.out.println("Actual:\n" +
                    dumpHexBytes(actualPRK, 16, "\n", " "));
            System.out.println();
        }

        if (Arrays.equals(expandOKM, testData.expectedOKM)) {
            System.out.println("* HKDF-Expand: Pass");
        } else {
            result = false;
            System.out.println("* HKDF-Expand: FAIL");
            System.out.println("Expected:\n" +
                    dumpHexBytes(testData.expectedOKM, 16, "\n", " "));
            System.out.println("Actual:\n" +
                    dumpHexBytes(expandOKM, 16, "\n", " "));
            System.out.println();
        }

        if (Arrays.equals(macroOKM, testData.expectedOKM)) {
            System.out.println("* HKDF-Extract-then-Expand: Pass");
        } else {
            result = false;
            System.out.println("* HKDF-Extract-then-Expand: FAIL");
            System.out.println("Expected:\n" +
                    dumpHexBytes(testData.expectedOKM, 16, "\n", " "));
            System.out.println("Actual:\n" +
                    dumpHexBytes(macroOKM, 16, "\n", " "));
            System.out.println();
        }
