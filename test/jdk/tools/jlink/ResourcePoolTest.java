  ...
  private static class ResourceVisitor implements Function<ResourcePoolEntry, ResourcePoolEntry> {
    ...
    public ResourcePoolEntry apply(ResourcePoolEntry resource) {
      ...
      switch (index) {
        case 0:
          ++amountAfter;
          return ResourcePoolEntry.create(
              resource.path() + SUFFIX, resource.type(), resource.contentBytes());
        case 1:
          ++amountAfter;
  ...
  private void checkResourceAdding() {
    ...
    test(
        ...
        (resources, module, path) -> {
          try {
            resources.add(
                ResourcePoolManager.newCompressedResource(
                    ResourcePoolEntry.create(path, new byte[0]),
                    ByteBuffer.allocate(99),
                    "bitcruncher",
                    ((ResourcePoolManager) resources).getStringTable(),
