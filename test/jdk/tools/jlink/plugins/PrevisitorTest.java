...
public class PrevisitorTest {
  ...
  public void test() throws Exception {
    ...
    ImagePluginStack stack =
        ImagePluginConfiguration.parseConfiguration(
            new Jlink.PluginsConfiguration(plugins, null, null));
    ResourcePoolManager inResources =
        new ResourcePoolManager(ByteOrder.nativeOrder(), new CustomStringTable());
    ...
    Collection<String> input =
        inResources.entries().map(Object::toString).collect(Collectors.toList());
    Collection<String> output =
        outResources.entries().map(Object::toString).collect(Collectors.toList());
    if (!input.equals(output)) {
      throw new AssertionError(
          "Input and output resources differ: input: " + input + ", output: " + output);
    }
  }
  ...
  private static class CustomPlugin implements Plugin, ResourcePrevisitor {
    ...
    public ResourcePool transform(ResourcePool inResources, ResourcePoolBuilder outResources) {
      ...
      CustomStringTable table =
          (CustomStringTable) ((ResourcePoolImpl) inResources).getStringTable();
      if (table.size() == 0) {
