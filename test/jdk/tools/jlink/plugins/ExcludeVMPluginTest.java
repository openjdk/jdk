...
public class ExcludeVMPluginTest {
  ...
  private static final String JVM_CFG_SERVER_ALIAS_OTHERS =
      TAG + "-server KNOWN\n-client ALIASED_TO -server\n-minimal ALIASED_TO -server\n";
  private static final String JVM_CFG_CLIENT_ALIAS_OTHERS =
      TAG + "-client KNOWN\n-server ALIASED_TO -client\n-minimal ALIASED_TO -client\n";
  private static final String JVM_CFG_MINIMAL_ALIAS_OTHERS =
  ...
  public void checkVM(
      String vm, String[] input, String jvmcfg, String[] expectedOutput, String expectedJvmCfg)
      throws Exception {
  ...
  private void doCheckVM(
    ...
    poolMgr.add(
        ResourcePoolEntry.create(
            "/java.base/lib/jvm.cfg", ResourcePoolEntry.Type.NATIVE_LIB, jvmcfgContent));
    ...
    poolMgr.add(
        ResourcePoolEntry.create(
            ...
            getJavaBaseModuleInfo()));
    for (String in : input) {
      poolMgr.add(ResourcePoolEntry.create(in, ResourcePoolEntry.Type.NATIVE_LIB, new byte[0]));
    }
    ...
    if (out.entryCount() != (expectedOutput.length + 2)) {
      ...
      throw new Exception(
          "Invalid output size " + out.entryCount() + " expected " + (expectedOutput.length + 2));
    }
