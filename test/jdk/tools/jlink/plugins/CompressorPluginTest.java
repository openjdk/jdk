...
public class CompressorPluginTest {
  ...
  public void test() throws Exception {
    ...
    checkCompress(
        gatherResources(javabase),
        new ZipPlugin(),
        null,
        new ResourceDecompressorFactory[] {new ZipDecompressorFactory()});
    ...
    checkCompress(
        ...
        null,
        new ResourceDecompressorFactory[] {new StringSharingDecompressorFactory()});
    ...
    options0.setProperty(compressPlugin.getName(), DefaultCompressPlugin.LEVEL_0);
    checkCompress(classes, compressPlugin, options0, new ResourceDecompressorFactory[] {});
    ...
    options1.setProperty(DefaultCompressPlugin.FILTER, "**Exception.class");
    compressPlugin = new DefaultCompressPlugin();
    options1.setProperty(compressPlugin.getName(), DefaultCompressPlugin.LEVEL_1);
    checkCompress(
        classes,
    ...
    options2.setProperty(DefaultCompressPlugin.FILTER, "**Exception.class");
    compressPlugin = new DefaultCompressPlugin();
    ...
    checkCompress(classes, compressPluginZip0, optionsZip0, new ResourceDecompressorFactory[] {});
        ...
        .forEach(
            resource -> {
              List<Pattern> includesPatterns =
                  includes.stream().map(Pattern::compile).collect(Collectors.toList());
              ...
              ResourcePoolManager inputResourcesMgr =
                  new ResourcePoolManager(
                      ByteOrder.nativeOrder(),
                      new StringTable() {
                        @Override
              ...
              ResourcePool compressedResources =
                  applyCompressor(prov, inputResourcesMgr, resource, includesPatterns);
              original[0] += resource.contentLength();
              compressed[0] += Optional.empty().get().contentLength();
              applyDecompressors(
                  factories,
                  inputResourcesMgr.resourcePool(),
                  compressedResources,
                  strings,
                  includesPatterns);
            });
  ...
  private ResourcePool applyCompressor(
      Plugin plugin,
      ResourcePoolManager inputResources,
      ResourcePoolEntry res,
      List<Pattern> includesPatterns) {
    ResourcePoolManager resMgr =
        new ResourcePoolManager(ByteOrder.nativeOrder(), inputResources.getStringTable());
    ResourcePool compressedResourcePool =
        plugin.transform(inputResources.resourcePool(), resMgr.resourcePoolBuilder());
    ...
    if (isIncluded(includesPatterns, path)) {
      ...
      if (header.getDecompressorNameOffset() == 0) {
        throw new AssertionError("Invalid plugin offset " + header.getDecompressorNameOffset());
      }
      if (header.getResourceSize() <= 0) {
        throw new AssertionError("Invalid compressed size " + header.getResourceSize());
      }
    } else if (header != null) {
  ...
  private void applyDecompressors(
      ResourceDecompressorFactory[] decompressors,
            ...
            compressed -> {
              ...
              if (decompressed.length != orig.contentLength()) {
                throw new AssertionError(
                    "Invalid uncompressed size " + header.getUncompressedSize());
              }
