...
public class StripNativeDebugSymbolsPluginTest {
  ...
  private static final Path JAVA_SRC_DIR =
      Paths.get(System.getProperty("test.src")).resolve("src").resolve(MODULE_NAME_WITH_NATIVE);
  ...
  private void configureMultipleTimesSamePlugin() throws Exception {
    Map<String, String> keepDebug =
        Map.of(StripNativeDebugSymbolsPlugin.NAME, "keep-debuginfo-files");
    Map<String, String> excludeDebug =
        Map.of(StripNativeDebugSymbolsPlugin.NAME, "exclude-debuginfo-files");
    StripNativeDebugSymbolsPlugin plugin = createAndConfigPlugin(keepDebug);
    try {
      plugin.doConfigure(false, excludeDebug);
      throw new AssertionError(
          "should have thrown IAE for broken config: " + keepDebug + " and " + excludeDebug);
    } catch (IllegalArgumentException e) {
      // pass
      System.out.println("DEBUG: test threw IAE " + e.getMessage() + " as expected.");
    }
  }

  private void configureUnknownOptions() throws Exception {
    Map<String, String> config = Map.of(StripNativeDebugSymbolsPlugin.NAME, "foobar");
  ...
  private void configureObjcopyWithKeep() throws Exception {
    ...
    Map<String, String> config =
        Map.of(StripNativeDebugSymbolsPlugin.NAME, "keep-debuginfo-files", "objcopy", objcopyPath);
  ...
  private void configureObjcopyWithOmit() throws Exception {
    ...
    Map<String, String> config =
        Map.of(
            StripNativeDebugSymbolsPlugin.NAME, "exclude-debuginfo-files", "objcopy", objcopyPath);
  ...
  private void doConfigureConflictingOptions(Map<String, String> config) throws Exception {
    try {
      createAndConfigPlugin(config);
      throw new AssertionError(
          "keep-debuginfo-files and exclude-debuginfo-files " + " should have conflicted!");
    } catch (IllegalArgumentException e) {
      // pass
  ...
  private void doOmitDebugInfoFakeObjCopyTest(Map<String, String> config, String expectedObjCopy)
      throws Exception {
    String binFile = "mybin";
    String path = "/fib/bin/" + binFile;
    ResourcePoolEntry debugEntry = createMockEntry(path, ResourcePoolEntry.Type.NATIVE_CMD);
    ResourcePoolManager inResources = new ResourcePoolManager();
    inResources.add(debugEntry);
    // expect entry to be present
    throw new AssertionError("Test failed. Binary " + path + " not present after stripping!");
  }

  public void testTransformFakeObjCopyKeepDebugInfoFiles() throws Exception {
    Map<String, String> defaultConfig =
        Map.of(StripNativeDebugSymbolsPlugin.NAME, "keep-debuginfo-files=" + DEBUG_EXTENSION);
    doKeepDebugInfoFakeObjCopyTest(defaultConfig, DEBUG_EXTENSION, DEFAULT_OBJCOPY_CMD);
    System.out.println("testTransformFakeObjCopyKeepDebugInfoFiles() PASSED!");
  }

  private void doKeepDebugInfoFakeObjCopyTest(
      Map<String, String> config, String debugExt, String expectedObjCopy) throws Exception {
    String sharedLib = "myLib.so";
    String path = "/fib/lib/" + sharedLib;
    ResourcePoolEntry debugEntry = createMockEntry(path, ResourcePoolEntry.Type.NATIVE_LIB);
    ResourcePoolManager inResources = new ResourcePoolManager();
    inResources.add(debugEntry);
    throw new AssertionError(
        "Test failed. Binary files "
            + path
            + "{,."
            + debugExt
            + "} not present after "
  ...
  public void testStripNativeLibraryDefaults() throws Exception {
    ...
    JLink.run(
            ...
            "--verbose",
            "--module-path",
            modulePathWith(libFibJmod),
            "--add-modules",
            MODULE_NAME_WITH_NATIVE,
    ...
    if (postStripSize >= ORIG_LIB_FIB_SIZE) {
      throw new AssertionError(
          "Expected native library stripping to "
  ...
  public void testOptionsInvalidObjcopy() throws Exception {
    ...
    System.out.println("Debug: command: " + jlinkCmd.stream().collect(Collectors.joining(" ")));
  ...
  private ResourcePoolEntry createMockEntry(String path, ResourcePoolEntry.Type type) {
    byte[] mockContent = new byte[] {0, 1, 2, 3};
    ResourcePoolEntry entry = ResourcePoolEntry.create(path, type, mockContent);
    return entry;
  }

  private StripNativeDebugSymbolsPlugin createAndConfigPlugin(
      Map<String, String> config, String expectedObjcopy) throws IOException {
    TestObjCopyCmdBuilder cmdBuilder = new TestObjCopyCmdBuilder(expectedObjcopy);
    return createAndConfigPlugin(config, cmdBuilder);
  }

  private StripNativeDebugSymbolsPlugin createAndConfigPlugin(Map<String, String> config)
      throws IOException {
  ...
  private StripNativeDebugSymbolsPlugin createAndConfigPlugin(
      Map<String, String> config, TestObjCopyCmdBuilder builder) throws IOException {
    StripNativeDebugSymbolsPlugin plugin = new StripNativeDebugSymbolsPlugin(builder);
  ...
  private void verifyInvalidObjcopyError(InputStream errInput, String match) {
    ...
    if (!foundMatch) {
      throw new AssertionError("Expected to find " + match + " in error stream.");
    } else {
  ...
  private void verifyDebugInfoSymbolFilePresent(Path image)
    ...
    if (!Files.exists(debugSymsFile)) {
      throw new AssertionError(
          "Expected stripped debug info file " + debugSymsFile.toString() + " to exist.");
    }
    long debugSymsSize = debugSymsFile.toFile().length();
    if (debugSymsSize <= 0) {
      throw new AssertionError("sanity check for fib.FibJNI failed " + "post-stripping!");
    } else {
      System.out.println(
          "DEBUG: Debug symbols stripped from libFib.so "
              + "present ("
              + debugSymsFile.toString()
              + ") as expected.");
    }
  }

  private void verifyFibModule(Path image) throws IOException, InterruptedException {
    ...
    System.out.println("DEBUG: [command] " + args.stream().collect(Collectors.joining(" ")));
  ...
  private static boolean isObjcopyPresent() throws Exception {
    ...
    try {
      ...
      int status = p.waitFor();
      if (status != 0) {
        System.out.println("Debug: objcopy binary doesn't seem to be " + "present or functional.");
        return false;
      }
    } catch (IOException e) {
      System.out.println("Debug: objcopy binary doesn't seem to be present " + "or functional.");
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (isObjcopyPresent()) {
      ...
      System.out.println("DEBUG: objcopy binary not available. " + "Running reduced set of tests.");
    }
  ...
  static class JLink {
    ...
    private int execute(String... options) {
      System.out.println("jlink " + Stream.of(options).collect(Collectors.joining(" ")));

      StringWriter writer = new StringWriter();
      PrintWriter pw = new PrintWriter(writer);
      int rc = JLINK_TOOL.run(pw, pw, options);
      System.out.println(writer.toString());
      Stream.of(writer.toString().split("\\v")).map(String::trim).forEach(output::add);
      return rc;
    }
  ...
  private static class JmodFileBuilder {
    ...
    private static final ToolProvider JMOD_TOOL =
    ...
    private void compileModule() throws IOException {
      ...
      if (!CompilerUtils.compile(msrc, MODS_DIR, "--module-source-path", SRC_DIR.toString())) {}
    }
  ...
  private static class TestObjCopyCmdBuilder implements ObjCopyCmdBuilder {
    ...
    public List<String> build(String objCopy, String... options) {
      if (!expectedObjCopy.equals(objCopy)) {
        throw new AssertionError(
            "Expected objcopy to be '" + expectedObjCopy + "' but was '" + objCopy);
      }
