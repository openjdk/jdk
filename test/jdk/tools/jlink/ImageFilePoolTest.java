...
public class ImageFilePoolTest {
  ...
  private void checkVisitor() throws Exception {
    ...
    for (int i = 0; i < 1000; ++i) {
      ...
      input.add(
          newInMemoryImageFile(
              "/" + module + "/java/class" + i, ResourcePoolEntry.Type.CONFIG, "class" + i));
    }
    ...
    if (visitor.getAmountBefore() != input.entryCount()) {
      throw new AssertionError(
          "Number of visited resources. Expected: "
              + visitor.getAmountBefore()
              + ", got: "
              + input.entryCount());
    }
    if (visitor.getAmountAfter() != output.entryCount()) {
      throw new AssertionError(
          "Number of added resources. Expected: "
  ...
  private static class ResourceVisitor implements Function<ResourcePoolEntry, ResourcePoolEntry> {
    ...
    public ResourcePoolEntry apply(ResourcePoolEntry file) {
      ...
      switch (index) {
        case 0:
          ++amountAfter;
          return newInMemoryImageFile(file.path() + SUFFIX, file.type(), file.path());
        case 1:
          ++amountAfter;
          return newInMemoryImageFile(file.path(), file.type(), file.path());
      }
      return null;
    }
  ...
  private void checkNegative() throws Exception {
    ...
    if (input.contains(
        newInMemoryImageFile("/unknown/foo", ResourcePoolEntry.Type.CONFIG, "unknown"))) {
  ...
  private static ResourcePoolEntry newInMemoryImageFile(
      String path, ResourcePoolEntry.Type type, String content) {
