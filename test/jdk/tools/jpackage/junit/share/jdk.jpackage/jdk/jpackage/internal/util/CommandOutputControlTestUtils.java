...
public class CommandOutputControlTestUtils {
    ...
    // Based on https://www.geeksforgeeks.org/dsa/find-if-a-string-is-interleaved-of-two-other-strings-dp-33/
    static <T> boolean isInterleave(List<T> combined, List<T> a, List<T> b) {
        ...
        final var curr = new boolean[m + 1];

        // Base case
        prev[0] = true;

        // Fill first row (a empty)
        for (int j = 1; j <= m; j++) {
        ...
        for (int i = 1; i <= n; i++) {
            // first column
            curr[0] = prev[0] && Objects.equals(a.get(i - 1), combined.get(i - 1));

            for (int j = 1; j <= m; j++) {
                int k = i + j;
                curr[j] = (prev[j] && Objects.equals(a.get(i - 1), combined.get(k - 1))) ||
                          (curr[j - 1] && Objects.equals(b.get(j - 1), combined.get(k - 1)));
            }
