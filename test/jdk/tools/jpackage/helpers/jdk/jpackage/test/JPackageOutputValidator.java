...
public final class JPackageOutputValidator {
    ...
    public void applyTo(JPackageCommand cmd, Executor.Result result) {
        ...
        toStringIteratorConsumer(cmd).ifPresent(validator -> {
            if (stdout) {
                validator.accept(result.stdout().iterator());
            } else {
                validator.accept(result.stderr().iterator());
            }
        });
    }
    ...
    public JPackageOutputValidator validator(JPackageOutputValidator other) {
    ...
    private Optional<Consumer<Iterator<String>>> toStringIteratorConsumer(JPackageCommand cmd) {
        ...
        }).flatMap(Optional::stream).reduce(Consumer::andThen).map(validator -> {
