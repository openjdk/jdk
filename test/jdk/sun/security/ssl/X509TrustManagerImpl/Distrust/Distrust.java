...
public class Distrust {
    ...
    public static void testCodeSigningChain(String certPath, String name, Date date) throws Exception {
        ...
        v.setValidationDate(date);
        v.validate(loadCertificateChain(certPath, name));
    }

    public static void testCertificateChain(String certPath, String[] rootsToTest, Date notBefore,
                                            boolean isValid, X509TrustManager[] tms) throws Exception {
    ...
    public static void testTM(X509TrustManager xtm, X509Certificate[] chain,
        ...
        // fake the notBefore date since none of the test certs are issued
        // after then.
        chain[1] = new NeverExpiresTLSServerCert(chain[1]);
    ...
    public static X509Certificate[] loadCertificateChain(String certPath, String name)
            throws Exception {
        if (cf == null) {
    ...
    private static class NeverExpiresTLSServerCert extends X509Certificate {
        private final X509Certificate cert;
        NeverExpiresTLSServerCert(X509Certificate cert) {
            this.cert = cert;
        }
    ...
    private static class DistrustedTLSServerCert extends NeverExpiresTLSServerCert {
