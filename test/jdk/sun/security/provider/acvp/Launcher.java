...
/// There are 3 ways to run this test:
///
/// 1. If you have a local repository, run with
/// ```
/// jtreg -Dtest.acvp.local.repo=<path-to-repo> Launcher.java
/// ```
/// 2. If you have download an ZIP archive available under the
/// [tags section](https://github.com/usnistgov/ACVP-Server/tags)
/// of the repository, run with
/// ```
/// jtreg -Djdk.test.lib.artifacts.ACVP-Server=<path-to-archive-file> Launcher.java
/// ```
/// 3. If the ZIP archive is hosted on artifactory server, run with
/// ```
/// jtreg Launcher.java
/// ```
/// The test looks for test data files in the archive listed with `TEST_FILES`.
///
/// These tests are currently compatible with ACVP version 1.1.0.39.
///
...
/// registered.
///
/// Tests for each algorithm must be compliant to its specification linked
/// from https://github.com/usnistgov/ACVP#supported-algorithms.
public class Launcher {
    ...
    private static final String ACVP_BUNDLE_VERSION = "1.1.0.39";
    ...
    public static void main(String[] args) throws Exception {
        ...
        String localRepo = System.getProperty("test.acvp.local.repo");
        if (localRepo != null) {
            for (String testFile : TEST_FILES) {
                try (var ins = Files.newInputStream(Path.of(localRepo, testFile))) {
                    run(ins);
                }
            }
        } else {
