/*
 * @test
 * @summary Some tests for the divide(.) and toBigDecimal(.) methods.
 */
...
public class DivideAndRoundTests {
    ...
    public static int mcDivideTests() {
        ...
        for (int i = 0; i < value.length; i++) {
            for (int j = 0; j < value.length; j++) {
                Rational v1 = new Rational(value[i]);
                Rational v2 = new Rational(value[j]);
                BigDecimal res1 = v1.divide(v2).toBigDecimal(MathContext.DECIMAL64);

                if (!res1.toString().equals(results[i][j])) {
                    failures++;
                    System.err.println("Unexpected result from (" + v1 + ") / (" + v2 + "); expected " + results[i][j]
                            + " got " + res1);
                }
            }
        }
        for (int mpc = 1; mpc < 23; mpc++) {
            for (int i = 0; i < value.length; i++) {
                for (int j = 0; j < value.length; j++) {
                    Rational v1 = new Rational(value[i]);
                    Rational v2 = new Rational(value[j]);
                    BigDecimal res1 = v1.divide(v2).toBigDecimal(new MathContext(mpc, RoundingMode.HALF_EVEN));
                    BigDecimal res2 = v1.divide(v2).round(new MathContext(128, RoundingMode.HALF_EVEN))
                            .toBigDecimal(new MathContext(mpc, RoundingMode.HALF_EVEN));
                    // BigDecimal res1 = v1.divide(v2).toBigDecimal(new MathContext(mpc,
                    // RoundingMode.HALF_EVEN));
                    // BigDecimal res2 = v1.divide(v2).round(new MathContext(128,
                    // RoundingMode.HALF_EVEN))
                    // .toBigDecimal(new MathContext(mpc, RoundingMode.HALF_EVEN));

                    if (!res1.equals(res2)) {
                        failures++;
                        System.err.println("Unexpected result from (" + v1 + ") / (" + v2 + "); expected " + res2
                                + " got " + res1);
                    }
                }
            }
    ...
    public static void main(String argv[]) {
        ...
        if (failures > 0)
            throw new RuntimeException("Incurred " + failures + " failures while testing DECIMAL64 divide.");
    }
