...
public class LongValueExactTests {
    ...
    private static int longValueExactSuccessful() {
        ...
        Map<String, Long> successCases = Map.ofEntries(entry("9223372036854775807", Long.MAX_VALUE), // 2^63 -1
                entry("9223372036854775807.0", Long.MAX_VALUE), entry("9223372036854775807.00", Long.MAX_VALUE),

                entry("-9223372036854775808", Long.MIN_VALUE), // -2^63
                entry("-9223372036854775808.0", Long.MIN_VALUE), entry("-9223372036854775808.00", Long.MIN_VALUE),

                entry("1e0", 1L), entry("1e18", 1_000_000_000_000_000_000L),

                entry("0e13", 0L), entry("0e64", 0L), entry("0e1024", 0L),
        ...
        for (var testCase : successCases.entrySet()) {
            ...
            try {
                ...
                if (expected != longValueExact || longValueExact != simpleLongValueExact(r)) {
                    failures++;
                    System.err.println("Unexpected longValueExact result " + longValueExact + " on " + r);
                }
            } catch (Exception e) {
                failures++;
    ...
    private static int longValueExactExceptional() {
        int failures = 0;
        List<String> exceptionalCases = List.of("9223372036854775808", // Long.MAX_VALUE + 1
                "9223372036854775808.0", "9223372036854775808.00", "-9223372036854775809", // Long.MIN_VALUE - 1
                "-9223372036854775808.1", "-9223372036854775808.01",

                "9999999999999999999", "10000000000000000000",

                "0.99", "0.999999999999999999999");
