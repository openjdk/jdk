...
public sealed class QuicServer implements QuicInstance, AutoCloseable permits QuicStandaloneServer {
    ...
    // A set that contains originalConnectionIds before they are
    // retired
    private final ConcurrentSkipListSet<ComparableById> originalIds =
    ...
    sealed interface ComparableById extends Comparable<ComparableById>
            permits OriginalCidRecord, IdBytesRecord {
        ByteBuffer asByteBuffer();

        @Override
        default int compareTo(ComparableById o) {
            return asByteBuffer().compareTo(o.asByteBuffer());
        }

        default boolean matches(ByteBuffer idBytes) {
            return asByteBuffer().compareTo(idBytes) == 0;
        }

        static ComparableById of(ByteBuffer idBytes) {
            return new IdBytesRecord(idBytes);
        }

        static ComparableById of(QuicConnectionId cid) {
            return new OriginalCidRecord(cid);
        }
    }

    private record OriginalCidRecord(QuicConnectionId cid) implements ComparableById {
        @Override
        public int compareTo(ComparableById o) {
            return cid.compareBytes(o.asByteBuffer());
        }

        @Override
        public ByteBuffer asByteBuffer() {
            return cid.asReadOnlyBuffer();
        }

        @Override
        public boolean matches(ByteBuffer idBytes) {
            return cid.matches(idBytes);
        }
    }

    private record IdBytesRecord(ByteBuffer idbytes) implements ComparableById {
        @Override
        public ByteBuffer asByteBuffer() {
            return idbytes;
        }
     ...
     * id generated by this server. The byte buffer is expected to have
     ...
     * a connection id generated by this server, {@code null}
     * otherwise.
     */
    @Override
    public QuicConnectionId unsafeConnectionIdFor(ByteBuffer buffer) {
        var cid = idFactory().unsafeConnectionIdFor(buffer);
        if (cid == null) {
            var found = originalIds.floor(ComparableById.of(buffer));
            if (found instanceof OriginalCidRecord cidrec && found.matches(buffer)) {
                cid = cidrec.cid();
            }
        }
        return cid;
    ...
    void connectionAcknowledged(QuicConnection connection,
        ...
        // originalIds.remove(ComparableById.of(originalConnectionId));
    }
