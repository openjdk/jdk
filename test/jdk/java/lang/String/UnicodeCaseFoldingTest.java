...
public class UnicodeCaseFoldingTest {
    ...
    void testAllCommnFullCodePointsListedInCaseFoldinigTxt() throws Throwable {
        ...
        var results = Files.lines(UCDFiles.CASEFOLDING)
                ...
                .map(line -> {
                    ...
                    // (2) Verify compareToCaseFold() result
                    assertEquals(0, source.compareToCaseFold(expected), "source.compareToCaseFold(expected)");
                    assertEquals(0, expected.compareToCaseFold(source), "expected.compareToCaseFold(source)");

                    // (3) Verify equalsCaseFold() result
                    assertEquals(true, source.equalsCaseFold(expected), "source.equalsCaseFold(expected)");
                    assertEquals(true, expected.equalsCaseFold(source), "expected.equalsCaseFold(source)");
                    return null;
                })
    ...
    void testAllSimpleCodePointsListedInCaseFoldinigTxt() throws Throwable {
        ...
        var results = Files.lines(UCDFiles.CASEFOLDING)
                ...
                .map(line -> {
                    ...
                    // (1) Verify compareToCaseFold() result
                    assertEquals(0, source.compareToCaseFold(expected), "source.compareToCaseFold(expected)");
                    assertEquals(0, expected.compareToCaseFold(source), "expected.compareToCaseFold(source)");

                    // (2) Verify equalsCaseFold() result
                    assertEquals(true, source.equalsCaseFold(expected), "source.equalsCaseFold(expected)");
                    assertEquals(true, expected.equalsCaseFold(source), "expected.equalsCaseFold(source)");
                    return null;
                })
    ...
    void testCompareToCaseFoldEquals(String s1, String s2) {
        ...
        assertEquals(true, s1.equalsCaseFold(s2));
        assertEquals(true, s2.equalsCaseFold(s1));
    ...
    void testCaseFoldRoundTrip(String s) {
        ...
        assertEquals(true, s.equalsCaseFold(folded));
        assertEquals(true, folded.equalsCaseFold(s));
    }
}
