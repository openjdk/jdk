...
// At various places we need to keep a vector of sigaction-by-signal-number,
//  e.g. for signal chaining and signal handler checks. This is a helper class
//  for those cases, containing the sigaction array as well as member information,
//  and may or may not be overwritable.
class SavedSignalHandlers {
  struct sigaction _v[NSIG];
  sigset_t _set;
  const bool _allow_overwriting;
...
public:
  ...
  SavedSignalHandlers(bool allow_overwriting)
    : _allow_overwriting(allow_overwriting) {
    sigemptyset(&_set);
  ...
  void set(int sig, const struct sigaction* act) {
    if (check_signal_number(sig)) {
      assert(_allow_overwriting || is_set(sig) == false, "Overwriting %d", sig);
      if (_allow_overwriting || is_set(sig) == false) {
        _v[sig] = *act;
        mark_as_set(sig);
      }
    }
...
static SavedSignalHandlers expected_handlers(false);
...
static SavedSignalHandlers chained_handlers(false);
...
//  signal chaining.
static SavedSignalHandlers replaced_handlers(true);
...
static void check_signal_handler(int sig) {
  ...
  // Retrieve expected signal setup. If this is NULL, we have no expectations.
  ...
  // - remove the signal from the expected setup set since we do not need to match it again
...
void* os::signal(int sig, void* handler) {
  char buf[1024];
  ...
  if (sigaction(sig, &sigAct, &oldAct)) {
  ...
  // For diagnostic purposes (see print_signal_handler) store information about
  // the preexisting handler if it was non-default and non-ignore
  address oldhand = get_signal_handler(&oldAct);
