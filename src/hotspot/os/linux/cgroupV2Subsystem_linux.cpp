...
int CgroupV2CpuController::cpu_quota() {
  jlong quota_val;
  bool is_ok = read_numerical_tuple_value("/cpu.max", true /* use_first */, &quota_val);
  if (!is_ok) {
...
int CgroupV2CpuController::cpu_period() {
  jlong period_val;
  bool is_ok = read_numerical_tuple_value("/cpu.max", false /* use_first */, &period_val);
  if (!is_ok) {
...
jlong CgroupV2MemoryController::memory_soft_limit_in_bytes(julong phys_mem) {
  ...
  CONTAINER_READ_NUMBER_CHECKED_MAX(this, "/memory.low", "Memory Soft Limit", mem_soft_limit);
...
jlong CgroupV2MemoryController::memory_and_swap_limit_in_bytes(julong phys_mem, julong host_swap) {
  jlong swap_limit;
  bool is_ok = read_number_handle_max("/memory.swap.max", &swap_limit);
  if (!is_ok) {
...
jlong CgroupV2MemoryController::read_memory_limit_in_bytes(julong phys_mem) {
  jlong limit = mem_limit_val(this);
