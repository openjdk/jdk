...
static bool transparent_huge_pages_sanity_check(bool warn,
                                                size_t page_size) {
  bool result = false;
...
static size_t hugetlbfs_page_size_flag(size_t page_size) {
...
static bool hugetlbfs_sanity_check(bool warn, size_t page_size) {
  ...
  const os::PageSizes pagesizes = HugePages::static_info().pagesizes();
  ...
  if (p != MAP_FAILED) {
    ...
    return true;
      ...
      for (size_t page_size_ = pagesizes.next_smaller(page_size);
          page_size_ != 0;
          page_size_ = pagesizes.next_smaller(page_size_)) {
...
void os::large_page_init() {
  ...
  if ((!UseTransparentHugePages && !HugePages::supports_static_hugepages()) ||
      ( UseTransparentHugePages && !HugePages::supports_thp())) {
  ...
  const size_t default_hugepage_size = HugePages::default_static_hugepage_size();
  ...
  all_large_pages.add(default_hugepage_size);
 ...
 if (FLAG_IS_DEFAULT(LargePageSizeInBytes) ||
      ...
      LargePageSizeInBytes == default_hugepage_size) {
    _large_page_size = default_hugepage_size;
    ...
    if (all_large_pages.contains(LargePageSizeInBytes)) {
      ...
      log_info(pagesize)("Overriding default large page size (" SIZE_FORMAT "%s) "
                         ...
                         byte_size_in_exact_unit(default_hugepage_size),
                         exact_unit_for_byte_size(default_hugepage_size),
                         byte_size_in_exact_unit(_large_page_size),
                         exact_unit_for_byte_size(_large_page_size));
    } else {
      _large_page_size = default_hugepage_size;
  ...
  UseLargePages = setup_large_page_type(_large_page_size);
