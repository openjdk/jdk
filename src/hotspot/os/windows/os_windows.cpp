...
bool os::pd_release_memory(char* addr, size_t bytes) {
  ...
  do {
    ...
    if (::VirtualFree(p, 0, MEM_RELEASE) == FALSE) {
      log_info(os)("VirtualFree [" PTR_FORMAT "-" PTR_FORMAT ") failed (%u)",
                   p2i(start), p2i(end), ::GetLastError());
      return false;
    }
    first_mapping = false;
    p = mi.base + mi.size;
...
bool os::win32::find_mapping(address addr, mapping_info_t* mi) {
  // Query at addr to find allocation base; then, starting at allocation base, query all regions, until
  // we either find the next allocation or a free area.
...
// Given a filled MEMORY_BASIC_INFORMATION area, the result of a successful VirtualQuery call which
// returned information about a non-free region:
// Print out all regions in that allocation until its end is reached. Indicate, in the output,
// regions that are outside the given range [start, end).
  ...
  while (p < end) {
    ...
    if (checkedVirtualQuery(p, &minfo)) {
      if (minfo.State != MEM_FREE) {
        ...
        // Note: for free regions, allmost all members in MEMORY_BASIC_INFORMATION are undefined but
        //  region start and size. Use those to jump to the end of the free range.
        address region_start = (address)minfo.BaseAddress;
        address region_end = region_start + minfo.RegionSize;
