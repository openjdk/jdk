...
void MacroAssembler::call_VM_base(Register oop_result,
  ...
  // if ((UseCompressedOops || USE_COMPRESSED_CLASS_POINTERS_ALWAYS_TRUE) && !TraceBytecodes) verify_heapbase("call_VM_base: heap base corrupted?");
#endif // ASSERT
...
#if 0
  assert (UseCompressedOops || USE_COMPRESSED_CLASS_POINTERS_ALWAYS_TRUE, "should be compressed");
...
MacroAssembler::KlassDecodeMode  MacroAssembler::klass_decode_mode(address base, int shift, const size_t range) {
  assert(USE_COMPRESSED_CLASS_POINTERS_ALWAYS_TRUE, "not using compressed class pointers");
