...
#ifndef CPU_AARCH64_AOT_CONFIG_AARCH64_HPP
#define CPU_AARCH64_AOT_CONFIG_AARCH64_HPP

// list of VM flags that can affect AOT code
#define AOT_CODE_VM_FLAGS(decl) \
  decl(AvoidUnalignedAccesses) \
  decl(UseSIMDForMemoryOps) \
  decl(UseSIMDForArrayEquals) \
  decl(UseBlockZeroing) \
  decl(UseSVE) \
  decl(UseLSE)
...
#define AOT_CODE_VM_PARAMS(decl) \
  decl(PrefetchCopyIntervalInBytes, intx) \
  decl(BlockZeroingLowLimit, intx) \
  decl(SoftwarePrefetchHintDistance, int)
...
#endif // CPU_AARCH64_AOT_CONFIG_AARCH64_HPP
