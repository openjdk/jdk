...
instruct vtest_alltrue(rFlagsReg cr, pRegGov src1, pRegGov src2, pReg ptmp)
%{
  predicate(UseSVE > 0 &&
  ...
  effect(TEMP ptmp);
  ins_cost(SVE_COST);
  format %{ "sve_eors $ptmp, $src1, $src2\n\t" %}
  ins_encode %{
...
instruct vtest_anytrue(rFlagsReg cr, pRegGov src1, pRegGov src2)
%{
  predicate(UseSVE > 0 &&
