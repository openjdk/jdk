...
instruct gather(vReg dst, indirect mem, vReg idx) %{
  predicate(UseSVE > 0);
  match(Set dst (LoadVectorGather mem idx));
  ins_cost(SVE_COST);
  format %{ "load_vector_gather $dst, $mem, $idx\t# vector load gather (sve)" %}
  ins_encode %{
    assert(Matcher::vector_length_in_bytes(this) == MaxVectorSize, "invalid vector length");
    BasicType bt = Matcher::vector_element_basic_type(this);
    if (bt == T_INT || bt == T_FLOAT) {
      ...
      assert(bt == T_LONG || bt == T_DOUBLE, "unsupported basic type");
...
instruct gather_masked(vReg dst, indirect mem, vReg idx, pRegGov pg) %{
  predicate(UseSVE > 0);
  match(Set dst (LoadVectorGatherMasked mem (Binary idx pg)));
  ins_cost(SVE_COST);
  format %{ "load_vector_gather $dst, $pg, $mem, $idx\t# vector load gather predicated (sve)" %}
  ins_encode %{
    BasicType bt = Matcher::vector_element_basic_type(this);
    if (bt == T_INT || bt == T_FLOAT) {
...
instruct scatter(indirect mem, vReg src, vReg idx) %{
  predicate(UseSVE > 0);
  match(Set mem (StoreVectorScatter mem (Binary src idx)));
  ins_cost(SVE_COST);
  format %{ "store_vector_scatter $mem, $idx, $src\t# vector store scatter (sve)" %}
  ins_encode %{
...
instruct scatter_masked(indirect mem, vReg src, vReg idx, pRegGov pg) %{
  predicate(UseSVE > 0);
  match(Set mem (StoreVectorScatterMasked mem (Binary src (Binary idx pg))));
  ins_cost(SVE_COST);
  format %{ "store_vector_scatter $mem, $pg, $idx, $src\t# vector store scatter predicate (sve)" %}
  ins_encode %{
    BasicType bt = Matcher::vector_element_basic_type(this, $src);
    if (bt == T_INT || bt == T_FLOAT) {
