...
void ZBarrierSetAssembler::try_resolve_jobject_in_native(MacroAssembler* masm, Register dst, Register jni_env,
  ...
  // Test for weak tag
  __ andi_(tmp, obj, JNIHandles::weak_tag_mask);
      ...
      if (vm_reg->is_Register()) {
        ...
        if (std_reg->encoding() >= R2->encoding() && std_reg->encoding() <= R12->encoding()) {
          ...
          if (action == ACTION_SAVE) {
            _masm->std(std_reg, -reg_save_index * BytesPerWord, R1_SP);
          } else if (action == ACTION_RESTORE) {
            _masm->ld(std_reg, -reg_save_index * BytesPerWord, R1_SP);
          } else {
            fatal("Sanity");
          }
        }
        ...
        if (fp_reg->encoding() >= F0->encoding() && fp_reg->encoding() <= F13->encoding()) {
          ...
          if (action == ACTION_SAVE) {
            _masm->stfd(fp_reg, -reg_save_index * BytesPerWord, R1_SP);
          } else if (action == ACTION_RESTORE) {
            _masm->lfd(fp_reg, -reg_save_index * BytesPerWord, R1_SP);
          } else {
            fatal("Sanity");
          }
        }
  ...
  ZSetupArguments(MacroAssembler* masm, ZLoadBarrierStubC2* stub) :
    ...
    if (_ref_addr.base() == noreg) {
      ...
      if (_ref != R4_ARG2) {
        ...
        __ add(R4_ARG2, _ref_addr.disp(), _ref_addr.base());
        __ mr_if_needed(R3_ARG1, _ref);
      } else if (_ref_addr.base() != R3_ARG1) {
        __ mr(R3_ARG1, _ref);
        __ add(R4_ARG2, _ref_addr.disp(), _ref_addr.base()); // Cloberring _ref
      } else {
