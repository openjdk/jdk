...
void LIR_Assembler::type_profile_helper(Register mdo,
  ...
  // under initial slot updates, we may end up updating the slot for a different receiver.
  // But this imprecision allows us to perform a single scan. When the scan encounters
  // the first nullptr, it can assume we are in unallocated tail, there were no matching
  // receivers, and thus we can just take this free slot.

  __ movptr(offset, base_receiver_offset);
  __ bind(L_loop);
    __ cmpptr(Address(mdo, offset), NULL_WORD);
    __ jccb(Assembler::notEqual, L_not_null);
      // TODO: This installation should really be atomic.
