...
address TemplateInterpreterGenerator::generate_StackOverflowError_handler() {
...
#ifdef ASSERT
  {
    ...
    __ shadd(t0, t0, fp, LogBytesPerWord);
...
address TemplateInterpreterGenerator::generate_return_entry_for(TosState state, int step, size_t index_size) {
  ...
  __ shadd(esp, t0, fp, LogBytesPerWord);
...
address TemplateInterpreterGenerator::generate_cont_resume_interpreter_adapter() {
  ...
  __ ld(t0, Address(fp, frame::interpreter_frame_last_sp_offset * wordSize));
  __ shadd(esp, t0, fp, Interpreter::logStackElementSize);
  // and null it as marker that esp is now tos until next java call
  __ sd(zr, Address(fp, frame::interpreter_frame_last_sp_offset * wordSize));

  // Restore machine SP
  __ ld(t0, Address(fp, frame::interpreter_frame_extended_sp_offset * wordSize));
  __ shadd(sp, t0, fp, LogBytesPerWord);
...
void TemplateInterpreterGenerator::generate_stack_overflow_check(void) {
  ...
  __ shadd(x10, x13, t0, Interpreter::logStackElementSize);  // 2 slots per parameter.
...
void TemplateInterpreterGenerator::histogram_bytecode_pair(Template* t) {
  ...
  __ shadd(counter_addr, index, x7, LogBytesPerInt);
