...
DumpTimeClassInfo::DumpTimeClassInfo(const DumpTimeClassInfo& src) {
...

  {
    ...
    if (n > 0) {
      _verifier_constraints = new (mtClass) GrowableArray<DTVerifierConstraint>(mtClass, n);
      _verifier_constraint_flags = new (mtClass) GrowableArray<char>(mtClass, n);
      for (int i = 0; i < n; i++) {
...

  {
    ...
    if (n > 0) {
      _loader_constraints = new (mtClass) GrowableArray<DTLoaderConstraint>(mtClass, n);
      for (int i = 0; i < n; i++) {
...
void DumpTimeClassInfo::add_verification_constraint(InstanceKlass* k, Symbol* name,
  ...
  if (_verifier_constraints == NULL) {
    _verifier_constraints = new (mtClass) GrowableArray<DTVerifierConstraint>(mtClass, 4);
  }
  if (_verifier_constraint_flags == NULL) {
    _verifier_constraint_flags = new (mtClass) GrowableArray<char>(mtClass, 4);
  }
...
void DumpTimeClassInfo::record_linking_constraint(Symbol* name, Handle loader1, Handle loader2) {
  ...
  if (_loader_constraints == NULL) {
    _loader_constraints = new (mtClass) GrowableArray<DTLoaderConstraint>(mtClass, 4);
  }
...
void DumpTimeClassInfo::add_enum_klass_static_field(int archived_heap_root_index) {
  if (_enum_klass_static_fields == NULL) {
    _enum_klass_static_fields = new (mtClass) GrowableArray<int>(mtClass, 20);
  }
