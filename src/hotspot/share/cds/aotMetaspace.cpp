...
void AOTMetaspace::init_heap_settings() {
  if (UseCompressedOops) {
    if (!AOTCodeCache::is_caching_enabled()) {
      // We don't need it -- always disable for better compiled code.
      FLAG_SET_ERGO(AOTCompatibleOopCompression, false);
    } else if (CDSConfig::is_dumping_final_static_archive()) {
      // Obey the command-line switch. Do not overwrite
...
void AOTMetaspace::initialize_runtime_shared_and_meta_spaces() {
  ...
  FileMapInfo* static_mapinfo = FileMapInfo::current_info(); // might by opened by set_compatible_oop_compression()
