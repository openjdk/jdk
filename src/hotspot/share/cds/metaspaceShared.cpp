...
void VM_PopulateDumpSharedSpace::doit() {
  ...
  const char* static_archive = CDSConfig::GetSharedArchivePath();
...
FileMapInfo* MetaspaceShared::open_dynamic_archive() {
  ...
  const char* dynamic_archive = CDSConfig::GetSharedDynamicArchivePath();
...
void MetaspaceShared::initialize_shared_spaces() {
  ...
  if (PrintSharedArchiveAndExit) {
    // Print archive names
    if (dynamic_mapinfo != nullptr) {
      tty->print_cr("\n\nBase archive name: %s", CDSConfig::GetSharedArchivePath());
