...
ArchiveBuilder::SourceObjList::SourceObjList() : _ptrmap(16 * K, mtClassShared) {
  _total_bytes = 0;
  _objs = new (mtClassShared) GrowableArray<SourceObjInfo*>(mtClassShared, 128 * K);
}
...
{
  _klasses = new (mtClassShared) GrowableArray<Klass*>(mtClassShared, 4 * K);
  _symbols = new (mtClassShared) GrowableArray<Symbol*>(mtClassShared, 256 * K);
  _special_refs = new (mtClassShared) GrowableArray<SpecialRefInfo>(mtClassShared, 24 * K);
