...
public:
  ...
  address inline_entry_point() const              { return code_begin() + _hdr->_inline_entry_offset; }             // inline type entry point (unpack all inline type args)
  address verified_inline_entry_point() const     { return code_begin() + _hdr->_verified_inline_entry_offset; }    // inline type entry point (unpack all inline type args) without class check
  address verified_inline_ro_entry_point() const  { return code_begin() + _hdr->_verified_inline_ro_entry_offset; } // inline type entry point (only unpack receiver) without class check
  ...
  bool  has_unsafe_access() const                 { return _hdr->_flags.has_unsafe_access(); }
  bool  has_monitors() const                      { return _hdr->_flags.has_monitors(); }
  bool  has_scoped_access() const                 { return _hdr->_flags.has_scoped_access(); }
  bool  has_wide_vectors() const                  { return _hdr->_flags.has_wide_vectors(); }
  bool  needs_stack_repair() const                { return _hdr->_flags.needs_stack_repair(); }
