...
JRT_BLOCK_ENTRY(void, JVMCIRuntime::new_instance_common(JavaThread* current /* TRAPS */, Klass* klass, bool null_on_fail))
  JRT_BLOCK;
...
JRT_BLOCK_ENTRY(void, JVMCIRuntime::new_array_common(JavaThread* current /* TRAPS */, Klass* array_klass, jint length, bool null_on_fail))
  JRT_BLOCK;
...
JRT_ENTRY(void, JVMCIRuntime::new_multi_array_common(JavaThread* current /* TRAPS */, Klass* klass, int rank, jint* dims, bool null_on_fail))
  assert(klass->is_klass(), "not a class");
...
JRT_ENTRY(void, JVMCIRuntime::dynamic_new_array_common(JavaThread* current /* TRAPS */, oopDesc* element_mirror, jint length, bool null_on_fail))
  RetryableAllocationMark ram(current, null_on_fail);
  oop obj = Reflection::reflect_new_array(element_mirror, length, CHECK);
  current->set_vm_result(obj);
JRT_END

JRT_ENTRY(void, JVMCIRuntime::dynamic_new_instance_common(JavaThread* current /* TRAPS */, oopDesc* type_mirror, bool null_on_fail))
...
JRT_BLOCK_ENTRY(int, JVMCIRuntime::throw_and_post_jvmti_exception(JavaThread* current /* TRAPS */, const char* exception, const char* message))
  JRT_BLOCK;
...
JRT_ENTRY(jlong, JVMCIRuntime::invoke_static_method_one_arg(JavaThread* current /* TRAPS */, Method* method, jlong argument))
  ResourceMark rm;
