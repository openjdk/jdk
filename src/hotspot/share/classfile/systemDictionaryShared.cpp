...
const char* SystemDictionaryShared::check_self_exclusion_helper(InstanceKlass* k, bool& log_warning) {
  ...
  DumpTimeClassInfo* info = SystemDictionaryShared::get_info_locked(k);
  if (info != nullptr && !info->is_loaded_from_file()) {
    return "Does not come from a file";
  }
...
void SystemDictionaryShared::init_dumptime_info(InstanceKlass* k, bool is_loaded_from_file) {
  MutexLocker ml(DumpTimeTable_lock, Mutex::_no_safepoint_check_flag);
