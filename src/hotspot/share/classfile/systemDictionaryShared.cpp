...
void SystemDictionaryShared::set_resolution_info(InstanceKlass* k, DumpTimeResolutionInfo* res_info) {
  MutexLocker ml(DumpTimeTable_lock, Mutex::_no_safepoint_check_flag);
  DumpTimeClassInfo* info = get_info_locked(k);
  info->set_resolution_info(res_info);
}

DumpTimeResolutionInfo* SystemDictionaryShared::get_resolution_info_locked(InstanceKlass* k) {
  assert_lock_strong(DumpTimeTable_lock);
  assert(!has_been_redefined(k) && !k->is_scratch_class(), "not supported because of CP transfer");
  DumpTimeClassInfo* info = get_info_locked(k);
  return info->resolution_info();
}
