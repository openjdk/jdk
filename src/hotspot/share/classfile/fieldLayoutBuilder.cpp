...
static LayoutKind field_layout_selection(FieldInfo field_info, Array<ValueFieldLayoutInfo>* value_field_layout_info_array,
  ...
  const ValueKlass* vk = value_field_layout_info->klass();
  ...
  while (ik != nullptr) {
    for (AllFieldStream fs(ik); !fs.done(); fs.next()) {
      ...
      if (fs.is_flat()) {
        ...
        const ValueKlass* vk = layout_info.klass();
...
void FieldLayoutBuilder::compute_value_class_layout() {
  ...
  if (!_is_abstract_value && vm_uses_flattening) { // Flat layouts are only for concrete value classes
    ...
    const int required_alignment = MAX2(default_alignment,
                                        least_restrictive_layout_of(LayoutKind::NULL_FREE_ATOMIC_FLAT,
    ...
    layouts().set_size_in_bytes_of(LayoutKind::BUFFERED,
                                   least_restrictive_layout_of(LayoutKind::BUFFERED,
                                                   LayoutKind::NULLABLE_ATOMIC_FLAT,
