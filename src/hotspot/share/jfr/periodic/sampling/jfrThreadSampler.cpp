...
class OSThreadSampler : public SuspendedThreadTask {
 ...
 public:
  ...
  void do_task(const SuspendedThreadTaskContext& context) {
    ...
    new (request.callback_storage()) JfrWallClockStackWalkerCallback(JfrTicks::now(), false /* is_native_sample */);
    StackWalker::install_stack_walk_request(request, jt, context.ucontext());
    _success = true;
  }
};
...
bool JfrSamplerThread::sample_native_thread(JavaThread* jt) {
  ...
  StackWalkRequest request;
  request.set_max_frames(_max_frames);
  new (request.callback_storage()) JfrWallClockStackWalkerCallback(JfrTicks::now(), true /* is_native_sample */);
  // Enqueue a biased request (null bcp/pc/sp). The target thread will
  // walk from its own last_frame() when it processes the request.
  StackWalker::enqueue_stack_walk_request(request, jt);
  return true;
}
