...
class JfrStackTraceRepository : public JfrCHeapObj {
 ...
 private:
  ...
  traceid record(JavaThread* current_thread, int skip, long stack_filter_id, JfrStackFrame* frames, u4 max_frames);

 public:
  static traceid record(Thread* current_thread, int skip = 0, long stack_filter_id = 0);
};
