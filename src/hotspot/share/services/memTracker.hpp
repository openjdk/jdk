...
class MemTracker : AllStatic {
 ...
 public:
  ...
  // Initialize NMT to a given tracking level. Only call once.
  // Allowed transitions here are:
  //  unknown -> off
  //  unknown -> summary
  //  unknown -> detail
  ...
  static NMT_TrackingLevel parse_level_string(const char* s);
  ...
  // Note: we expect no virtual memory operations to happen before NMT initialization.
