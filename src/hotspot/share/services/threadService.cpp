...
StackFrameInfo::StackFrameInfo(javaVFrame* jvf, bool with_lock_info) {
  ...
  if (with_lock_info) {
    ...
    if (length > 0) {
      _locked_monitors = new (mtServiceability) GrowableArray<OopHandle>(mtServiceability, length);
...
ThreadStackTrace::ThreadStackTrace(JavaThread* t, bool with_locked_monitors) {
  _thread = t;
  _frames = new (mtServiceability) GrowableArray<StackFrameInfo*>(mtServiceability, INITIAL_ARRAY_SIZE);
  _depth = 0;
  _with_locked_monitors = with_locked_monitors;
  if (_with_locked_monitors) {
    _jni_locked_monitors = new (mtServiceability) GrowableArray<OopHandle>(mtServiceability, INITIAL_ARRAY_SIZE);
  } else {
...
void ConcurrentLocksDump::dump_at_safepoint() {
  ...
  GrowableArray<oop>* aos_objects = new (mtServiceability) GrowableArray<oop>(mtServiceability, INITIAL_ARRAY_SIZE);
...
ThreadConcurrentLocks::ThreadConcurrentLocks(JavaThread* thread) {
  _thread = thread;
  _owned_locks = new (mtServiceability) GrowableArray<OopHandle>(mtServiceability, INITIAL_ARRAY_SIZE);
  _next = NULL;
}
...
DeadlockCycle::DeadlockCycle() {
  _threads = new (mtServiceability) GrowableArray<JavaThread*>(mtServiceability, INITIAL_ARRAY_SIZE);
  _next = NULL;
}
