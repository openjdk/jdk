...
inline void KlassInfoEntry::add_subclass(KlassInfoEntry* cie) {
  if (_subclasses == NULL) {
    _subclasses = new (mtServiceability) GrowableArray<KlassInfoEntry*>(mtServiceability, 4);
  }
...
KlassInfoHisto::KlassInfoHisto(KlassInfoTable* cit) :
  _cit(cit) {
  _elements = new (mtServiceability) GrowableArray<KlassInfoEntry*>(mtServiceability, _histo_initial_size);
}
