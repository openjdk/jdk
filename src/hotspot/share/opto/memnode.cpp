...
Node* LoadNode::find_previous_arraycopy(PhaseGVN* phase, Node* ld_alloc, Node*& mem, bool can_see_stored_value) const {
...
ArrayCopyNode* MemNode::find_array_copy_clone(PhaseGVN* phase, Node* ld_alloc, Node* mem) const {
...
// of aliasing.
Node* MemNode::can_see_stored_value(Node* st, PhaseGVN* phase) const {
  Node* ld_adr = in(MemNode::Address);
  intptr_t ld_off = 0;
...
//
bool StoreNode::value_never_loaded(PhaseGVN* phase) const {
  Node *adr = in(Address);
...
bool ClearArrayNode::step_through(Node** np, uint instance_id, PhaseGVN* phase) {
  Node* n = *np;
...
intptr_t InitializeNode::get_store_offset(Node* st, PhaseGVN* phase) {
...
#ifdef ASSERT
bool InitializeNode::stores_are_sane(PhaseGVN* phase) {
  if (is_complete())
