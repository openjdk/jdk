...
bool ArrayCopyNode::may_modify(const TypeOopPtr* t_oop, PhaseGVN* phase) {
  Node* dest = in(ArrayCopyNode::Dest);
  if (dest->is_top()) {
    return false;
  }
...
bool ArrayCopyNode::may_modify_helper(const TypeOopPtr* t_oop, Node* n, PhaseGVN* phase, CallNode* &call) {
  if (n != nullptr &&
      n->is_Call() &&
...
bool ArrayCopyNode::may_modify(const TypeOopPtr* t_oop, MemBarNode* mb, PhaseGVN* phase, ArrayCopyNode* &ac) {
  Node* c = mb->in(0);
...
bool ArrayCopyNode::modifies(intptr_t offset_lo, intptr_t offset_hi, PhaseGVN* phase, bool must_modify) const {
