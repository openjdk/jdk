...
JVMState* LibraryIntrinsic::generate(JVMState* jvms) {
  ...
  if (callee->check_intrinsic_candidate() &&
    ...
    const char *inline_msg = is_virtual() ? "(intrinsic, virtual)"
                                          : "(intrinsic)";
    CompileTask::print_inlining_ul(callee, jvms->depth() - 1, bci, Inlining::SUCCESS, inline_msg);
    if (C->print_intrinsics() || C->print_inlining()) {
      C->print_inlining(callee, jvms->depth() - 1, bci, Inlining::SUCCESS, inline_msg);
    }
  ...
  if (jvms->has_method()) {
    ...
    CompileTask::print_inlining_ul(callee, jvms->depth() - 1, bci, Inlining::FAILURE, msg);
    if (C->print_intrinsics() || C->print_inlining()) {
      C->print_inlining(callee, jvms->depth() - 1, bci, Inlining::FAILURE, msg);
    }
  } else {
    // Root compile
    ResourceMark rm;
...
Node* LibraryIntrinsic::generate_predicate(JVMState* jvms, int predicate) {
  ...
  if (jvms->has_method()) {
    ...
    CompileTask::print_inlining_ul(kit.callee(), jvms->depth() - 1, bci, Inlining::FAILURE, msg);
    if (C->print_intrinsics() || C->print_inlining()) {
      C->print_inlining(kit.callee(), jvms->depth() - 1, bci, Inlining::FAILURE, msg);
    }
  } else {
    // Root compile
    ResourceMark rm;
