...
void SuperWordVTransformBuilder::build() {
  ...
  assert(!_vector_vtransform.has_graph(), "start with empty vtransform");
...
void SuperWordVTransformBuilder::build_scalar_vtnodes_for_non_packed_nodes() {
  ...
  //     vtn = new (_vector_vtransform.arena()) VTransformMemopScalarNode(_vector_vtransform, mem, mem_p);
  //   } else if (n->is_Phi()) {
  //     vtn = new (_vector_vtransform.arena()) VTransformPhiScalarNode(_vector_vtransform, n->as_Phi());
  //   } else if (n->is_CountedLoop()) {
  //     vtn = new (_vector_vtransform.arena()) VTransformCountedLoopNode(_vector_vtransform, n->as_CountedLoop());
  //   } else if (n->is_CFG()) {
  //     vtn = new (_vector_vtransform.arena()) VTransformCFGNode(_vector_vtransform, n);
  //   } else {
  //     vtn = new (_vector_vtransform.arena()) VTransformDataScalarNode(_vector_vtransform, n);
  //   }
...
VTransformVectorNode* SuperWordVTransformBuilder::make_vector_vtnode_for_pack(const Pack* pack) const {
  assert(false, "TODO impl");
  return nullptr;
  // Node* p0 = pack->at(0);
  ...
  //   vtn = new (_vector_vtransform.arena()) VTransformLoadVectorNode(_vector_vtransform, properties, vector_p, p0->adr_type(), control_dependency);
  // } else if (p0->is_Store()) {
  //   const VPointer& scalar_p = _vloop_analyzer.vpointers().vpointer(p0->as_Store());
  //   const VPointer vector_p(scalar_p.make_with_size(scalar_p.size() * vlen));
  //   vtn = new (_vector_vtransform.arena()) VTransformStoreVectorNode(_vector_vtransform, properties, vector_p, p0->adr_type());
  // } else if (p0->is_Cmp()) {
  //   vtn = new (_vector_vtransform.arena()) VTransformCmpVectorNode(_vector_vtransform, properties);
  // } else if (p0->is_Bool()) {
  //   VTransformBoolTest kind = _packset.get_bool_test(pack);
  //   vtn = new (_vector_vtransform.arena()) VTransformBoolVectorNode(_vector_vtransform, properties, kind);
  // } else if (p0->is_CMove()) {
  //   vtn = new (_vector_vtransform.arena()) VTransformElementWiseVectorNode(_vector_vtransform, p0->req(), properties, Op_VectorBlend);
  // } else if (_vloop_analyzer.reductions().is_marked_reduction(p0)) {
  //   vtn = new (_vector_vtransform.arena()) VTransformReductionVectorNode(_vector_vtransform, properties);
  ...
  //   vtn = new (_vector_vtransform.arena()) VTransformElementWiseVectorNode(_vector_vtransform, 3, properties, vopc);
  ...
  //   vtn = new (_vector_vtransform.arena()) VTransformElementWiseVectorNode(_vector_vtransform, p0->req(), properties, vopc);
  ...
  //   vtn = new (_vector_vtransform.arena()) VTransformReinterpretVectorNode(_vector_vtransform, properties, src_bt);
  ...
  //   vtn = new (_vector_vtransform.arena()) VTransformElementWiseLongOpWithCastToIntVectorNode(_vector_vtransform, properties);
  // } else {
  //   assert(p0->req() == 3 ||
...
void SuperWordVTransformBuilder::init_req_with_scalar(const VTransformNode* old, VTransformNode* vtn, const int index) {
  assert(false, "TODO impl");
  //VTransformNode* req = get_vtnode_or_wrap_as_outer(old->in(index));
...
VTransformNode* SuperWordVTransformBuilder::get_or_make_vtnode_vector_input_at_index(const Pack* pack, const int index) {
  assert(false, "TODO impl");
  return nullptr;
//  Node* p0 = pack->at(0);
//
//  Pack* pack_in = _packset.pack_input_at_index_or_null(pack, index);
//  if (pack_in != nullptr) {
...
//    if (use_bt != def_bt && !p0->is_Convert() && VectorCastNode::is_supported_subword_cast(def_bt, use_bt, pack->length())) {
//      VTransformNode* in = get_vtnode(pack_in->at(0));
//      const VTransformVectorNodeProperties properties = VTransformVectorNodeProperties::make_from_pack(pack, _vloop_analyzer);
//      VTransformNode* cast = new (_vector_vtransform.arena()) VTransformElementWiseVectorNode(_vector_vtransform, 2, properties, VectorCastNode::opcode(-1, def_bt));
//      cast->set_req(1, in);
//
//      return cast;
//    }
...
//    Pack* pack_in0 = _packset.strided_pack_input_at_index_or_null(pack, index, 2, 0);
//    if (pack_in0 != nullptr) {
//      return get_vtnode(pack_in0->at(0));
//    }
//    Pack* pack_in1 = _packset.strided_pack_input_at_index_or_null(pack, index, 2, 1);
//    if (pack_in1 != nullptr) {
...
//    const TypeVect* vt = TypeVect::make(element_bt, pack->length());
//    VTransformNode* populate_index = new (_vector_vtransform.arena()) VTransformPopulateIndexNode(_vector_vtransform, vt);
...
//      const TypeVect* vt = TypeVect::make(element_bt, pack->length());
//      VTransformNode* shift_count = new (_vector_vtransform.arena()) VTransformShiftCountNode(_vector_vtransform, vt, mask, p0->Opcode());
...
//        VTransformNode* conv = new (_vector_vtransform.arena()) VTransformConvI2LNode(_vector_vtransform);
//        conv->init_req(1, same_input_vtn);
//        same_input_vtn = conv;
//      }
//      const TypeVect* vt = TypeVect::make(element_bt, pack->length());
//      VTransformNode* replicate = new (_vector_vtransform.arena()) VTransformReplicateNode(_vector_vtransform, vt);
...
VTransformNode* SuperWordVTransformBuilder::get_vtnode_or_wrap_as_outer(const VTransformNode* old) {
  assert(false, "TODO impl");
  return nullptr;
  ...
  //vtn = new (_vector_vtransform.arena()) VTransformOuterNode(_vector_vtransform, old);
  //map_node_to_vtnode(n, vtn);
  //assert(vtn == get_vtnode_or_null(n), "consistency");
  //return vtn;
}

void SuperWordVTransformBuilder::init_req_with_vector(const Pack* pack, VTransformNode* vtn, int j) {
  VTransformNode* req = get_or_make_vtnode_vector_input_at_index(pack, j);
  vtn->init_req(j, req);
}

void SuperWordVTransformBuilder::init_all_req_with_scalars(const VTransformNode* old, VTransformNode* vtn) {
  assert(false, "TODO impl");
  //assert(vtn->req() == old->req(), "scalars must have same number of reqs");
  //for (uint j = 0; j < old->req(); j++) {
  //  Node* def = old->in(j);
  //  if (def == nullptr) { continue; }
  //  init_req_with_scalar(old, vtn, j);
  //}
}

void SuperWordVTransformBuilder::init_all_req_with_vectors(const Pack* pack, VTransformNode* vtn) {
  assert(false, "TODO impl");
  //Node* p0 = pack->at(0);
...
void SuperWordVTransformBuilder::add_memory_dependencies_of_node_to_vtnode(const VTransformNode* old, VTransformNode* vtn, VectorSet& vtn_memory_dependencies) {
  assert(false, "TODO impl");
