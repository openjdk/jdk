...
void G1HeapRegionManager::initialize(G1RegionToSpaceMapper* heap_storage,
  ...
  _committed_map.initialize(max_reserved_regions());
}
...
void G1HeapRegionManager::commit_regions(uint index, size_t num_regions, WorkerThreads* pretouch_workers) {
  ...
  guarantee(num_regions <= inactive_regions(),
...
uint G1HeapRegionManager::find_contiguous_in_free_list(uint num_regions) {
  ...
  } while (candidate == G1_NO_HRM_INDEX && range.end() < max_reserved_regions());
...
uint G1HeapRegionManager::find_contiguous_allow_expand(uint num_regions) {
  ...
  if (num_regions > free_or_inactive_regions()) {
    return G1_NO_HRM_INDEX;
  }
  // Find any candidate.
  return find_contiguous_in_range(0, max_reserved_regions(), num_regions);
}
...
void G1HeapRegionManager::verify() {
  ...
  for (uint i = _next_highest_used_hrm_index; i < max_reserved_regions(); i++) {
...
public:
  G1RebuildFreeListTask(G1HeapRegionManager* hrm, uint num_workers) :
      ...
      _worker_chunk_size((_hrm->max_reserved_regions() + num_workers - 1) / num_workers),
  ...
  void work(uint worker_id) {
    ...
    uint end = MIN2(start + _worker_chunk_size, _hrm->max_reserved_regions());
...
void G1HeapRegionManager::rebuild_free_list(WorkerThreads* workers) {
  ...
  uint const num_workers = clamp(max_reserved_regions(), 1u, workers->active_workers());
