...
bool G1ParScanThreadState::inject_evacuation_failure(const HeapRegion* from_region) {
  return _g1h->evac_failure_injector()->evacuation_should_fail(_evac_failure_inject_counter, from_region);
}
#endif
...
oop G1ParScanThreadState::do_copy_to_survivor_space(G1HeapRegionAttr const region_attr,
  ...
  if (inject_evacuation_failure(from_region)) {
