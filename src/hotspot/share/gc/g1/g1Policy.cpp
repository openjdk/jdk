...
G1CollectorState G1Policy::record_young_collection_end(bool concurrent_operation_is_full_mark,
  ...
  if (update_stats) {
    ...
    size_t copied_bytes = p->sum_thread_work_items(G1GCPhaseTimes::FlushPSS, G1GCPhaseTimes::MergePSSCopiedBytes);
...

  {
    ...
    record_dirtying_stats(TimeHelper::counter_to_millis(_g1h->last_refinement_epoch_start()),
                          ...
                          yield_duration_ms,
                          phase_times()->sum_thread_work_items(G1GCPhaseTimes::FlushPSS, G1GCPhaseTimes::MergePSSPendingCards),
                          phase_times()->sum_thread_work_items(G1GCPhaseTimes::FlushPSS, G1GCPhaseTimes::MergePSSToYoungGenCards));
  }
