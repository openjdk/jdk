...
template <typename EventT>
bool ObjectCountEventSenderTemplate<EventT>::should_send_event() {
#if INCLUDE_JFR
  return _should_send_requestable_event || EventT::is_enabled();
#else
  return false;
#endif // INCLUDE_JFR
}
...
template <typename EventT>
bool ObjectCountEventSenderTemplate<EventT>::_should_send_requestable_event = false;

template <typename EventT>
void ObjectCountEventSenderTemplate<EventT>::enable_requestable_event() {
  _should_send_requestable_event = true;
}


template <typename EventT>
void ObjectCountEventSenderTemplate<EventT>::disable_requestable_event() {
  _should_send_requestable_event = false;
}

template <typename EventT>
template <typename T>
void ObjectCountEventSenderTemplate<EventT>::send_event_if_enabled(Klass* klass, jlong count, julong size, const Ticks& timestamp) {
  T event(UNTIMED);
...
template <typename EventT>
void ObjectCountEventSenderTemplate<EventT>::send(const KlassInfoEntry* entry, const Ticks& timestamp) {
  Klass* klass = entry->klass();
  jlong count = entry->count();
  julong total_size = entry->words() * BytesPerWord;
  
  send_event_if_enabled<EventT>(klass, count, total_size, timestamp);
}
