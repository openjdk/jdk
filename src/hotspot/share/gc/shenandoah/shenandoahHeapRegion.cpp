...
size_t ShenandoahHeapRegion::setup_sizes(size_t max_heap_size) {
  ...
  // Enforce additional limit of 256 KB which applies in case HeapRegionSize is larger than 2 MB.
  guarantee(MaxTLABSizeWords == 0, "we should only set it once");
  MaxTLABSizeWords = align_down(MIN2(RegionSizeWords / 8, (size_t) (256 * 1024) / HeapWordSize), MinObjAlignment);
