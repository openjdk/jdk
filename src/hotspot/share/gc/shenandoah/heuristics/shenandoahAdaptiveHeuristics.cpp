...
bool ShenandoahAdaptiveHeuristics::should_start_gc() {
  ...
  if (_gc_times_learned < max_learn) {
    ...
    if (available < init_threshold) {
      log_info(gc, ergo)("Trigger: Learning " SIZE_FORMAT " of " SIZE_FORMAT
                         ". Free (" SIZE_FORMAT "%s) is below initial threshold (" SIZE_FORMAT "%s)",
  ...
  if (avg_cycle_time > allocation_headroom / avg_alloc_rate) {
    log_info(gc, ergo)("Trigger: Average GC time (%.2f ms) is above the time for average allocation rate (%.0f %sB/s) "
  ...
  if (is_spiking && avg_cycle_time > allocation_headroom / rate) {
    log_info(gc, ergo)("Trigger: Average GC time (%.2f ms) is above the time for instantaneous allocation rate (%.0f %sB/s) "
