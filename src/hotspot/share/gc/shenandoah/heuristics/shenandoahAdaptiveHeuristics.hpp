...
class ShenandoahGCQuantityEstimator {
 ...
 public:
  explicit ShenandoahGCQuantityEstimator(const char *name) :
    _name(name),
    _changed(true),
...
#ifdef KELVIN_OBSOLESCENCE
/* kelvin introduced this as part of phase-accounting, but it clashes
 * with a typedef provided in shenandoahAllocRate.hpp.  I had added
 * two methods: double average_rate(double sds) const and double
 * most_recent_instantaneous()(). 
 */
...
private:
  // These instance fields hold values cached by most recent execution of accelerated_consumption()
  double _cached_acceleration;
  double _cached_rate;
  double _cached_weighted_average_rate;
  double _cached_momentary_rate;
  double _timestamp_of_cache;
...
public:
...
#ifdef KELVIN_DEPRECATE_REPLACED_BY_KEMPERW
  ...
  // Based on results computed by most recent invocation of accelerated_consumption(), return the current allocation rate,
  // the current rate of acceleration, and the amount of memory that we expect to consume if we start GC after waiting
  // delay_until_next_check and GC takes predicted_cycle_duration to complete.
  size_t accelerated_consumption_from_cached_values(double& acceleration, double& current_rate,
  ...
  size_t accelerated_consumption(double& acceleration, double& current_rate,
                                 double avg_rate_words_per_sec, double predicted_cycle_time);
...
#define KELVIN_PLANNED_ENHANCEMENTS
...
#define KELVIN_PRIMITIVES
  ...
  // Feeds into predict_gc_mark_time().
  void log_marked_young_words(double at_gc_start_time, size_t words_marked) {
    ...
    _most_recently_marked_young_words = words_marked;
#ifdef KELVIN_PRIMITIVES
    log_info(gc)("log_marked_young_words(at_gc_start_time: %.3f, words_marked: %zu)", at_gc_start_time, words_marked);
#endif
  }

  // Returns the most recently logged value.
  size_t most_recently_marked_young_words() {
    return _most_recently_marked_young_words;
  }

  // Use linear model to predict how many young words will be marked in the next GC cycle as a function of when the next
  // GC cycle will begin. This assumes a linear model, returning max of average and linear prediction plus stdev.
  size_t predict_marked_young_words(double gc_start_time) {
    // Learning cycles generally prime prediction of marked young words
    size_t result = (size_t) _marked_young_words.predict_at(gc_start_time);
#ifdef KELVIN_PRIMITIVES
  ...
  // Use linear model to predict size of the stable remset, returning max of average and linear prediction, plus stdev.
  ...
  // including promotions in place. All of this data is identified as DIRTY. We do not model this quantity with a linear
  // prediction model.  Rather, we remember only the most recent value.
...
#ifdef KELVIN_PRIMITIVES
    log_info(gc)("most_recent_burest_remset_words() returns %zu", _burst_of_remset_words);
#endif
  ...
  void log_words_promoted_by_evacuation(size_t words_promoted) {
    ...
    _words_promoted_during_old_gc += words_promoted;
    _words_promoted_since_start_of_completed_old_gc += words_promoted;
  }

  // This reports the value of the argument most recently passed to log_words_promoted_by_evacuation() for the purpose of
  // calculating the extra remembered set scanning that is required during the next update-references and marking efforts.
  size_t most_recent_words_promoted_by_evacuation() {
#ifdef KELVIN_PRIMITIVES
    log_info(gc)("most_recent_words_promoted_by_evacuation() returns %zu", _words_promoted_by_evacuation);
#endif
    return _words_promoted_by_evacuation;
  }

  // At end of choose_collection_set(), call this to identify how many words of live data will be promoted in place.
  void log_words_promoted_in_place(size_t promote_in_place_words) {
    ...
    _words_promoted_during_old_gc += promote_in_place_words;
    _words_promoted_since_start_of_completed_old_gc += promote_in_place_words;
  }

  // This reports the value of the argument most recently passed to log_words_promoted_in_place() for the purpose of
  // calculating the extra remembered set scanning that is required during the next update references and marking efforts.
  ...
  // Log the words evacuated from young to young durinm most recent GC cycle.
  ...
  // We assume that we can predict words to be evacuated using a linear prediction model in time.  The prediction is
  // the larger of average plus stdev and linear prediction plust stdev.
  size_t predict_young_evacuation_words(double at_gc_start_time) {
    size_t result = (size_t) _words_evacuated_from_young_to_young.predict_at(at_gc_start_time);
#ifdef KELVIN_PRIMITIVES
    log_info(gc)("predict_young_evacuation_words(gc_start_time: %.3f) returns %zu", at_gc_start_time, result);
#endif
    return result;
  }
  ...
  // This reports the value of the argument most recently passed to log_words_allocated_during_mark() for the purpose
  // of caluclating the extra remembered set scanning that is required during the next update references and marking efforts.
...
#endif
    _marked_old_words.add_sample(at_old_gc_start_time, (double) words_marked);
    _words_promoted_since_start_of_completed_old_gc = words_promoted_during_mark;
  }

  // Returns most recently logged value of old words marked.
  ...
  // How many words in old need to be updated, worst case.  This is the known "live" data in old, computed as the sum of
  // live after previous old mark plus subsequent promotions (either by evacuation or in place), excluding non-live data
  // (which has been coalesced and filled).
  ...
  void add_to_words_promoted_since_start_of_old_gc(size_t planned_promotion) {
    _words_promoted_since_start_of_completed_old_gc += planned_promotion;
    ...
    log_info(gc)("add_to_words_promoted_since_start_of_old_gc(planned_promotion: %zu), total: %zu",
                 planned_promotion, _words_promoted_since_start_of_completed_old_gc);
#endif
  }
  ...
  // collection set and/or some of the planned promotions may have failed.

  // Kelvin wonders: probably want to take delta from planned expended vs expended promotion reserve. Is that different from
  // promotion reserve?
  void subtract_from_recently_promoted_words(size_t recently_not_promoted) {
    assert(_words_promoted_since_start_of_completed_old_gc > recently_not_promoted,
           "Expect _words_promoted_since_start_of_old_gc (%zu) > recently_not_promoted (%zu)",
           _words_promoted_since_start_of_completed_old_gc, recently_not_promoted);
    ...
    log_info(gc)("subtract_from_recently_promoted_words(recently_not_promoted: %zu), total: %zu",
                 recently_not_promoted, _words_promoted_since_start_of_completed_old_gc);
#endif
  }
  ...
  void add_to_recently_promoted_words(size_t excess_recently_promoted) {
    _words_promoted_since_start_of_completed_old_gc += excess_recently_promoted;
    ...
    log_info(gc)("add_to_recently_promoted_words(excess_recently_promoted: %zu), total: %zu",
                 excess_recently_promoted, _words_promoted_since_start_of_completed_old_gc);
#endif
  }

  // Note that we care about words promoted since most recently completed old GC even if a new old GC has started since that time.
  size_t words_promoted_since_start_of_most_recently_completed_old_gc() {
#ifdef KELVIN_PRIMITIVES
    log_info(gc)("words_promoted_since_start_of_old_gc() returns %zu", _words_promoted_since_start_of_completed_old_gc);
#endif
    return _words_promoted_since_start_of_completed_old_gc;
  }
  ...
  // The total processed words is sum of marked_young_words plus stable_remset_words plus burst_remset_words.
  void log_gc_mark_time(double at_gc_start_time, size_t marked_young_words, size_t stable_remset_words, size_t burst_remset_words,
    ...
    log_burst_remset_words(burst_remset_words);
  ...
  double predict_gc_mark_time(size_t anticipated_mark_words, size_t anticipated_stable_remset_words, size_t burst_remset_words) {
    size_t anticipated_processed_words =  anticipated_mark_words + anticipated_stable_remset_words + burst_remset_words;
    return predict_mark_time(anticipated_processed_words);
  }
  ...
  // The linear prediction model predicts evacuation time as a function of total words evacuated.  We use a weighted sum
  // of the first four arguments to represent the independent value.  evacuation_time is the dependent value.
  void log_gc_evac_time(size_t words_evacuated_to_young, size_t words_evacuated_to_old, size_t words_promoted_by_evacuation, 
                        size_t words_promoted_in_place, double evacuation_time);

  // Use the linear prediction model to estimate evacuation time. The input to this model is a weighted sum of this function's
  // arguments.  The resulting prediction includes a stdev from the best-fit line.
  double predict_gc_evac_time(size_t words_evacuated_to_young, size_t words_evacuated_to_old, size_t words_promoted_by_evacuation, 
                              size_t words_promoted_in_place);
                        
#ifdef KELVIN_DEPRECATE
  // Removing this function because the caller will explicitly compute estimates of the input argument values
...
#ifdef KELVIN_DEPRECATE
  // REMOVE THIS ONE ALSO.  USE THE SAME PREDICTION AS ABOVE, WITH MORE ACCURATE INPUT PARAMETERS.
...
#ifdef KELVIN_DEPRECATE
  // THIS IS TOO COMPLICATED, LET THE CALLER CHOOSE HOW TO PARAMETERIZE THE ABOVE.
  ...
  // void log_gc_update_time(size_t total_remset_words, size_t young_updated_words, size_t old_updated_words, double update_time) {
  double predict_gc_update_time(size_t total_remset_words,
                                size_t anticipated_young_update_words,
                                size_t anticipated_old_update_words);
#endif // KELVIN_PLANNED_ENHANCEMENTS

/*
  Kelvin plans to redistribute/remove this commentary after implementation is more complete.
...
*/
  ...
  size_t _most_recently_marked_young_words;
  ...
  ShenandoahGCQuantityEstimator _stable_remset_words{"stable_remset_words"};

  // input: uptime at start of young gc, output: anticipated words evacuated from young to young
  ShenandoahGCQuantityEstimator _words_evacuated_from_young_to_young{"words_evacuated_from_young_to_young"};
...
protected:
  ...
  double predict_mark_time(size_t anticipated_processed_words) override;
  double predict_evac_time(size_t anticipated_evac_words, size_t anticipated_pip_words) override;
  double predict_update_time(size_t anticipated_processed_words) override;
  ...
  double predict_gc_time_by_phases(double time_at_start) override;
  ...
  double predict_global_mark_time_after_trigger(double gc_start_time) {
    ...
    size_t anticipated_processed_words = predict_marked_young_words(gc_start_time) + predict_marked_old_words(gc_start_time);
    return predict_mark_time(anticipated_processed_words);
  }
  ...
  void log_gc_evac_time(double start_gc_time,
                        ...
                        size_t words_promoted_in_place, double evacuation_time) {
    log_young_evacuation_words(start_gc_time, words_evacuated_to_young);
    log_words_promoted_by_evacuation(words_promoted_by_evacuation);
    log_words_promoted_in_place(words_promoted_in_place);
    ...
    record_phase_duration(ShenandoahMajorGCPhase::_evac, total_workload, evacuation_time);
  }
  ...
  double predict_evac_time_before_trigger(double at_gc_start_time) {
    ...
    return predict_evac_time(anticipated_young_evac_words + anticipated_promo_evac_words + anticipated_old_evac_words, (size_t) 0);
  }

#ifdef KELVIN_DEPRECATE
  // Just use predict_gc_evac_time() with updated, more accurate values of parameters.
   ...
   // more expensive than words evacuated to young generation). The linear prediction model does recognize that it is easier
   // to promote words in place than to promote by evacuation.
  double predict_gc_evac_time_after_mark(size_t words_evacuated_to_young, size_t words_promoted_by_evacuation,
  ...
  void log_gc_update_time(size_t total_remset_words, size_t young_updated_words, size_t old_updated_words, double update_time) {
    assert((total_remset_words == 0) || (old_updated_words == 0),
    ...
    size_t total_update_words = total_remset_words + young_updated_words + old_updated_words;
    record_phase_duration(ShenandoahMajorGCPhase::_update, total_update_words, update_time);
  }
  ...
  double  predict_update_time_before_trigger(double at_gc_start_time, double predicted_mark_time) {
    ...
    // There's a new definition of ShenandoahAllocationRate
    ...
    // Use the new definition of ShenandoahAllocationRate
...
#else
    // This is just a placeholder until I fix up the code above
  ...
  size_t predict_global_update_time_after_trigger(double at_gc_start_time, double predicted_mark_time);

  size_t predict_update_time_after_mark(ShenandoahCollectionSet* collection_set, double anticipated_start_time);
  ...
  double predict_gc_time(double timestamp_at_start) override;
