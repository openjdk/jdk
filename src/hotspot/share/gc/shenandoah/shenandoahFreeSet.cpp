...
HeapWord* ShenandoahFreeSet::try_allocate_in(ShenandoahHeapRegion* r, ShenandoahAllocRequest& req, bool& in_new_region) {
  ...
  if (_heap->is_concurrent_weak_root_in_progress() && r->is_trash()) {
    // We cannot use this region for allocation when weak roots are in progress because the collector may be
    // concurrently writing nullptrs to unmarked weak references. Seems like this should be part of the region's
    // state machine.
    return nullptr;
  }
