...
HeapWord* ShenandoahFreeSet::try_allocate_in(ShenandoahHeapRegion* r, ShenandoahAllocRequest& req, bool& in_new_region) {
  ...
  if (in_new_region) {
    log_debug(gc)("Using new region (%zu) for %s (" PTR_FORMAT ").",
...
void ShenandoahFreeSet::find_regions_with_alloc_capacity(size_t &young_cset_regions, size_t &old_cset_regions,
  ...
  log_debug(gc)("  At end of prep_to_rebuild, mutator_leftmost: %zu"
  ...
  log_debug(gc)("  old_collector_leftmost: %zu"
  ...
  for (size_t i = _heap->num_regions(); i > 0; i--) {
    ...
    if (move_to_old_collector) {
      ...
      if (r->is_trash() || !r->is_affiliated()) {
        ...
        log_debug(gc)("  Shifting region %zu from mutator_free to old_collector_free", idx);
    ...
    if (move_to_collector) {
      ...
      _partitions.move_from_partition_to_partition(idx, ShenandoahFreeSetPartitionId::Mutator,
                                                   ShenandoahFreeSetPartitionId::Collector, ac);
      log_debug(gc)("  Shifting region %zu from mutator_free to collector_free", idx);
      log_debug(gc)("  Shifted Mutator range [%zd, %zd],"
...
#define BUFFER_SIZE 80
    ...
    log_debug(gc)(" mutator free range [%zu..%zu] allocating from %s, "
                  " collector free range [%zu..%zu], "
                  "old collector free range [%zu..%zu] allocates from %s",
                  _partitions.leftmost(ShenandoahFreeSetPartitionId::Mutator),
                  _partitions.rightmost(ShenandoahFreeSetPartitionId::Mutator),
