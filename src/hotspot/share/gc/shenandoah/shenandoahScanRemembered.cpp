...
#undef KELVIN_DEBUG
#ifdef KELVIN_DEBUG
  log_info(gc)("calc_regular_group_size: num_regions(" SIZE_FORMAT "), group_size(" SIZE_FORMAT ")",
               _heap->num_regions(), group_size);
#endif
...
#ifdef KELVIN_DEBUG
  log_info(gc)("calc_first_group_chunk_size_b4_rebalance: words_in_first_chunk(" SIZE_FORMAT ")",
               words_in_first_chunk);
#endif
...
#ifdef KELVIN_DEBUG
  log_info(gc)("calc_num_groups: current_group_span(" SIZE_FORMAT "), smallest_group_span(" SIZE_FORMAT ")",
               current_group_span, smallest_group_span);
#endif
...
#ifdef KELVIN_DEBUG
    log_info(gc)(" incr num_groups: " SIZE_FORMAT ", cumulative_group_span: " SIZE_FORMAT ", current group span: " SIZE_FORMAT,
                 num_groups, cumulative_group_span, current_group_span);
#endif
...
#ifdef KELVIN_DEBUG
      log_info(gc)(" add one more to num_groups: " SIZE_FORMAT ", current_group_span: "
                   SIZE_FORMAT ", smallest_group_span: " SIZE_FORMAT,
                   num_groups, current_group_span, smallest_group_span);
#endif
    }
...
#ifdef KELVIN_DEBUG
  log_info(gc)(" final answer, num_groups: " SIZE_FORMAT, num_groups);
#endif
  return num_groups;
}
...
size_t ShenandoahRegionChunkIterator::calc_total_chunks() {
...
#ifdef KELVIN_DEBUG
  log_info(gc)("calc_total_chunks, region_size_words: %zu, _maximum_chunk_size_words: %zu",
               region_size_words, _maximum_chunk_size_words);
#endif
...
#ifdef KELVIN_DEBUG
    log_info(gc)("calc_total_chunks, effective_chunk_size initialized to %zu", effective_chunk_size);
    log_info(gc)(" current_group_span: %zu, which is group_chunk_size_b4_rebalance: %zu * regular_group_size: %zu",
                 current_group_span, _first_group_chunk_size_b4_rebalance, _regular_group_size);
#endif
...
#ifdef KELVIN_DEBUG
      log_info(gc)(" reposturing group[%u] as num_chunks: %zu times chunk_size: %zu",
                   coalesced_groups, current_group_span / _maximum_chunk_size_words, _maximum_chunk_size_words);
#endif
...
#ifdef KELVIN_DEBUG
    log_info(gc)(" We have repostured %u groups, spanning %zu chunks (%zu words)", coalesced_groups, num_chunks,
                 _heap->num_regions() * region_size_words - unspanned_heap_size);
...
#ifdef KELVIN_DEBUG
    log_info(gc)(" Adjusting _num_groups to %zu, _largest_chunk_size_words: %zu", _adjusted_num_groups, _largest_chunk_size_words);
#endif
  } else {
...
#ifdef KELVIN_DEBUG
  log_info(gc)("calc_total_chunks, first group spans: " SIZE_FORMAT " words, which is " SIZE_FORMAT
               " chunks, next group will span " SIZE_FORMAT,
               _heap->num_regions() * region_size_words - unspanned_heap_size, num_chunks, current_group_span);
  log_info(gc)(" _largest_chunk_size_words: " SIZE_FORMAT, _largest_chunk_size_words);
#endif
  ...
  while (unspanned_heap_size > 0) {
    if (current_group_span <= unspanned_heap_size) {
      ...
      num_chunks += _regular_group_size;
#ifdef KELVIN_DEBUG
      log_info(gc)("group[" SIZE_FORMAT "] spans " SIZE_FORMAT " words, " SIZE_FORMAT " chunks",
                   spanned_groups, current_group_span, _regular_group_size);
#endif
      spanned_groups++;
      ...
      if (spanned_groups >= _num_groups) {
...
#ifdef KELVIN_DEBUG
        log_info(gc)("adding " SIZE_FORMAT " chunks, each of span " SIZE_FORMAT " words to group[" SIZE_FORMAT "]",
                     extra_chunks, chunk_span, _num_groups - 1);
        log_info(gc)("total spanned chunks: " SIZE_FORMAT, num_chunks);
#endif
        return num_chunks;
        ...
        num_chunks += extra_chunks;
#ifdef KELVIN_DEBUG
        log_info(gc)("adding " SIZE_FORMAT " smallest chunks, each of span " SIZE_FORMAT " words to group[" SIZE_FORMAT "]",
                     extra_chunks, chunk_span, spanned_groups -1);
        log_info(gc)("total spanned chunks: " SIZE_FORMAT, num_chunks);
#endif
        return num_chunks;
      } else {
...
#ifdef KELVIN_DEBUG
      log_info(gc)("Truncated group[" SIZE_FORMAT "] spans " SIZE_FORMAT " words, " SIZE_FORMAT " chunks",
                   spanned_groups, last_group_size, last_group_size);
        log_info(gc)("total spanned chunks: " SIZE_FORMAT, num_chunks);
#endif
      return num_chunks;
    }
...
#ifdef KELVIN_DEBUG
    log_info(gc)("_group_chunk_size[0]: " SIZE_FORMAT ", group_entries[0]: " SIZE_FORMAT,
                 _maximum_chunk_size_words, num_chunks);
#endif
  } else {
...
#ifdef KELVIN_DEBUG
    log_info(gc)("_group_chunk_size[0]: " SIZE_FORMAT ", group_entries[0]: " SIZE_FORMAT,
                 _first_group_chunk_size_b4_rebalance, _regular_group_size);
#endif
  }

#ifdef KELVIN_DEBUG
    log_info(gc)("[0] _group_chunk_size: %7zu, _region_index: " SIZE_FORMAT
                 ", _group_offset: " SIZE_FORMAT ", _group_entries: " SIZE_FORMAT,
                 _group_chunk_size[0], _region_index[0], _group_offset[0], _group_entries[0]);
#endif
...
#ifdef KELVIN_DEBUG
    log_info(gc)("[" SIZE_FORMAT "] _group_chunk_size: %7zu, _region_index: " SIZE_FORMAT
                 ", _group_offset: " SIZE_FORMAT ", _group_entries: " SIZE_FORMAT,
                 i, _group_chunk_size[i], _region_index[i], _group_offset[i], _group_entries[i]);
#endif
  }
...
#ifdef KELVIN_DEBUG
    log_info(gc)("adjusting _group_entries[" SIZE_FORMAT "] to " SIZE_FORMAT, _adjusted_num_groups-1, _total_chunks);
#endif
  }
