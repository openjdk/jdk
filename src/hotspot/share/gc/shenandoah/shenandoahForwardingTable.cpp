...
void ShenandoahForwardingTable::enter_forwarding(HeapWord* original, HeapWord* forwardee) {
  ...
  log_develop_trace(gc)("Finding slot, start at index: %llu, for original: " PTR_FORMAT ", forwardee: " PTR_FORMAT, index, p2i(original), p2i(forwardee));
  ShenandoahMarkingContext* ctx = ShenandoahHeap::heap()->marking_context();
  while (table[index].original() != nullptr || table[index].is_marked(ctx)) {
    log_develop_trace(gc)("Collision on %llu: [" PTR_FORMAT ", " PTR_FORMAT "): is_marked: %s, original: " PTR_FORMAT ", forwardee: " PTR_FORMAT, index, p2i(&table[index]._original), p2i(&table[index]._forwardee), BOOL_TO_STR(table[index].is_marked(ctx)), p2i(table[index].original()), p2i(table[index].forwardee()));
    index++;
...
HeapWord* ShenandoahForwardingTable::forwardee(HeapWord* original) const {
  ...
  uint64_t hash = FastHash::get_hash64(reinterpret_cast<uint64_t>(original), reinterpret_cast<uint64_t>(table));
  uint64_t index = hash % _num_entries;
  log_develop_trace(gc)("Finding slot, start at index: %llu, for original: " PTR_FORMAT, index, p2i(original));
  ShenandoahMarkingContext* ctx = ShenandoahHeap::heap()->marking_context();
