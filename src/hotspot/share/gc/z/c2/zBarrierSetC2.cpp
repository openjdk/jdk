...
class ZArenaHashtable : public ResourceObj {
  ...
  static const size_t table_mask = table_size - 1;

  Arena* _arena;
  ZArenaHashtableEntry* _table[table_size];
...
public:
  ...
  public:
    ...
    void next() {
      ...
      while (_current_entry == nullptr && ++_current_index < table_size) {
  ...
  void add(K key, V value) {
    ...
    entry->_key = key;
    entry->_value = value;
    entry->_next = _table[key & table_mask];
    _table[key & table_mask] = entry;
  }

  V* get(K key) const {
    for (ZArenaHashtableEntry* e = _table[key & table_mask]; e != nullptr; e = e->_next) {
      if (e->_key == key) {
        return &(e->_value);
      }
    }
