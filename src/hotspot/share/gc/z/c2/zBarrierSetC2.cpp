  ...
  if (ZBarrierSet::barrier_needed(access.decorators(), access.type())) {
    if (access.decorators() & C2_TIGHTLY_COUPLED_ALLOC) {
      ...
      if (access.decorators() & ON_WEAK_OOP_REF) {
        access.set_barrier_data(ZBarrierWeak);
      } else if (access.decorators() & ON_PHANTOM_OOP_REF) {
        access.set_barrier_data(ZBarrierPhantom);
      } else {
        access.set_barrier_data(ZBarrierStrong);
      }
    }
  }
}
