...
private:
  ...
  Ticks     _end_of_last;
  NumberSeq _normalized_duration;

public:
  ZStatCycle();

  void at_start();
  void at_end(GCCause::Cause cause, double boost_factor);

  bool is_warm();
  uint64_t nwarmup_cycles();

  bool is_normalized_duration_trustable();
  const AbsSeq& normalized_duration();
