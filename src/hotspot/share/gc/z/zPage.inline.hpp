...
inline bool ZPage::is_object_marked_live(zaddress addr) const {
  ...
  assert(!is_old() || ZHeap::heap()->major_collector()->is_phase_mark(), "Location should match phase");
  assert(!is_young() || ZHeap::heap()->minor_collector()->is_phase_mark(), "Location should match phase");
...
#define assert_zpage_mark_state()                                                              \
  do {                                                                                         \
    ...
    assert(!is_young() || !ZHeap::heap()->minor_collector()->is_phase_mark(), "Wrong phase");  \
    assert(!is_old() || !ZHeap::heap()->major_collector()->is_phase_mark(), "Wrong phase");    \
  } while (0)
