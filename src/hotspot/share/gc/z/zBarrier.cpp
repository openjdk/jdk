...
static void mark(zaddress addr) {
  ...
  // Mark
  ZHeap::heap()->mark_object<follow, finalizable, publish>(addr);
}
...
void ZBarrier::keep_alive(zaddress addr) {
  if (matches_mark_phase(addr)) {
    mark<ZMark::Follow, ZMark::Strong, ZMark::Publish>(addr);
  }
}
...
zaddress ZBarrier::mark_slow_path(zaddress addr) {
  ...
  // Mark
  mark<ZMark::Follow, ZMark::Strong, ZMark::Overflow>(addr);
...
zaddress ZBarrier::mark_finalizable_slow_path(zaddress addr) {
  ...
  // Mark
  mark<ZMark::Follow, ZMark::Finalizable, ZMark::Overflow>(addr);
