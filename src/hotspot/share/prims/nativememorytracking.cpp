...
JVM_ENTRY(void, NMT_paintAllocation(JNIEnv *env, jobject ignored_this, jlong address, jlong mem_tag)) {
  MemTag tag = (MemTag)mem_tag;
  void* o = (void*)address;
  MallocHeader* header = MallocHeader::resolve_checked(o);
  size_t size = header->size();
  MallocTracker::record_free_block(o);
  MallocTracker::record_malloc((void*)header, size, tag, CURRENT_PC);
}
JVM_END

static JNINativeMethod NMT_methods[] = {
  {CC "makeTag", CC "(Ljava/lang/String;)J", FN_PTR(NMT_makeTag)},
  {CC "paintAllocation", CC "(JJ)V", FN_PTR(NMT_paintAllocation)}
};
