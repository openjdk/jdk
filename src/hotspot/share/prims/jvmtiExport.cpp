...
void JvmtiDynamicCodeEventCollector::register_stub(const char* name, address start, address end) {
 if (_code_blobs == NULL) {
   _code_blobs = new (mtServiceability) GrowableArray<JvmtiCodeBlobDesc*>(mtServiceability, 1);
 }
...
void JvmtiObjectAllocEventCollector::record_allocation(oop obj) {
  ...
  if (_allocated == NULL) {
    _allocated = new (mtServiceability) GrowableArray<OopHandle>(mtServiceability, 1);
  }
