...
bool MethodComparator::args_same(Bytecodes::Code const c_old,  Bytecodes::Code const c_new,
  ...
  case Bytecodes::_invokedynamic: {
    ...
    int cpi_new;
    if (UseNewIndyCode) {
      cpi_old = old_cp->cache()->resolved_indy_info(old_cp->decode_invokedynamic_index(cpci_old))->cpool_index();
      cpi_new = new_cp->cache()->resolved_indy_info(new_cp->decode_invokedynamic_index(cpci_new))->cpool_index();
      ...
      cpi_old = old_cp->cache()->entry_at(cpci_old)->constant_pool_index();
      cpi_new = new_cp->cache()->entry_at(cpci_new)->constant_pool_index();
    }
