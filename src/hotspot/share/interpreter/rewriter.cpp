...
void Rewriter::maybe_rewrite_invokehandle(address opc, int cp_index, int cache_index, bool reverse) {
  if (!reverse) {
    if ((*opc) == (u1)Bytecodes::_invokevirtual ||
      ...
      if (status == 0) {
        if (_pool->klass_ref_at_noresolve(cp_index, (Bytecodes::Code)(*opc)) == vmSymbols::java_lang_invoke_MethodHandle() &&
          ...
          status = +1;
        } else if (_pool->klass_ref_at_noresolve(cp_index, (Bytecodes::Code)(*opc)) == vmSymbols::java_lang_invoke_VarHandle() &&
    ...
    switch (c) {
      ...
      case Bytecodes::_putfield       : {
        if (!reverse) {
          ...
          Symbol* ref_class_name = cp->klass_name_at(cp->klass_ref_index_at(bc_index, c));
