...
class InterpreterRuntime: AllStatic {
 ...
 private:
  ...
  static void      note_trap(JavaThread* current, int reason, TRAPS);
 ...
 public:
  // Constants
  static void    ldc           (JavaThread* current /* TRAPS */, bool wide);
  static void    resolve_ldc   (JavaThread* current /* TRAPS */, Bytecodes::Code bytecode);

  // Allocation
  static void    _new          (JavaThread* current /* TRAPS */, ConstantPool* pool, int index);
  static void    newarray      (JavaThread* current /* TRAPS */, BasicType type, jint size);
  static void    anewarray     (JavaThread* current /* TRAPS */, ConstantPool* pool, int index, jint size);
  static void    multianewarray(JavaThread* current /* TRAPS */, jint* first_size_address);
  static void    register_finalizer(JavaThread* current /* TRAPS */, oopDesc* obj);

  // Quicken instance-of and check-cast bytecodes
  static void    quicken_io_cc(JavaThread* current /* TRAPS */);

  // Exceptions thrown by the interpreter
  static void    throw_AbstractMethodError(JavaThread* current /* TRAPS */);
  static void    throw_AbstractMethodErrorWithMethod(JavaThread* current /* TRAPS */, Method* oop);
  static void    throw_AbstractMethodErrorVerbose(JavaThread* current /* TRAPS */,
                                                  Klass* recvKlass,
                                                  Method* missingMethod);

  static void    throw_IncompatibleClassChangeError(JavaThread* current /* TRAPS */);
  static void    throw_IncompatibleClassChangeErrorVerbose(JavaThread* current /* TRAPS */,
                                                           Klass* resc,
  ...
  static void    throw_ArrayIndexOutOfBoundsException(JavaThread* current /* TRAPS */, arrayOopDesc* a, jint index);
  static void    throw_ClassCastException(JavaThread* current /* TRAPS */, oopDesc* obj);
  static void    throw_NullPointerException(JavaThread* current /* TRAPS */);

  static void    create_exception(JavaThread* current /* TRAPS */, char* name, char* message);
  static void    create_klass_exception(JavaThread* current /* TRAPS */, char* name, oopDesc* obj);
 ...
 private:
  ...
  // Calls
  static void resolve_invoke(JavaThread* current /* TRAPS */, Bytecodes::Code bytecode);
  static void resolve_invokehandle (JavaThread* current /* TRAPS */);
  static void resolve_invokedynamic(JavaThread* current /* TRAPS */);
 ...
 public:
  ...
  static void    throw_illegal_monitor_state_exception(JavaThread* current /* TRAPS */);
  ...
  static void prepare_native_call(JavaThread* current /* TRAPS */, Method* method);
