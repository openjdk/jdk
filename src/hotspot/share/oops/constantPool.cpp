...
size_t BSMAttributeEntry::copy_into(u2* data, size_t size) const {
...
void BSMAttributeEntries::copy_into(Array<u4>* new_offsets, Array<u2>* new_array,
                                    int num_entries, int initial_offset_index, int initial_array_offset) {
  ...
  for (int i = 0; i < num_entries; i++) {
    ...
    BSMAttributeEntry* bsmae = entry(i);
...
void BSMAttributeEntries::append(BSMAttributeEntries& other, ClassLoaderData* loader_data, TRAPS) {
...
BSMAttributeEntry* BSMAttributeEntries::InsertionIterator::reserve_new_entry(u2 bsmi, u2 argc) {
  if (cur_offset >= insert_into->offsets()->length() ||
      cur_array + argc >= insert_into->bootstrap_methods()->length()) {
    return nullptr;
  }

  const int offset = cur_offset;
  insert_into->_offsets->at_put(offset, cur_array);
  BSMAttributeEntry* e = insert_into->entry(offset);
  // Insert data
...
void BSMAttributeEntries::truncate_to(InsertionIterator& iter, ClassLoaderData* loader_data,
  ...
  assert(iter.cur_array <= this->_bootstrap_methods->length(), "must be");

  if (iter.cur_offset == this->_offsets->length() &&
      iter.cur_array == this->_bootstrap_methods->length()) {
    return;
  }

  Array<u4>* new_offsets =
      MetadataFactory::new_array<u4>(loader_data, iter.cur_offset, 0, CHECK);
  Array<u2>* new_array =
    MetadataFactory::new_array<u2>(loader_data, iter.cur_array, CHECK);
  // Copy over the remaining entries
  copy_into(new_offsets, new_array, iter.cur_offset);
  // Replace content
  deallocate_contents(loader_data);
  _offsets = new_offsets;
  _bootstrap_methods = new_array;
