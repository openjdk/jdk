...
class BSMAttributeEntry {
  ...
  size_t copy_into(u2* data, size_t size) const;
...
public:
  ...
  // The number of u2s the BSM data takes.
  // As this is a straight copy of the BootstrapMethods attribute, thi
  int array_length() const {
    return _bootstrap_methods == nullptr ? 0 :  _bootstrap_methods->length();
  }

  struct InsertionIterator {
    BSMAttributeEntries* insert_into;
    // Current unused offset into BSMAEs offset array (this is also the number of entries)
    int cur_offset;
    // Current unused offset into BSMAEs bsm-data array
    int cur_array;

    InsertionIterator() : insert_into(nullptr), cur_offset(-1), cur_array(-1) {}
    ...
    bool failed() { return insert_into == nullptr; }

    int current_offset() { return cur_offset; }
    // Add a new BSMAE, reserving the necessary memory for filling the argument vector.
    // Returns null if there isn't enough space.
    BSMAttributeEntry* reserve_new_entry(u2 bsmi, u2 argc);
  };
