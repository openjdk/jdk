...
class EscapeBarrier : StackObj {
  ...
  bool deoptimize_objects_internal(JavaThread* deoptee, frame fr, const RegisterMap* reg_map);
  ...
  bool deoptimize_objects(frame fr, const RegisterMap* reg_map) {
    return deoptimize_objects_internal(deoptee_thread(), fr, reg_map);
  }
...
public:
  ...
  static bool objs_are_deoptimized(JavaThread* thread, frame fr, const RegisterMap* reg_map);
