...
void ObjectMonitor::enter_internal(JavaThread* current, ObjectWaiter* currentNode) {
  // We use reenter path only for monitor reentry in wait(). It is a contented slow path.
  // Reenter path requires currentNode != nullptr, whereas enter path does not need it at all.
  const bool enterPath = currentNode == nullptr;
