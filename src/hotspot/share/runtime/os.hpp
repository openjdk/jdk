...
class os: AllStatic {
 ...
 public:
  ...
  // set to -1 (if offset is non-nullptr).
  ...
  // and offset is set to -1 (if offset is non-nullptr).
  ...
  // in case of an error nullptr is returned and an error message is stored in ebuf
  ...
  // return current frame. pc() and sp() are set to nullptr on failure.
  ...
  // returns nullptr if exception_code is not an OS exception/signal.
  ...
  // handles nullptr pointers
  static void  free    (void *memblock);
  static char* strdup(const char *, MEMFLAGS flags = mtInternal);  // Like strdup
  // Like strdup, but exit VM when strdup() returns nullptr
