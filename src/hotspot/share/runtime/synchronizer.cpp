...
void MonitorList::unlink_deflated(GrowableArray<ObjectMonitor*>* unlinked_list) {
  size_t deleted = 0;
  ...
  while (m != NULL) {
    ...
    if (SafepointMechanism::should_process(JavaThread::current())) {
      // Honor block request.
...
void ObjectSynchronizer::deflate_idle_monitors() {
  ...
  size_t deflated = deflate_monitor_list();
  if (deflated > 0) {
    ...
    _in_use_list.unlink_deflated(&delete_list);
