 ...
 *
 * @param number           unique identifier for the register. A valid register has {@code number >= 0}.
 * @param encoding         the target machine encoding for the register, which may or may not be the
 *                         same as {@link #number}.
...
public record Register(int              number,
                       int              encoding,
                       String           name,
                       RegisterCategory registerCategory) implements Comparable<Register> {
    /**
     * A platform specific register type that describes which values can be stored in a register.
     */
    public record RegisterCategory(String name, boolean mayContainReference) {
