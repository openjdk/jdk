...
public class XRRenderer implements PixelDrawPipe, PixelFillPipe, ShapeDrawPipe {
    ...
    public void fillRect(SunGraphics2D sg2d, int x, int y, int width, int height) {
        ...
        try {
            validateSurface(sg2d);
            rectBuffer.add(new Rect(x, y, width, height));
            tileManager.fillMask((XRSurfaceData) sg2d.surfaceData);
        } finally {
            SunToolkit.awtUnlock();
        }
    ...
    private class XRDrawHandler extends ProcessPath.DrawHandler {
        ...
        public void drawLine(int x1, int y1, int x2, int y2) {
            ...
            if (xDiff == 0 || yDiff == 0) {
                ...
                rectBuffer.add(new Rect(x1, y1, 1, 1));
                rectBuffer.add(new Rect(x2, y2, 1, 1));
            } else {
        ...
        public void drawScanline(int x1, int x2, int y) {
            rectBuffer.add(new Rect(x1, y, x2 - x1 + 1, 1));
        }
    }
    ...
    protected void fillSpans(SunGraphics2D sg2d, SpanIterator si,
        ...
        try {
            ...
            while (si.nextSpan(spanBox)) {
                rectBuffer.add(new Rect(spanBox[0] + transx,
                                    spanBox[1] + transy,
                                    spanBox[2] - spanBox[0],
                                    spanBox[3] - spanBox[1]));
            }
