...
final class StandardHelpFormatter {
    ...
    final static class OptionGroupContent {
        ...
        static OptionGroupContent launcherPlatformGroupContent(OperatingSystem os) {
            return platform().options(launcherOptions(os)).create();
        }
        ...
        static OptionGroupContent nativePackagePlatformGroupContent(OperatingSystem os) {
            return platform().options(nativePackageOptions(os)).create();
        }
        ...
        private static Builder shared(OperatingSystem os) {
            return new Builder(sharedOption()).optionSpecDescriptionGetter(standardOptionSpecDescriptionGetter(os));
        }

        private static Builder platform() {
        ...
        private static final class Builder {
            ...
            Builder options(Stream<? extends OptionSpec<?>> v) {
                optionSpecs.addAll(v.map(optionSpec -> {
        ...
        private static Stream<? extends OptionSpec<?>> nativePackageOptions(OperatingSystem os) {
            ...
            final var base = StandardOption.options().stream().map(Option::getSpec).filter(platformOption(os))
        ...
        private static String formatStandardPackageTypeList(OperatingSystem os) {
            return StandardBundlingOperation.ofPlatform(os)
                    .map(StandardBundlingOperation::packageTypeValue)
        ...
        private static Function<OptionSpec<?>, String> standardOptionSpecDescriptionGetter(OperatingSystem os) {
            return optionSpec -> {
                if (equals(StandardOption.TYPE, optionSpec)) {
                    return I18N.format(optionSpec.description(), formatStandardPackageTypeList(os));
                } else if (equals(StandardOption.PREDEFINED_APP_IMAGE, optionSpec) && OperatingSystem.MACOS == os) {
                    optionSpec = optionSpec.copyWithDescription("main.option.app-image.mac");
                } else if (equals(StandardOption.APP_CONTENT, optionSpec) && OperatingSystem.MACOS == os) {
                    optionSpec = optionSpec.copyWithDescription("main.option.app-content.mac");
                } else if (equals(StandardOption.INSTALL_DIR, optionSpec) && OperatingSystem.WINDOWS == os) {
                    optionSpec = optionSpec.copyWithDescription("main.option.install-dir.windows");
                }
                return getDefaultOptionSpecDescription(optionSpec);
            };
        }

        private static boolean equals(OptionValue<?> optionValue, OptionSpec<?> optionSpec) {
            return optionValue.getSpec().name().equals(optionSpec.name());
        }
    ...
    void formatNoArgsHelp(Consumer<CharSequence> sink) {
        sink.accept(I18N.format("MSG_Help_no_args"));
        eol(sink);
    }
    ...
    private static final OptionSpec<Path> RUNTIME_INSTALLER_RUNTIME_IMAGE =
            StandardOption.PREDEFINED_RUNTIME_IMAGE.getSpec().copyWithDescription("main.option.installer-runtime-image");
