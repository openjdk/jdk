...
import jdk.jpackage.internal.cli.StandardAppImageFileOption;
    ...
    private static ApplicationWithDetails createMacApplicationInternal(Options optionValues) {
        ...
        if (MAC_APP_STORE.containsIn(optionValues)) {
            predefinedRuntimeLayout.ifPresent(layout -> {
                if (Files.isDirectory(layout.runtimeDirectory().resolve("bin"))) {
                    throw new JPackageException(I18N.format("ERR_MacAppStoreRuntimeBinExists",
                            layout.rootDirectory().toAbsolutePath()));
                }
            });
        }
