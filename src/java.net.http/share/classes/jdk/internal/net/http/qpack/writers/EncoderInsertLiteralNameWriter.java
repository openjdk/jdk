...
final class EncoderInsertLiteralNameWriter implements BinaryRepresentationWriter {
    ...
    EncoderInsertLiteralNameWriter(QPACK.Logger logger) {
        this.logger = logger;
    }
    ...
    public BinaryRepresentationWriter configure(TableEntry e) throws IndexOutOfBoundsException {
        if (logger.isLoggable(EXTRA)) {
            logger.log(EXTRA, () -> format(
                    "Insert With Literal Name (%s, '%s', huffmanName=%b, huffmanValue=%b)",
                    e.name(), e.value(), e.huffmanName(), e.huffmanValue()));
        }
        return this.name(e).value(e);
    }
    ...
    private EncoderInsertLiteralNameWriter name(TableEntry e) {
        int N = 5;
    ...
    private EncoderInsertLiteralNameWriter value(TableEntry e) {
        int N = 7;
