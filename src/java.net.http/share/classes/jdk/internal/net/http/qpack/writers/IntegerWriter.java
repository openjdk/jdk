...
public final class IntegerWriter {
    ...
    //       payload           N=5
    //
    // payload is the contents of the left-hand side part of the first octet;
    //         it is truncated to fit into 8-N bits, where 1 <= N <= 8;
    //
    public IntegerWriter configure(long value, int N, int payload) {
        if (state != NEW) {
    ...
    public static int requiredBufferSize(int N, long value) {
        checkPrefix(N);
        int size = 1;
        int max = (2 << (N - 1)) - 1;
        if (value < max) {
