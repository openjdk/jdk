...
public class kqueue_h {
    ...
    private static class kevent {
        ...
        public static final MethodHandle HANDLE = Linker.nativeLinker().downcallHandle(ADDR, DESC,  Linker.Option.captureCallState("errno"));
    }
     ...
     * }
     */
    public static int kevent(int kq, MemorySegment changelist, int nchanges, MemorySegment eventlist, int nevents, MemorySegment timeout, MemorySegment errnoCaptureState) {
        var mh$ = kevent.HANDLE;
        try {
            if (FFMUtils.TRACE_DOWNCALLS) {
                FFMUtils.traceDowncall("kevent", kq, changelist, nchanges, eventlist, nevents, timeout);
            }
            return (int)mh$.invokeExact(errnoCaptureState, kq, changelist, nchanges, eventlist, nevents, timeout);
        } catch (Throwable ex$) {
           throw new AssertionError("should not reach here", ex$);
        }
    }
