...
class KQueue {
    ...
    static public int register(int kqfd, int fd, int filter, int flags) {
        ...
        try (var arena = Arena.ofConfined()) {
            MemorySegment errnoCapture = ErrnoTools.allocateCaptureStateSegment(arena);
            try {
                ...
                do {
                    result = kqueue_h.kevent(
                            ...
                            0, NULL, errnoCapture);
                    errno = ErrnoTools.errno(errnoCapture);
                } while ((result == -1) && (errno == errno_h.EINTR()));
            } catch (Throwable e) {
    ...
    static public int poll(int kqfd, MemorySegment pollAddress, int nevents, long timeout) {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            result = kqueue_h.kevent(
                    ...
                    nevents, tsp, errnoCapture);
            int errno = ErrnoTools.errno(errnoCapture);
            if (result < 0) {
                if (errno == errno_h.EINTR()) {
                    return IOStatus.INTERRUPTED;
                } else {
                    throw ErrnoTools.IOExceptionWithLastError(errno, "kqueue_poll failed.", arena);
                }
            }
        } catch (Throwable e) {
            throw new RuntimeException(e);
