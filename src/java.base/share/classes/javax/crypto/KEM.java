 ...
 * <p>
 * Optionally, a KEM implementation may support authenticated encapsulation
 * that ensures the shared secret was generated by the holder of a private key.
 * The sender calls {@code newAuthEncapsulator} with its own private key as
 * the second argument to create an encapsulator. The corresponding receiver
 * calls {@code newAuthDecapsulator} with the sender's public key as the second
 * argument to create a decapsulator. By calling the {@code decapsulate} method
 * to retrieve the shared secret, the receiver can be guaranteed that the
 * shared secret is only known by a party that owns the private key.
 *
 * Example:
 * {@snippet lang = java:
 *    // Receiver side
...
public final class KEM {
    ...
    private static final class DelayedKEM {
        ...
        private Encapsulator newEncapsulator(
                PublicKey publicKey,
                AlgorithmParameterSpec spec, SecureRandom secureRandom)
                throws InvalidAlgorithmParameterException, InvalidKeyException {
            return newBothEncapsulator(publicKey, null, spec, secureRandom);
        }

        private Encapsulator newAuthEncapsulator(
            ...
            return newBothEncapsulator(publicKeyR, privateKeyS, spec, secureRandom);
        }

        private Encapsulator newBothEncapsulator(
                PublicKey publicKeyR, PrivateKey privateKeyS,
                AlgorithmParameterSpec spec, SecureRandom secureRandom)
                throws InvalidAlgorithmParameterException, InvalidKeyException {
            if (publicKeyR == null) {
            ...
            for (Provider.Service service : list) {
                if (!service.supportsParameter(publicKeyR)) {
                    continue;
                }
                if (privateKeyS != null && !service.supportsParameter(privateKeyS)) {
                    continue;
                }
                try {
                    ...
                    if (privateKeyS == null) {
                        return new Encapsulator(
                                spi.engineNewEncapsulator(publicKeyR, spec, secureRandom),
                                service.getProvider());
                    } else {
                        return new Encapsulator(
                                spi.engineNewAuthEncapsulator(publicKeyR, privateKeyS, spec, secureRandom),
                                service.getProvider());
                    }
            ...
            throw new InvalidKeyException("No installed provider supports this key: "
                            + publicKeyR.getClass().getName(), re);
        }
        ...
        private Decapsulator newDecapsulator(PrivateKey privateKey, AlgorithmParameterSpec spec)
                throws InvalidAlgorithmParameterException, InvalidKeyException {
            return newBothDecapsulator(privateKey, null, spec);
        }
        private Decapsulator newAuthDecapsulator(PrivateKey privateKeyR, PublicKey publicKeyS, AlgorithmParameterSpec spec)
                throws InvalidAlgorithmParameterException, InvalidKeyException {
            if (publicKeyS == null) {
                throw new InvalidKeyException("input key is null");
            }
            return newBothDecapsulator(privateKeyR, publicKeyS, spec);
        }
        private Decapsulator newBothDecapsulator(PrivateKey privateKeyR, PublicKey publicKeyS, AlgorithmParameterSpec spec)
                throws InvalidAlgorithmParameterException, InvalidKeyException {
            if (privateKeyR == null) {
                throw new InvalidKeyException("input key is null");
            }
            ...
            NoSuchAlgorithmException nsae = null;
            for (Provider.Service service : list) {
                if (!service.supportsParameter(privateKeyR)) {
                    continue;
                }
                if (publicKeyS != null && !service.supportsParameter(publicKeyS)) {
                    continue;
                }
                try {
                    KEMSpi spi = (KEMSpi) service.newInstance(null);
                    if (publicKeyS == null) {
                        return new Decapsulator(
                                spi.engineNewDecapsulator(privateKeyR, spec),
                                service.getProvider());
                    } else {
                        return new Decapsulator(
                                spi.engineNewAuthDecapsulator(privateKeyR, publicKeyS, spec),
                                service.getProvider());
                    }
                } catch (NoSuchAlgorithmException e) {
                    nsae = merge(nsae, e);
            ...
            throw new InvalidKeyException("No installed provider supports this key: "
                    + privateKeyR.getClass().getName(), re);
        }
    }
     ...
     * <p>
     * This method is equivalent to
     * {@code newAuthEncapsulator(publicKeyR, privateKeyS, null, null)}.
     *
     ...
     * @throws UnsupportedOperationException if the KEM implementation does not
     *          support authentication encapsulation or this method requires
     *          an {@code AlgorithmParameterSpec} to be provided
     * @since 23
     */
    public Encapsulator newAuthEncapsulator(PublicKey publicKeyR, PrivateKey privateKeyS)
            throws InvalidKeyException {
        try {
            return newAuthEncapsulator(publicKeyR, privateKeyS, null, null);
        } catch (InvalidAlgorithmParameterException e) {
            throw new UnsupportedOperationException(
                    "AlgorithmParameterSpec must be provided", e);
        }
     ...
     * <p>
     * This method is equivalent to
     * {@code newAuthEncapsulator(publicKeyR, privateKeyS, null, secureRandom)}.
     *
     * @param publicKeyR   the receiver's public key, must not be {@code null}
     * @param privateKeyS  the sender's private key, must not be {@code null}
     * @param secureRandom the source of randomness for encapsulation.
     ...
     * @since 23
     */
    public Encapsulator newAuthEncapsulator(PublicKey publicKeyR,
            PrivateKey privateKeyS, SecureRandom secureRandom)
            throws InvalidKeyException {
        try {
            return newAuthEncapsulator(publicKeyR, privateKeyS, null, secureRandom);
        } catch (InvalidAlgorithmParameterException e) {
            throw new UnsupportedOperationException(
                    "AlgorithmParameterSpec must be provided", e);
        }
    ...
    public Encapsulator newEncapsulator(PublicKey publicKey,
        ...
        return delayed != null
                ...
                : new Encapsulator(spi.engineNewEncapsulator(
                        publicKey, spec, secureRandom), provider);
    }
     ...
     * instance with the {@code params} byte array received and recover
     * an {@code AlgorithmParameterSpec} object to be used in its
     * {@link #newAuthDecapsulator(PrivateKey, PublicKey, AlgorithmParameterSpec)}
     * call.
     *
     * @param publicKeyR   the receiver's public key, must not be {@code null}
     * @param privateKeyS  the sender's private key, must not be {@code null}
    ...
    public Encapsulator newAuthEncapsulator(PublicKey publicKeyR, PrivateKey privateKeyS,
            ...
            throws InvalidAlgorithmParameterException, InvalidKeyException {
        return delayed != null
                ? delayed.newAuthEncapsulator(publicKeyR, privateKeyS, spec, secureRandom)
                : new Encapsulator(spi.engineNewAuthEncapsulator(
                        publicKeyR, privateKeyS, spec, secureRandom), provider);
    }
     ...
     * <p>
     * This method is equivalent to
     * {@code newAuthDecapsulator(privateKeyR, publicKeyS, null)}.
     *
     ...
     * @since 23
     */
    public Decapsulator newAuthDecapsulator(PrivateKey privateKeyR, PublicKey publicKeyS)
            throws InvalidKeyException {
        try {
            return newAuthDecapsulator(privateKeyR, publicKeyS, null);
        } catch (InvalidAlgorithmParameterException e) {
            throw new UnsupportedOperationException(e);
        }
    }
     ...
     * @since 23
     */
    public Decapsulator newAuthDecapsulator(PrivateKey privateKeyR,
            PublicKey publicKeyS, AlgorithmParameterSpec spec)
            throws InvalidAlgorithmParameterException, InvalidKeyException {
        return delayed != null
                ? delayed.newAuthDecapsulator(privateKeyR, publicKeyS, spec)
                : new Decapsulator(spi.engineNewAuthDecapsulator(
                        privateKeyR, publicKeyS, spec), provider);
    }
