...
public class FtpClient extends sun.net.ftp.FtpClient {
  ...
  private static String[] patStrings = {
    ...
    "([\\-ld](?:[r\\-][w\\-][x\\-]){3})\\s*\\d+"
        + " (\\w+)\\s*(\\w+)\\s*(\\d+)\\s*([A-Z][a-z][a-z]\\s*\\d+)\\s*(\\d\\d:\\d\\d)\\s*(\\p{Print}*)",
    // drwxr-xr-x  1 user01        ftp   512 Jan 29 1997 prog
    "([\\-ld](?:[r\\-][w\\-][x\\-]){3})\\s*\\d+"
        + " (\\w+)\\s*(\\w+)\\s*(\\d+)\\s*([A-Z][a-z][a-z]\\s*\\d+)\\s*(\\d{4})\\s*(\\p{Print}*)",
        ...
        AccessController.doPrivileged(
            new PrivilegedAction<String>() {
              public String run() {
                ...
                vals[0] =
                    Integer.getInteger("sun.net.client.defaultReadTimeout", 300_000).intValue();
                vals[1] =
                    Integer.getInteger("sun.net.client.defaultConnectTimeout", 300_000).intValue();
  ...
  /**
   * Test the named character encoding to verify that it converts ASCII characters correctly. We
   * have to use an ASCII based encoding, or else the NetworkClients will not work correctly in
   * EBCDIC based systems. However, we cannot just use ASCII or ISO8859_1 universally, because in
   * Asian locales, non-ASCII characters may be embedded in otherwise ASCII based protocols (e.g.
   * HTTP). The specifications (RFC2616, 2398) are a little ambiguous in this matter. For instance,
   * RFC2398 [part 2.1] says that the HTTP request URI should be escaped using a defined mechanism,
   * but there is no way to specify in the escaped string what the original character set is. It is
   * not correct to assume that UTF-8 is always used (as in URLs in HTML 4.0). For this reason,
   * until the specifications are updated to deal with this issue more comprehensively, and more
   * importantly, HTTP servers are known to support these mechanisms, we will maintain the current
   * behavior where it is possible to send non-ASCII characters in their original unescaped form.
   */
  private static boolean isASCIISuperset(String encoding) throws Exception {
    ...
    byte[] chkB = {
      ...
      106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 45, 95,
      46, 33, 126, 42, 39, 40, 41, 59, 47, 63, 58, 64, 38, 61, 43, 36, 44
    };
  ...
  private class DefaultParser implements FtpDirParser {
     ...
     * Possible patterns:
     *
     * <p>drwxr-xr-x 1 user01 ftp 512 Jan 29 23:32 prog drwxr-xr-x 1 user01 ftp 512 Jan 29 1997 prog
     * drwxr-xr-x 1 1 1 512 Jan 29 23:32 prog lrwxr-xr-x 1 user01 ftp 512 Jan 29 23:32 prog ->
     * prog2000 drwxr-xr-x 1 username ftp 512 Jan 29 23:32 prog -rw-r--r-- 1 jcc staff 105009 Feb 3
     * 15:05 test.1
     *
     * <p>01-29-97 11:32PM <DIR> prog 04/28/2006 09:12a 3,563 genBuffer.sh
     *
     * <p>drwxr-xr-x folder 0 Jan 29 23:32 prog
     *
     * <p>0 DIR 01-29-97 23:32 PROG
     */
  ...
  private void getTransferSize() {
    lastTransSize = -1;
    /**
     * If it's a start of data transfer response, let's try to extract the size from the response
     * string. Usually it looks like that:
     *
     * <p>150 Opening BINARY mode data connection for foo (6701 bytes).
     */
  ...
  /**
   * extract the created file name from the response string: 226 Transfer complete (unique file
   * name:toto.txt.1). Usually happens when a STOU (store unique) command had been issued.
   */
  ...
  /**
   * Sends a command to the FTP server and returns the error code (which can be a "success") sent by
   * the server.
   *
   * @param cmd
   * @return <code>true</code> if the command was successful
   * @throws IOException
   */
  private boolean issueCommand(String cmd) throws IOException, sun.net.ftp.FtpProtocolException {
    if (!isConnected()) {
  ...
  private void issueCommandCheck(String cmd) throws sun.net.ftp.FtpProtocolException, IOException {
    if (!issueCommand(cmd)) {
      throw new sun.net.ftp.FtpProtocolException(
          cmd + ":" + getResponseString(), getLastReplyCode());
    }
  }
  ...
  /**
   * Opens a "PASSIVE" connection with the server and returns the connected <code>Socket</code>.
   *
   * @return the connected <code>Socket</code>
   * @throws IOException if the connection was unsuccessful.
   */
  private Socket openPassiveDataConnection(String cmd)
     ...
     * Here is the idea:
     *
     * <p>- First we want to try the new (and IPv6 compatible) EPSV command But since we want to be
     * nice with NAT software, we'll issue the EPSV ALL command first. EPSV is documented in RFC2428
     * - If EPSV fails, then we fall back to the older, yet ok, PASV - If PASV fails as well, then
  ...
  /**
   * Returns an InetSocketAddress, based on value of acceptPasvAddressVal and other conditions such
   * as the server address returned by pasv is not a hostname, is a socks proxy, or the loopback. An
   * exception is thrown if none of the valid conditions are met.
   */
  ...
  /**
   * Opens a data connection with the server according to the set mode (ACTIVE or PASSIVE) then send
   ...
   */
  private Socket openDataConnection(String cmd)
      throws sun.net.ftp.FtpProtocolException, IOException {
    Socket clientSocket;
    if (passiveMode) {
      try {
    ...
    if (useCrypto) {
      try {
        clientSocket =
            sslFact.createSocket(
                clientSocket, serverAddr.getHostName(), serverAddr.getPort(), true);
      } catch (Exception ex) {
  ...
  /**
   * Set the transfer mode to <I>passive</I>. In that mode, data connections are established by
   * having the client connect to the server. This is the recommended default mode as it will work
   * best through firewalls and NATs.
   *
   ...
   *
   * @param timeout the timeout value, in milliseconds, to use for the connect operation. A value of
   *     zero or less, means use the default timeout.
   * @return This FtpClient
   */
   ...
   *
   * @param timeout the timeout value, in milliseconds, to use for the read operation. A value of
   *     zero or less, means use the default timeout.
   * @return This FtpClient
   */
   ...
   *
   * @return the <code>Proxy</code>, this client is using, or <code>null</code> if none is used.
   * @see #setProxy(Proxy)
   */
  ...
  public sun.net.ftp.FtpClient connect(SocketAddress dest)
      throws sun.net.ftp.FtpProtocolException, IOException {
  ...
  public sun.net.ftp.FtpClient connect(SocketAddress dest, int timeout)
    ...
    if (!readReply()) {
      throw new sun.net.ftp.FtpProtocolException(
          "Welcome message: " + getResponseString(), lastReplyCode);
    }
    welcomeMsg = getResponseString().substring(4);
    return this;
  }

  private void tryLogin(String user, char[] password)
      throws sun.net.ftp.FtpProtocolException, IOException {
  ...
  /**
   * Logs out the current user. This is in effect terminates the current session and the connection
  ...
  /**
   * Returns the server current working directory, or <code>null</code> if the PWD command failed.
   *
   * @return a <code>String</code> containing the current working directory, or <code>null</code>
   * @throws IOException
   */
  ...
  /**
   * Sets the restart offset to the specified value. That value will be sent through a <code>REST
   * </code> command to server before a file transfer and has the effect of resuming a file transfer
   * from the specified point. After a transfer the restart offset is set back to zero.
   *
   * @param offset the offset in the remote file at which to start the next transfer. This must be a
  ...
  /**
   * Retrieves a file from the ftp server and writes it to the specified <code>OutputStream</code>.
   * If the restart offset was set, then a <code>REST</code> command will be sent before the RETR in
   * order to restart the transfer from the specified offset. The <code>OutputStream</code> is not
   * closed by this method at the end of the transfer.
   *
   * @param name a {@code String} containing the name of the file to retrieve from the server.
  ...
  /**
   * Retrieves a file from the ftp server, using the RETR command, and returns the InputStream from*
   * the established data connection. {@link #completePending()} <b>has</b> to be called once the
   * application is done reading from the returned stream.
   *
   * @param name the name of the remote file
   * @return the {@link java.io.InputStream} from the data connection, or <code>null</code> if the
  ...
  /**
   * Transfers a file from the client to the server (aka a <I>put</I>) by sending the STOR or STOU
   * command, depending on the <code>unique</code> argument, and returns the <code>OutputStream
   * </code> from the established data connection. {@link #completePending()} <b>has</b> to be
   * called once the application is finished writing to the stream.
   *
   * <p>A new file is created at the server site if the file specified does not already exist.
   *
   * <p>If <code>unique</code> is set to <code>true</code>, the resultant file is to be created
   * under a name unique to that directory, meaning it will not overwrite an existing file, instead
   * the server will generate a new, unique, file name. The name of the remote file can be
   * retrieved, after completion of the transfer, by calling {@link #getLastFileName()}.
   *
   * @param name the name of the remote file to write.
   * @param unique <code>true</code> if the remote files should be unique, in which case the STOU
   *     command will be used.
   * @return the {@link java.io.OutputStream} from the data connection or <code>null</code> if the
   *     command was unsuccessful.
   * @throws IOException if an error occurred during the transmission.
   */
  ...
  /**
   * Transfers a file from the client to the server (aka a <I>put</I>) by sending the STOR command.
   * The content of the <code>InputStream</code> passed in argument is written into the remote file,
   * overwriting any existing data.
   *
   * <p>A new file is created at the server site if the file specified does not already exist.
   *
   * @param name the name of the remote file to write.
   * @param local the <code>InputStream</code> that points to the data to transfer.
   * @param unique <code>true</code> if the remote file should be unique (i.e. not already
   *     existing), <code>false</code> otherwise.
  ...
  /**
   * Sends the APPE command to the server in order to transfer a data stream passed in argument and
   * append it to the content of the specified remote file.
   *
   * @param name A <code>String</code> containing the name of the remote file to append to.
   * @param local The <code>InputStream</code> providing access to the data to be appended.
   * @return <code>true</code> if the transfer was successful.
   * @throws IOException if an error occurred during the transmission.
   */
   ...
   */
  public sun.net.ftp.FtpClient rename(String from, String to)
      throws sun.net.ftp.FtpProtocolException, IOException {
   ...
   *
   * @param name a <code>String</code> containing the name of the file to delete.
   * @return <code>true</code> if the command was successful
   * @throws IOException if an error occurred during the exchange
   */
  public sun.net.ftp.FtpClient deleteFile(String name)
      throws sun.net.ftp.FtpProtocolException, IOException {
   ...
   *
   * @param name a <code>String</code> containing the name of the directory to create.
   * @return <code>true</code> if the operation was successful.
   * @throws IOException if an error occurred during the exchange
   */
  public sun.net.ftp.FtpClient makeDirectory(String name)
      throws sun.net.ftp.FtpProtocolException, IOException {
   ...
   *
   * @param name a <code>String</code> containing the name of the directory to remove.
   * @return <code>true</code> if the operation was successful.
   * @throws IOException if an error occurred during the exchange.
   */
  public sun.net.ftp.FtpClient removeDirectory(String name)
      throws sun.net.ftp.FtpProtocolException, IOException {
  ...
  /**
   * Sends a No-operation command. It's useful for testing the connection status or as a <I>keep
   * alive</I> mechanism.
   *
  ...
  /**
   * Sends the STAT command to the server. This can be used while a data connection is open to get a
   * status on the current transfer, in that case the parameter should be <code>null</code>. If used
   * between file transfers, it may have a pathname as argument in which case it will work as the
   * LIST command except no data connection will be created.
   *
   * @param name an optional <code>String</code> containing the pathname the STAT command should
   *     apply to.
   * @return the response from the server or <code>null</code> if the command failed.
   * @throws IOException if an error occurred during the transmission.
   */
  ...
  /**
   * Sends the FEAT command to the server and returns the list of supported features in the form of
   * strings.
   *
   * <p>The features are the supported commands, like AUTH TLS, PROT or PASV. See the RFCs for a
   * complete list.
   *
   * <p>Note that not all FTP servers support that command, in which case the method will return
   * <code>null</code>
   *
   * @return a <code>List</code> of <code>Strings</code> describing the supported additional
   *     features, or <code>null</code> if the command is not supported.
   * @throws IOException if an error occurs during the transmission.
   */
  ...
  /**
   * sends the ABOR command to the server. It tells the server to stop the previous command or
   * transfer.
   *
   * @return <code>true</code> if the command was successful.
   * @throws IOException if an error occurred during the transmission.
  ...
  /**
   * Some methods do not wait until completion before returning, so this method can be called to
   * wait until completion. This is typically the case with commands that trigger a transfer like
   * {@link #getFileStream(String)}. So this method should be called before accessing information
   * related to such a command.
   *
   * <p>This method will actually block reading on the command channel for a notification from the
   * server that the command is finished. Such a notification often carries extra information
   * concerning the completion of the pending action (e.g. number of bytes transferred).
   *
   * <p>Note that this will return true immediately if no command or action is pending
   *
   * <p>It should be also noted that most methods issuing commands to the ftp server will call this
   * method if a previous command is pending.
   *
   * <p>Example of use:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Since {@link #logout()} will call completePending() if necessary.
   *
   * @return <code>true</code> if the completion was successful or if no action was pending.
   * @throws IOException
   */
  ...
  /**
   * Changes the transfer type (binary, ascii, ebcdic) and issue the proper command (e.g. TYPE A) to
   * the server.
   *
   ...
   */
  public sun.net.ftp.FtpClient setType(TransferType type)
      throws sun.net.ftp.FtpProtocolException, IOException {
    String cmd = "NOOP";
  ...
  /**
   * Issues a LIST command to the server to get the current directory listing, and returns the
   * InputStream from the data connection. {@link #completePending()} <b>has</b> to be called once
   * the application is finished writing to the stream.
   *
   * @param path the pathname of the directory to list, or <code>null</code> for the current working
   *     directory.
  ...
  /**
   * Issues a NLST path command to server to get the specified directory content. It differs from
   * {@link #list(String)} method by the fact that it will only list the file names which would make
   * the parsing of the somewhat easier.
   *
   * <p>{@link #completePending()} <b>has</b> to be called once the application is finished writing
   * to the stream.
   *
   * @param path a <code>String</code> containing the pathname of the directory to list or <code>
   *     null</code> for the current working directory.
   * @return the <code>InputStream</code> from the resulting data connection
   * @throws IOException if an error occurs during the transmission.
   */
   ...
   * Note that this command may not be supported by the server. In which case -1 will be returned.
   *
   * @param path a <code>String</code> containing the pathname of the file.
   * @return a <code>long</code> containing the size of the file or -1 if the server returned an
   *     error, which can be checked with {@link #getLastReplyCode()}.
   * @throws IOException if an error occurs during the transmission.
   */
  ...
  private static final DateTimeFormatter RFC3659_DATETIME_FORMAT =
      DateTimeFormatter.ofPattern("yyyyMMddHHmmss[.SSS]").withZone(ZoneOffset.UTC);

  /**
   * Issues the MDTM [path] command to the server to get the modification time of a specific file on
   * the server. Note that this command may not be supported by the server, in which case <code>null
   * </code> will be returned.
   *
   * @param path a <code>String</code> containing the pathname of the file.
   * @return a <code>Date</code> representing the last modification time or <code>null</code> if the
   *     server returned an error, which can be checked with {@link #getLastReplyCode()}.
   * @throws IOException if an error occurs during the transmission.
   */
  ...
  /**
   * Sets the parser used to handle the directory output to the specified one. By default the parser
   * is set to one that can handle most FTP servers output (Unix base mostly). However it may be
   * necessary for and application to provide its own parser due to some uncommon output format.
   *
  ...
  /**
   * Issues a MLSD command to the server to get the specified directory listing and applies the
   * current parser to create an Iterator of {@link java.net.ftp.FtpDirEntry}. Note that the
   * Iterator returned is also a {@link java.io.Closeable}. If the server doesn't support the MLSD
   * command, the LIST command is used instead.
   *
   * <p>{@link #completePending()} <b>has</b> to be called once the application is finished
   * iterating through the files.
   *
   * @param path the pathname of the directory to list or <code>null</code> for the current working
   *     directory.
   * @return a <code>Iterator</code> of files or <code>null</code> if the command failed.
   * @throws IOException if an error occurred during the transmission
  ...
  /**
   * Attempts to use Kerberos GSSAPI as an authentication mechanism with the ftp server. This will
   * issue an <code>AUTH GSSAPI</code> command, and if it is accepted by the server, will followup
   * with <code>ADAT</code> command to exchange the various tokens until authentication is
   ...
   *
   * @return a <code>String</code> containing the message the server returned during connection or
   *     <code>null</code>.
   */
   ...
   *
   * @return the message string, which can be quite long, last returned by the server.
   */
  ...
  /**
   * Returns, when available, the size of the latest started transfer. This is retrieved by parsing
   * the response string received as an initial response to a RETR or similar request.
   *
   * @return the size of the latest transfer or -1 if either there was no transfer or the
  ...
  /**
   * Returns, when available, the remote name of the last transferred file. This is mainly useful
   * for "put" operation when the unique flag was set since it allows to recover the unique file
   * name created on the server which may be different from the one submitted with the command.
   *
   * @return the name the latest transferred file remote name, or <code>null</code> if that
  ...
  /**
   * Attempts to switch to a secure, encrypted connection. This is done by sending the "AUTH TLS"
   * command.
   *
   * <p>See <a href="http://www.ietf.org/rfc/rfc4217.txt">RFC 4217</a> If successful this will
   * establish a secure command channel with the server, it will also make it so that all other
   * transfers (e.g. a RETR command) will be done over an encrypted channel as well unless a {@link
   * #reInit()} command or a {@link #endSecureSession()} command is issued.
   *
   * @return <code>true</code> if the operation was successful.
   * @throws IOException if an error occurred during the transmission.
  ...
  /**
   * Sends a <code>CCC</code> command followed by a <code>PROT C</code> command to the server
   * terminating an encrypted session and reverting back to a non encrypted transmission.
   *
   * @return <code>true</code> if the operation was successful.
  ...
  /**
   * Sends the "Allocate" (ALLO) command to the server telling it to pre-allocate the specified
   * number of bytes for the next transfer.
   *
   * @param size The number of bytes to allocate.
   * @return <code>true</code> if the operation was successful.
   * @throws IOException if an error occurred during the transmission.
   */
  public sun.net.ftp.FtpClient allocate(long size)
      throws sun.net.ftp.FtpProtocolException, IOException {
  ...
  /**
   * Sends the "Structure Mount" (SMNT) command to the server. This let the user mount a different
   * file system data structure without altering his login or accounting information.
   *
   * @param struct a <code>String</code> containing the name of the structure to mount.
   * @return <code>true</code> if the operation was successful.
   * @throws IOException if an error occurred during the transmission.
   */
  public sun.net.ftp.FtpClient structureMount(String struct)
      throws sun.net.ftp.FtpProtocolException, IOException {
  ...
  /**
   * Sends a SYST (System) command to the server and returns the String sent back by the server
   * describing the operating system at the server.
   *
   * @return a <code>String</code> describing the OS, or <code>null</code> if the operation was not
   *     successful.
   * @throws IOException if an error occurred during the transmission.
   */
  ...
  /**
   * Sends the HELP command to the server, with an optional command, like SITE, and returns the text
   * sent back by the server.
   *
   * @param cmd the command for which the help is requested or <code>null</code> for the general
   *     help
   * @return a <code>String</code> containing the text sent back by the server, or <code>null</code>
  ...
  public String getHelp(String cmd) throws sun.net.ftp.FtpProtocolException, IOException {
    ...
    /**
     * HELP 214-The following commands are implemented. USER EPRT STRU ALLO DELE SYST RMD MDTM ADAT
     * PASS EPSV MODE REST CWD STAT PWD PROT QUIT LPRT RETR RNFR LIST HELP CDUP PBSZ PORT LPSV STOR
     * RNTO NLST NOOP STOU AUTH PASV TYPE APPE ABOR SITE MKD SIZE CCC 214 Direct comments to
     * ftp-bugs@jsn.
     *
     * <p>HELP SITE 214-The following SITE commands are implemented. UMASK HELP GROUPS IDLE ALIAS
     * CHECKMETHOD CHMOD CDPATH CHECKSUM 214 Direct comments to ftp-bugs@jsn.
     */
    Vector<String> resp = getResponseStrings();
    if (resp.size() == 1) {
  ...
  /**
   * Sends the SITE command to the server. This is used by the server to provide services specific
   * to his system that are essential to file transfer.
   *
   * @param cmd the command to be sent.
   * @return <code>true</code> if the command was successful.
   * @throws IOException if an error occurred during transmission
   */
  public sun.net.ftp.FtpClient siteCmd(String cmd)
      throws sun.net.ftp.FtpProtocolException, IOException {
