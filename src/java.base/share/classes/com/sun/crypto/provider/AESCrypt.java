...
public final class AESCrypt extends SymmetricCipher {
    ...
    // Irreducible Polynomial of x^8 + x^4 + x^3 + x + 1
    private static final byte MX = 0x1B;
    ...
    void init(boolean decrypting, String algorithm, byte[] key)
      throws InvalidKeyException {
        ...
        if (algorithm.equalsIgnoreCase("AES_128") ||
                key.length == AES_128_NKEYS) {
            rounds = AES_128_ROUNDS;
            nk = AES_128_NKEYS/WB;
        } else if (algorithm.equalsIgnoreCase("AES_192") ||
                key.length == AES_192_NKEYS) {
            rounds = AES_192_ROUNDS;
            nk = AES_192_NKEYS/WB;
        } else if (algorithm.equalsIgnoreCase("AES_256") ||
                key.length == AES_256_NKEYS) {
            rounds = AES_256_ROUNDS;
            nk = AES_256_NKEYS/WB;
        } else {
            throw new InvalidKeyException(
                    "Invalid algorithm name (" + algorithm + ").");
        }
        expandedKey = genRKeys(rounds, key, nk);
        invExpandedKey = invGenRKeys(rounds, key, nk);
    }

    private static void printArray(int[] ek) {
        for (int eke : ek) {
            System.out.printf("%02x %02x %02x %02x ", (eke>>24)&0xFF,
                    (eke>>16)&0xFF, (eke>>8)&0xFF, eke&0xFF);
        }
        System.out.println();
    }

    private static void printArray(int ek) {
        System.out.printf("%02x %02x %02x %02x ", (ek>>24)&0xFF, (ek>>16)&0xFF,
                (ek>>8)&0xFF, ek&0xFF);
        System.out.println();
    }

    private static void printArray(byte[] m) {
        for (int me : m) {
            System.out.printf("%02x ", me);
        }
        System.out.println();
    }
    ...
    private static byte mult(byte b) {
        byte tmp = (byte) (b<<1);
    ...
    private static byte multMix(byte s, byte mix) {
        byte two, four, eight;
        ...
        switch(mix) {
            ...
            case 0x03 -> s ^= two;
            case 0x09 -> s ^= eight;
            case 0x0b -> s ^= (byte) (eight ^ two);
            case 0x0d -> s ^= (byte) (eight ^ four);
            case 0x0e -> s = (byte) (eight ^ four ^ two);
            // The default case will never happen
        }
        return s;
    }

    private void genTablesMult() {
        System.out.println("    private static final int[] TMI = {");
        System.out.print("        ");
        for (int i = 0; i < 256; i++) {
            //Shifting is irrelevant as the value is preshifted for LUTs
            byte a0 = multMix((byte) i, (byte) 0x09);
            byte a1 = multMix((byte) i, (byte) 0x0B);
            byte a2 = multMix((byte) i, (byte) 0x0D);
            byte a3 = multMix((byte) i, (byte) 0x0E);
    ...
    private void genTablesI() {
        for (int j = 0; j < 4; j++) {
            System.out.println("    private static final int[] TI" + j + " = {");
            System.out.print("        ");
            for (int i = 0; i < 256; i++) {
                //Shifting is irrelevant as the value is preshifted for LUTs
                byte a0 = multMix(ISBOX[(i&0xF0)>>4][i&0x0F], IMC[0][j]);
                byte a1 = multMix(ISBOX[(i&0xF0)>>4][i&0x0F], IMC[1][j]);
                byte a2 = multMix(ISBOX[(i&0xF0)>>4][i&0x0F], IMC[2][j]);
                byte a3 = multMix(ISBOX[(i&0xF0)>>4][i&0x0F], IMC[3][j]);
    ...
    private void genTables() {
        for (int j = 0; j < 4; j++) {
            ...
            System.out.print("        ");
            for (int i = 0; i < 256; i++) {
                byte a0 = multMix(SBOX[(i&0xF0)>>4][i&0x0F], MC[0][j]);
                byte a1 = multMix(SBOX[(i&0xF0)>>4][i&0x0F], MC[1][j]);
                byte a2 = multMix(SBOX[(i&0xF0)>>4][i&0x0F], MC[2][j]);
                byte a3 = multMix(SBOX[(i&0xF0)>>4][i&0x0F], MC[3][j]);
    ...
    private static void invGenMix(int[] state) {
        ...
        for (int k = 0; k < len; k++) {
            for (int i = 0; i < len; i++) {
                byte s1 = invMix((byte) ((state[k] >> 24) & 0xFF), IMC[i][0]);
                byte s2 = invMix((byte) ((state[k] >> 16) & 0xFF), IMC[i][1]);
                byte s3 = invMix((byte) ((state[k] >> 8) & 0xFF), IMC[i][2]);
                byte s4 = invMix((byte) (state[k] & 0xFF), IMC[i][3]);
                // XOR to add MixColumn matrix per element
                byte aSum = (byte) (s1 ^ s2 ^ s3 ^ s4);
                mSum[k] |= (aSum & 0xFF) << (((len - 1) - i) * 8);
            }
        }
    ...
    private int[] invGenRKeys(int r, byte[] key, int nk) {
        int len = WB;
        int[] wMat = new int[len];
        int[] w = genRKeys(r, key, nk);

        for (int i = 1; i < r; i++) {
            System.arraycopy(w, i * len, wMat, 0, len);
            invGenMix(wMat);
    ...
    private static final int[] TMI = {
        ...
        0x82A6CAFC, 0x8BADC7F2, 0xB49CE4D8, 0xBD97E9D6, 0xA68AFEC4, 0xAF81F3CA,
        0xD8E8B890, 0xD1E3B59E, 0xCAFEA28C, 0xC3F5AF82, 0xFCC48CA8, 0xF5CF81A6,
        0xEED296B4, 0xE7D99BBA, 0x3B7BBBDB, 0x3270B6D5, 0x296DA1C7, 0x2066ACC9,
        0x1F578FE3, 0x165C82ED, 0x0D4195FF, 0x044A98F1, 0x7323D3AB, 0x7A28DEA5,
        0x6135C9B7, 0x683EC4B9, 0x570FE793, 0x5E04EA9D, 0x4519FD8F, 0x4C12F081,
        0xABCB6B3B, 0xA2C06635, 0xB9DD7127, 0xB0D67C29, 0x8FE75F03, 0x86EC520D,
        0x9DF1451F, 0x94FA4811, 0xE393034B, 0xEA980E45, 0xF1851957, 0xF88E1459,
        0xC7BF3773, 0xCEB43A7D, 0xD5A92D6F, 0xDCA22061, 0x76F66DAD, 0x7FFD60A3,
        0x64E077B1, 0x6DEB7ABF, 0x52DA5995, 0x5BD1549B, 0x40CC4389, 0x49C74E87,
        0x3EAE05DD, 0x37A508D3, 0x2CB81FC1, 0x25B312CF, 0x1A8231E5, 0x13893CEB,
        0x08942BF9, 0x019F26F7, 0xE646BD4D, 0xEF4DB043, 0xF450A751, 0xFD5BAA5F,
        0xC26A8975, 0xCB61847B, 0xD07C9369, 0xD9779E67, 0xAE1ED53D, 0xA715D833,
        0xBC08CF21, 0xB503C22F, 0x8A32E105, 0x8339EC0B, 0x9824FB19, 0x912FF617,
        0x4D8DD676, 0x4486DB78, 0x5F9BCC6A, 0x5690C164, 0x69A1E24E, 0x60AAEF40,
        0x7BB7F852, 0x72BCF55C, 0x05D5BE06, 0x0CDEB308, 0x17C3A41A, 0x1EC8A914,
        0x21F98A3E, 0x28F28730, 0x33EF9022, 0x3AE49D2C, 0xDD3D0696, 0xD4360B98,
        0xCF2B1C8A, 0xC6201184, 0xF91132AE, 0xF01A3FA0, 0xEB0728B2, 0xE20C25BC,
        0x95656EE6, 0x9C6E63E8, 0x877374FA, 0x8E7879F4, 0xB1495ADE, 0xB84257D0,
        0xA35F40C2, 0xAA544DCC, 0xECF7DA41, 0xE5FCD74F, 0xFEE1C05D, 0xF7EACD53,
        0xC8DBEE79, 0xC1D0E377, 0xDACDF465, 0xD3C6F96B, 0xA4AFB231, 0xADA4BF3F,
        0xB6B9A82D, 0xBFB2A523, 0x80838609, 0x89888B07, 0x92959C15, 0x9B9E911B,
        0x7C470AA1, 0x754C07AF, 0x6E5110BD, 0x675A1DB3, 0x586B3E99, 0x51603397,
        0x4A7D2485, 0x4376298B, 0x341F62D1, 0x3D146FDF, 0x260978CD, 0x2F0275C3,
        0x103356E9, 0x19385BE7, 0x02254CF5, 0x0B2E41FB, 0xD78C619A, 0xDE876C94,
        0xC59A7B86, 0xCC917688, 0xF3A055A2, 0xFAAB58AC, 0xE1B64FBE, 0xE8BD42B0,
        0x9FD409EA, 0x96DF04E4, 0x8DC213F6, 0x84C91EF8, 0xBBF83DD2, 0xB2F330DC,
        0xA9EE27CE, 0xA0E52AC0, 0x473CB17A, 0x4E37BC74, 0x552AAB66, 0x5C21A668,
        0x63108542, 0x6A1B884C, 0x71069F5E, 0x780D9250, 0x0F64D90A, 0x066FD404,
        0x1D72C316, 0x1479CE18, 0x2B48ED32, 0x2243E03C, 0x395EF72E, 0x3055FA20,
        0x9A01B7EC, 0x930ABAE2, 0x8817ADF0, 0x811CA0FE, 0xBE2D83D4, 0xB7268EDA,
        0xAC3B99C8, 0xA53094C6, 0xD259DF9C, 0xDB52D292, 0xC04FC580, 0xC944C88E,
        0xF675EBA4, 0xFF7EE6AA, 0xE463F1B8, 0xED68FCB6, 0x0AB1670C, 0x03BA6A02,
        0x18A77D10, 0x11AC701E, 0x2E9D5334, 0x27965E3A, 0x3C8B4928, 0x35804426,
        0x42E90F7C, 0x4BE20272, 0x50FF1560, 0x59F4186E, 0x66C53B44, 0x6FCE364A,
        0x74D32158, 0x7DD82C56, 0xA17A0C37, 0xA8710139, 0xB36C162B, 0xBA671B25,
        0x8556380F, 0x8C5D3501, 0x97402213, 0x9E4B2F1D, 0xE9226447, 0xE0296949,
        0xFB347E5B, 0xF23F7355, 0xCD0E507F, 0xC4055D71, 0xDF184A63, 0xD613476D,
        0x31CADCD7, 0x38C1D1D9, 0x23DCC6CB, 0x2AD7CBC5, 0x15E6E8EF, 0x1CEDE5E1,
        0x07F0F2F3, 0x0EFBFFFD, 0x7992B4A7, 0x7099B9A9, 0x6B84AEBB, 0x628FA3B5,
        0x5DBE809F, 0x54B58D91, 0x4FA89A83, 0x46A3978D,
    };
} //class AESCrypt
