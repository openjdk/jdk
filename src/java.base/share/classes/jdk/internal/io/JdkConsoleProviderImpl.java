...
/**
 * JdkConsoleProvider implementation based on the platform's TTY.
 */
public final class JdkConsoleProviderImpl implements JdkConsoleProvider {
    private static final StableValue<JdkConsole> INSTANCE = StableValue.of();

    @Override
    public JdkConsole console(boolean isTTY, Charset inCharset, Charset outCharset) {
        return INSTANCE.orElseSet(() -> new JdkConsoleImpl(inCharset, outCharset));
    }

    /**
     * JdkConsole implementation based on the platform's TTY.
     */
    static final class JdkConsoleImpl implements JdkConsole {
        @Override
                ...
                synchronized(readLock) {
                    ...
                    try {
                        ...
                        if (ioe != null) {
                            Arrays.fill(passwd, ' ');
                            try {
                                if (reader instanceof jdk.internal.io.JdkConsoleImpl.LineReader lr) {
                                    lr.zeroOut();
                                }
                            } catch (IOException _) {
                                // ignore
        ...
        public JdkConsoleImpl(Charset inCharset, Charset outCharset) {
            ...
            reader = new JdkConsoleImpl.LineReader(StreamDecoder.forInputStreamReader(
