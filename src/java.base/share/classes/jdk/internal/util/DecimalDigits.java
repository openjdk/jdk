...
public final class DecimalDigits {
    ...
    public static void appendPair(StringBuilder buf, int v) {
        ...
        int packed = DIGITS[v & 0x7f];
        // Use append(char) to allow StringOpts optimization to combine into appendLatin1
        // when the StringBuilder coder is Latin1. The chars are always '0'-'9' (Latin1).
        buf.append((char) packed);
        buf.append((char) (packed >> 8));
    }
    ...
    public static void appendQuad(StringBuilder buf, int v) {
        ...
        // Use append(char) to allow StringOpts optimization to combine into appendLatin1
        // when the StringBuilder coder is Latin1. The chars are always '0'-'9' (Latin1).
        buf.append((char) packedHigh);
        buf.append((char) (packedHigh >> 8));
        buf.append((char) packedLow);
        buf.append((char) (packedLow >> 8));
    }
}
