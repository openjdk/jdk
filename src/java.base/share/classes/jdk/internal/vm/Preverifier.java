...
public class Preverifier extends ClassVisitor {
     ...
     */
    public static byte[] patch(byte [] bytecode, boolean verbose) {
        shouldPrint = verbose;
        ClassReader cr;
        HashMap<String, Integer> methodMap = new HashMap<String, Integer>();
        cr = new ClassReader(bytecode);
        ClassNode cn = replaceOpcodes(cr, bytecode, methodMap);
        ...
        if (shouldPrint) {
            try {  // write out new .class file.
                String fname = cn.name.substring(cn.name.lastIndexOf("/") + 1);
                FileOutputStream fos = new FileOutputStream(new File(fname + ".class"));
                fos.write(cw.toByteArray());
            } catch (Throwable th) {
                log("Write of stream failed, name: " + cn.name, shouldPrint);
            }
        }
        checkStackSize(cw.toByteArray(), methodMap);
        log("Finished preverifer", shouldPrint);
 ...
 //    }
     ...
     */
    public static void checkStackSize(byte[] bytecode, HashMap<String, Integer> methodMap) {
        log("Verifying stack size", shouldPrint);
        ...
        for (MethodNode mn : mns) {
            String mapKey = mn.name + mn.desc;
            if (mn.maxStack > methodMap.get(mapKey)) {
                log("Stack sized changed", shouldPrint);
                throw new ClassFormatError("Altered stack size, method: " + mapKey +
                                           ", mn.maxStack: " + mn.maxStack +
                                           ", methodMap.get(mapKey): " +  methodMap.get(mapKey));
            }
        }
     ...
     */
    public static ClassNode replaceOpcodes(ClassReader cr, byte[] bytecode, HashMap<String, Integer> methodMap) {
        ...
        // HashMap<String, Integer> methodMap = new HashMap<String, Integer>();
        log("Class name: " + cn.name + ", Methods: " + mns.size(), shouldPrint);
        for (MethodNode mn : mns) {
            int timesScanned = 0;
            ...
            methodMap.put(mn.name + mn.desc, mn.maxStack);
            do {
                log("Method name: " + mn.name + mn.desc + " Instructions: " + inList.size(), shouldPrint);
                for (int i = 0; i < inList.size(); i++) {
                    ...
                    if (inList.get(i).getOpcode() == Opcodes.JSR || inList.get(i).getOpcode() == 201) { // JSR_W = 201
                        ...
                        if (mustExpand) {
                            ...
                            for (AbstractInsnNode n = inList.get(inList.indexOf(lb)+1);
                                 n.getOpcode() != Opcodes.RET; n=n.getNext()) {
                                // If there is a JSR in the code to be copied,
                                // replace it with the subroutine it points to.
