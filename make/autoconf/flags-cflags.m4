...
[
  ...
  if test "x${OPENJDK_$1_OS}" = xmacosx; then
    $1_OS_CFLAGS="-DMAC_OS_X_VERSION_MIN_REQUIRED=$MACOSX_VERSION_MIN_NODOTS \
        -mmacosx-version-min=$MACOSX_VERSION_MIN"

    if test -n "$MACOSX_VERSION_MAX"; then
        $1_OS_CFLAGS="${$1_OS_CFLAGS} \
            -DMAC_OS_X_VERSION_MAX_ALLOWED=$MACOSX_VERSION_MAX_NODOTS"
    fi
  fi

  $1_OS_CFLAGS="${$1_OS_CFLAGS} -DLIBC=${OPENJDK_$1_LIBC}"
  if test "x${OPENJDK_$1_LIBC}" = xmusl; then
    $1_OS_CFLAGS="${$1_OS_CFLAGS} -DMUSL_LIBC"
  fi
  ...
  $1_CFLAGS_JVM="${$1_DEFINES_CPU_JVM} ${$1_CFLAGS_CPU} ${$1_CFLAGS_CPU_JVM} \
      ${$1_TOOLCHAIN_CFLAGS} ${$1_OS_CFLAGS} ${$1_WARNING_CFLAGS_JVM}"
  $1_CFLAGS_JDK="${$1_DEFINES_CPU_JDK} ${$1_CFLAGS_CPU} ${$1_CFLAGS_CPU_JDK} \
      ${$1_TOOLCHAIN_CFLAGS} ${$1_OS_CFLAGS}"
